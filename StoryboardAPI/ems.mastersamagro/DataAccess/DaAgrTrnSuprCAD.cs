...
{
    /// <summary>
    /// This DataAccess will provide access to various functionalities in Post Tac Activity menu (Summaries, Process - accept/sendback cases)
    /// </summary>
    /// <remarks>Written by Sherin Augusta, Logapriya, Abilash.A, Premchander.K </remarks>
    public class DaAgrTrnSuprCAD
    {
        ...
        int mnResult, mnResult1, ls_port, mnResult2;
        ...
        public void DaUpdateProcessType(string employee_gid, MdlUpdateProcessType values)
        {
            ...
            if (mnResult != 0)
            {
                msSQL = "update agr_trn_tsuprprocesstype_assign set application_gid='" + values.application_gid + "' where application_gid='" + employee_gid + "'";
        ...
        public void DaGetPendingCADReviewSummary(string employee_gid, MdlMstCAD values)
        {
            ...
            //         " a.creditgroup_gid,a.product_gid,a.variety_gid from agr_mst_tsuprapplication a " +
            ...
            msSQL = "call agr_trn_suprpmgpendingapplicationsummary";
        ...
        public void DaGetSentBackToCCSummary(string employee_gid, MdlMstCAD values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customerref_name,a.customer_urn,a.creditgroup_name, " +
                     ...
                     " a.creditgroup_gid from agr_mst_tsuprapplication a " +
        ...
        public void DaGetCADAcceptedCustomerSummary(string employee_gid, MdlMstCAD values)
        {
            ...
            //         " group by a.application_gid order by a.updated_date desc ";
            msSQL = "call agr_trn_suprpmgacceptedapplicationsummary";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            var getapplicationadd_list = new List<cadapplicationlist>();
            if (dt_datatable.Rows.Count != 0)
            {
        ...
        public void DaGetCCRejectedSummary(string employee_gid, MdlMstCAD values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customerref_name,a.customer_urn,a.creditgroup_name, " +
                     ...
                     " date_format(a.updated_date, '%d-%m-%Y %h:%i %p') as ccrejected_date from agr_mst_tsuprapplication a " +
                     " left join hrm_mst_temployee b on b.employee_gid = a.updated_by " +
                     " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
        ...
        {
            msSQL = " select count(application_gid) as cadreview_count from agr_mst_tsuprapplication a " +
                     " where a.approval_status = 'CC Approved' and a.process_type is null";
            values.cadreview_count = objdbconn.GetExecuteScalar(msSQL);
            int cadreview_count = Convert.ToInt16(values.cadreview_count);

            msSQL = " select count(application_gid) as sentbackcc_count from agr_mst_tsuprapplication a " +
                     " where a.process_type = 'Sendback to CC'";
            values.sentbackcc_count = objdbconn.GetExecuteScalar(msSQL);
            int sentbackcc_count = Convert.ToInt16(values.sentbackcc_count);

            msSQL = " select count(application_gid) as accept_count from agr_mst_tsuprapplication a " +
                    " where a.process_type = 'Accept'";
            values.accept_count = objdbconn.GetExecuteScalar(msSQL);
            int accept_count = Convert.ToInt16(values.accept_count);

            msSQL = " select count(application_gid) as backtounderwriting_count from agr_mst_tsuprapplication a " +
                     " where a.process_type = 'Sendback to Credit Underwriting'";
            ...
            msSQL = " select count(application_gid) as ccrejected_count from agr_mst_tsuprapplication a " +
                     " where a.approval_status = 'CC Rejected' and a.process_type is null";
        ...
        public void DaPostProcessType(MdlProcessType values, string employee_gid)
        {
            if (values.applyall_flag == "N")
            {
                for (var i = 0; i < values.menulist.Count; i++)
                {
                    ...
                    msSQL = " insert into agr_trn_tsuprprocesstype_assign(" +
            ...
            {
                msSQL = " select module_code as menu_gid,module_name as menu_name from adm_mst_tmodule where module_gid in ('AGDSMPSDC','AGDSMPSPD','AGDSMPSCM', 'AGDSMPSSD')" +
                        " and module_gid not in (select menu_gid from agr_trn_tsuprprocesstype_assign where application_gid = '" + values.application_gid + "')";
        ...
        public void DaGetProcessTypeSummary(string application_gid, MdlMstProcessTypeSummary objmaster, string employee_gid)
        {
            try
            {
                msSQL = " SELECT a.processtypeassign_gid, a.application_gid, a.processtype_name, a.cadgroup_name, a.menu_name, " +
                        ...
                        " FROM agr_trn_tsuprprocesstype_assign a" +
                ...
                msSQL = " select onboarding_status from agr_mst_tsuprapplication where application_gid = '" + application_gid + "'";
        ...
        public void DaGetCADBasicView(string application_gid, MdlMstCAD values)
        {
            try
            {
                ...
                msSQL = " select stakeholder_type, stakeholdertype_gid from agr_mst_tsuprinstitution where application_gid = '" + application_gid + "'" +
                        " union " +
                        " select stakeholder_type, stakeholdertype_gid from agr_mst_tsuprcontact where application_gid = '" + application_gid + "' ";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetSanctionToList(string sanctiontype_name, string application_gid, MdlMstCAD values)
        {
            if (sanctiontype_name == "Applicant")
            {
                ...
                {
                    msSQL = " select company_name, institution_gid from agr_mst_tsuprinstitution where application_gid = '" + application_gid + "'" +
                ...
                {
                    msSQL = " select concat(first_name, ' ', middle_name, ' ', last_name) as individual_name, contact_gid from agr_mst_tsuprcontact" +
            ...
            {
                msSQL = " select a.company_name as sanctionto_name, a.institution_gid as sanctionto_gid from agr_mst_tsuprinstitution a " +
                        " inner join agr_mst_tsuprapplication b on a.application_gid = b.application_gid " +
                        " where b.application_gid = '" + application_gid + "' and a.stakeholder_type = '" + sanctiontype_name + "'" +
                        " union " +
                        " select concat(first_name, ' ', middle_name, ' ', last_name) as sanctionto_name, a.contact_gid as sanctionto_gid from agr_mst_tsuprcontact a " +
                        " inner join agr_mst_tsuprapplication b on a.application_gid = b.application_gid " +
                        " where b.application_gid = '" + application_gid + "' and a.stakeholder_type = '" + sanctiontype_name + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getsanctionto_list = new List<sanctionto_list>();
        ...
        {
            msSQL = " select contact_gid from agr_mst_tsuprcontact where contact_gid = '" + sanctionto_gid + "'";
            objODBCDataReader = objdbconn.GetDataReader(msSQL);
            if (objODBCDataReader.HasRows == true)
            {
                objODBCDataReader.Close();
                msSQL = "select concat(first_name,',',middle_name,',',last_name) as contactperson_name from agr_mst_tsuprcontact where contact_gid = '" + sanctionto_gid + "'";
                ...
                //msSQL = "select concat(addressline1,',',addressline2) as primary_address from agr_mst_tsuprcontact2address where contact_gid = '" + sanctionto_gid + "' and primary_status='Yes'";
                //objODBCDataReader1 = objdbconn.GetDataReader(msSQL);
                //if (objODBCDataReader1.HasRows == true)
                //{
                ...
                msSQL = " select concat(addressline1,',',addressline2) as address, contact2address_gid as address_gid from agr_mst_tsuprcontact2address where contact_gid = '" + sanctionto_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                msSQL = " select mobile_no, contact2mobileno_gid as mobileno_gid from agr_mst_tsuprcontact2mobileno where contact_gid = '" + sanctionto_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                msSQL = " select email_address, contact2email_gid as email_gid from agr_mst_tsuprcontact2email where contact_gid = '" + sanctionto_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
            ...
            {
                objODBCDataReader.Close();
                msSQL = "select concat(contactperson_firstname,',',contactperson_middlename,',',contactperson_lastname) as contactperson_name from agr_mst_tsuprinstitution where institution_gid = '" + sanctionto_gid + "'";
                objODBCDataReader1 = objdbconn.GetDataReader(msSQL);
                if (objODBCDataReader1.HasRows == true)
                {
                    values.contactperson_name = objODBCDataReader1["contactperson_name"].ToString();
                ...
                //msSQL = "select concat(addressline1,',',addressline2) as primary_address from agr_mst_tsuprinstitution2address where institution_gid = '" + sanctionto_gid + "' and primary_status='Yes'";
                //objODBCDataReader1 = objdbconn.GetDataReader(msSQL);
                //if (objODBCDataReader1.HasRows == true)
                //{
                //    values.primary_address = objODBCDataReader1["primary_address"].ToString();
                //}
                //objODBCDataReader1.Close();
                msSQL = " select concat(addressline1,',',addressline2) as address, institution2address_gid as address_gid from agr_mst_tsuprinstitution2address where institution_gid = '" + sanctionto_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var geaddress_list = new List<cadaddress_list>();
                if (dt_datatable.Rows.Count != 0)
                {
                ...
                msSQL = " select mobile_no, institution2mobileno_gid as mobileno_gid from agr_mst_tsuprinstitution2mobileno where institution_gid = '" + sanctionto_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getmobileno_list = new List<cadmobileno_list>();
                if (dt_datatable.Rows.Count != 0)
                {
                ...
                msSQL = " select email_address, institution2email_gid as email_gid from agr_mst_tsuprinstitution2email where institution_gid = '" + sanctionto_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getemail_list = new List<cademail_list>();
                if (dt_datatable.Rows.Count != 0)
                {
        ...
        {
            msSQL = " select application2loan_gid from agr_mst_tsuprapplication2loan where application_gid='" + application_gid + "' and product_type='Agri Receivable Finance (ARF)'";
            ...
            msSQL = " select application2buyer_gid,buyer_gid,buyer_name,buyer_limit,availed_limit,balance_limit,margin,bill_tenure " +
                   " from agr_mst_tsuprapplication2buyer where application2loan_gid='" + lsapplication2loan_gid + "'" +
                   " union" +
        ...
        public void DaGetProductList(string application_gid, MdlMstCAD values)
        {
            try
            {
                msSQL = "select product_type, producttype_gid from agr_mst_tsuprapplication2loan where application_gid='" + application_gid + "'";
        ...
        public void DaGetSubProductList(string producttype_gid, string application_gid, MdlMstCAD values)
        {
            try
            {
                msSQL = "select productsub_type,productsubtype_gid from agr_mst_tsuprapplication2loan where application_gid='" + application_gid + "' and producttype_gid='" + producttype_gid + "'";
        ...
        public void DaGetProductDetail(string application_gid, string productsubtype_gid, MdlMstCAD values)
        {
            msSQL = "select loanfacility_amount, rate_interest, facility_mode, tenureoverall_limit, loan_type, facility_type " +
                   " from agr_mst_tsuprapplication2loan where productsubtype_gid='" + productsubtype_gid + "' and application_gid='" + application_gid + "' ";
            objODBCDataReader = objdbconn.GetDataReader(msSQL);
            if (objODBCDataReader.HasRows == true)
            {
        ...
        public void DaGetMOMCAMDocument(string application_gid, MdlMstCAD values)
        {
            msSQL = " select application2momdoc_gid,application_gid,document_name,document_path,document_title, date_format(a.created_date,'%d-%m-%Y %H:%i %p') as created_date," +
                    " concat(c.user_firstname,' ',c.user_lastname,' / ',c.user_code) as created_by" +
                    " from  agr_mst_tsuprapplication2momdoc a " +
                    " left join hrm_mst_temployee b on a.created_by=b.employee_gid" +
            ...
            msSQL = " select application2camdoc_gid,application_gid,document_name,document_path,document_title, date_format(a.created_date,'%d-%m-%Y %H:%i %p') as created_date," +
                   " concat(c.user_firstname,' ',c.user_lastname,' / ',c.user_code) as created_by" +
                   " from  agr_mst_tsuprapplication2camdoc a " +
                   " left join hrm_mst_temployee b on a.created_by=b.employee_gid" +
                   " left join adm_mst_tuser c on b.user_gid = c.user_gid" +
                   " where application_gid='" + application_gid + "'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        {
            msSQL = " update agr_mst_tsuprapplication2loan set " +
        ...
        public void DaGetLoanDetail(string application2loan_gid, cadloanfacilitytype_list values)
        {
            msSQL = " select application2loan_gid,interchangeability,report_structure,document_limit, date_format(expiry_date,'%Y-%m-%d') as expiry_date" +
                    " from agr_mst_tsuprapplication2loan " +
        ...
        public bool DaPostCADSanction(string employee_gid, cadsanctiondetails values)
        {
            ...
            msSQL = " select b.entity_name,b.vertical_code from agr_mst_tsuprapplication a " +
            ...
            msSQL = " INSERT INTO agr_trn_tsuprapplication2sanction( " +
            ...
            msSQL = " select application_gid,relationshipmanager_name, relationshipmanager_gid, clustermanager_gid, clustermanager_name, zonalhead_name, zonalhead_gid," +
                    " regionalhead_name, regionalhead_gid, businesshead_name, businesshead_gid from agr_mst_tsuprapplication" +
                    " where application_gid = '" + values.application_gid + "'";
            objODBCDataReader = objdbconn.GetDataReader(msSQL);
            if (objODBCDataReader.HasRows == true)
            {
                msSQL = " update agr_trn_tsuprapplication2sanction set relationshipmgr_gid='" + objODBCDataReader["relationshipmanager_gid"].ToString() + "'," +
            ...
            if (mnResult1 != 0)
            {
                ...
                msSQL = " select application2loan_gid from agr_mst_tsuprapplication2loan where application_gid='" + values.application_gid + "' and productsub_type='Agri Receivable Finance (ARF)'";
                string lsapplication2loan_gid = objdbconn.GetExecuteScalar(msSQL);
        ...
        public void DaPostlimitproductinfo(string employee_gid, limitandproducts values)
        {
            msSQL = " select producttype_gid,product_type,productsubtype_gid,productsub_type,loanfacility_amount " +
                   " from agr_mst_tsuprapplication2loan where application2loan_gid = '" + values.application2loan_gid + "'";
            objODBCDataReader = objdbconn.GetDataReader(msSQL);
            if (objODBCDataReader.HasRows == true)
            {
            ...
            if (mnResult != 0)
            {
                msSQL = "update agr_mst_tsuprapplication2loan set limit_product='Y' where application2loan_gid ='" + values.application2loan_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                values.status = true;
        ...
        public void DAGetAppSanctionSummary(string application_gid, MdlMstCAD values)
        {
            msSQL = " select a.application2sanction_gid, a.sanction_refno, date_format(a.sanction_date,'%d-%m-%Y') as sanction_date, a.application_name," +
                    ...
                    " from agr_trn_tsuprapplication2sanction a " +
                    " left join agr_mst_tsuprapplication b on b.application_gid = a.application_gid" +
        ...
        public void DaCADSanctionDtls(string sanction_gid, cadsanctiondetails values)
        {
            try
            {
                ...
                "  format((sanction_amount), 2) as sanction_amount,a.sanctionto_name, date_format(sanction_date,'%d-%m-%Y') as sanctionDate," +
                 " a.esdeclaration_status,a.makerfile_path,a.makerfile_name, checkerletter_flag, checkerapproval_flag FROM agr_trn_tsuprapplication2sanction a " +
        ...
        {
            //msSQL = " update agr_trn_tsuprapplication2sanction set template_name='" + values.template_name + "', template_gid='" + values.template_gid + "' where application2sanction_gid='" + values.sanction_gid + "'";
            //mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
            ...
            msSQL = " select concat(product_type,'-', productsub_type) as product_type,format(loanfacility_amount,2) as facility_limit, " +
                  ...
                  " from agr_mst_tsuprapplication2loan where application_gid = '" + lsapplication_gid + "'";
        ...
        {
            msSQL = " select template_name, template_gid from agr_trn_tsuprapplication2sanction where application2sanction_gid='" + values.sanction_gid + "'";
            objODBCDataReader = objdbconn.GetDataReader(msSQL);
            if (objODBCDataReader.HasRows == true)
            {
                values.template_name = objODBCDataReader["template_name"].ToString();
            ...
            if (mnResult == 1)
            {
                ...
                msSQL = " update agr_trn_tsuprapplication2sanction set makerfile_path='" + values.lspath + "', makerfile_name='" + values.lsname + "', sanctionletter_status='Generated'," +
        ...
        {
            msSQL = "update agr_trn_tsuprapplication2sanction set sanctionletter_flag='Y', sanction_status='Checker Approval Pending' where application2sanction_gid='" + values.sanction_gid + "'";
            mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
            if (mnResult != 0)
            {
                msSQL = " select  processtypeassign_gid from agr_trn_tsuprprocesstype_assign a " +
                         " left join agr_trn_tsuprapplication2sanction b on a.application_gid = b.application_gid " +
        ...
        public bool DaSanctionToCheckerSummary(sanctiondetailsList values, string employee_gid)
        {
            msSQL = " SELECT a.application2sanction_gid, a.sanction_refno,date_format(a.sanction_date,'%d-%m-%Y') as sanction_date, " +
                   ...
                   " reset_flag, e.cadgroup_name FROM agr_trn_tsuprapplication2sanction a " +
                   " LEFT JOIN agr_mst_tsuprapplication b ON a.application_gid = b.application_gid" +
        ...
        {
            msSQL = " update agr_trn_tsuprapplication2sanction set checkerletter_flag='Y', sanction_status ='Final Approval Pending', checkerupdated_by ='" + employee_gid + "', " +
            ...
            if (mnResult != 0)
            {
                ...
                msSQL = " select template_name, template_gid, template_content from agr_trn_tsuprapplication2sanction where application2sanction_gid='" + values.sanction_gid + "'";
                objODBCDataReader = objdbconn.GetDataReader(msSQL);
                if (objODBCDataReader.HasRows == true)
                {
                    values.template_name = objODBCDataReader["template_name"].ToString();
        ...
        {
            msSQL = " update agr_trn_tsuprapplication2sanction set sanctionletter_flag='N', checkerpushback_remarks='" + values.pushback_remarks.Replace("'", "") + "'," +
            ...
            if (mnResult != 0)
            {
                ...
                if (lsprocesstypeassign_gid != "")
                {
                    msSQL = " update agr_trn_tsuprprocesstype_assign set maker_approvalflag='N', " +
        ...
        {
            msSQL = " SELECT template_content FROM agr_trn_tsuprapplication2sanction where application2sanction_gid='" + sanction_gid + "'";
            ...
            try
            {
                ...
                msSQL = " update agr_trn_tsuprapplication2sanction set digitalsignature_flag='Y', makerfile_path='" + values.lspath + "', makerfile_name='" + values.lsname + "'" +
                       " where application2sanction_gid='" + sanction_gid + "' ";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
        ...
        public void DaGetPDFGenerate(string sanction_gid, string employee_gid, cadtemplate_list values)
        {
            try
            {
                ...
                msSQL = " SELECT makerfile_path, makerfile_name FROM agr_trn_tsuprapplication2sanction where application2sanction_gid='" + sanction_gid + "'";
                objODBCDataReader = objdbconn.GetDataReader(msSQL);
                if (objODBCDataReader.HasRows == true)
                {
        ...
        public void DaUpdateCheckerApproval(cadtemplate_list values, string employee_gid)
        {
            ...
            {
                msSQL = " update agr_trn_tsuprapplication2sanction set checkerapproval_flag='Y', sanction_status='" + values.sanction_status + "', checkerapproved_by='" + employee_gid + "',";
            ...
            {
                msSQL = " update agr_trn_tsuprapplication2sanction set checkerapproval_flag='R', sanction_status='" + values.sanction_status + "', checkerapproved_by='" + employee_gid + "',";
                if (values.reject_remarks == "" || values.reject_remarks == null || values.reject_remarks == "undefined")
                {
                    msSQL += " checkerreject_remarks='', ";
                }
            ...
            if (mnResult != 0)
            {
                ...
                msSQL = " select a.processtypeassign_gid,a.application_gid from agr_trn_tsuprprocesstype_assign a " +
                      " left join agr_trn_tsuprapplication2sanction b on a.application_gid = b.application_gid " +
                      " where application2sanction_gid = '" + values.sanction_gid + "' and menu_gid ='" + getMenuClass.Sanction + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                ...
                if (lsprocesstypeassign_gid != "" && values.sanction_status == "Approved")
                {
                    ...
                    msSQL = " update agr_mst_tsuprapplication set sanction_approvalflag='Y' " +
                            " where application_gid='" + lsapplication_gid + "'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                }

                msSQL = " select template_name, template_gid, template_content from agr_trn_tsuprapplication2sanction " +
                        " where application2sanction_gid='" + values.sanction_gid + "'";
                objODBCDataReader = objdbconn.GetDataReader(msSQL);
                if (objODBCDataReader.HasRows == true)
                {
        ...
        {
            msSQL = " select count(application_gid) as cadsanction_count from agr_mst_tsuprapplication a " +
                    " where a.process_type = 'Accept'";
            values.cadmaker_count = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " select count(application2sanction_gid) as cadchecker_count from agr_trn_tsuprapplication2sanction a " +
                     " where a.sanctionletter_flag='Y' and a.checkerletter_flag='N' ";
            values.cadchecker_count = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " select count(application2sanction_gid) as cadcheckerapproval_count from agr_trn_tsuprapplication2sanction a " +
        ...
        {
            msSQL = " select count(*) as count from agr_mst_tsuprapplication2loan a " +
                    " left join agr_trn_tsuprapplication2sanction b on a.application_gid = b.application_gid " +
                    " where application2sanction_gid = '" + sanction_gid + "'";
            ...
            msSQL = " update agr_trn_tsuprapplication2sanction set template_name='" + lstemplate_name + "', template_gid='" + lstemplate_gid + "' where application2sanction_gid='" + sanction_gid + "'";
            mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
            //else
            //{
        ...
        public bool DaGetDocumentTypeList(string credit_gid, MdlMstCADCompany values)
        {
            msSQL = " SELECT companydocument_gid,companydocument_name,documenttypes_gid,documenttype_name,covenant_type " +
                   ...
                   " from agr_trn_tsuprdocumentchecktls where credit_gid = '" + credit_gid + "' and untagged_type is null group by companydocument_gid)";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        {
            msSQL = "select institution_gid from agr_mst_tsuprinstitution where institution_gid='" + values.credit_gid + "'";
            string lsinstitution = objdbconn.GetExecuteScalar(msSQL);
            msSQL = "select contact_gid from agr_mst_tsuprcontact where contact_gid='" + values.credit_gid + "'";
            string lsindividual = objdbconn.GetExecuteScalar(msSQL);
            msSQL = "select group_gid from agr_mst_tsuprgroup where group_gid='" + values.credit_gid + "'";
            ...
            foreach (string i in values.document_gid)
            {
                ...
                if (lsinstitution != "")
                {
                    ...
                    msSQL = " select groupdocumentchecklist_gid from agr_trn_tsuprdocumentchecktls where application_gid='" + values.application_gid + "'" +
                ...
                {
                    msSQL = " update agr_trn_tsuprdocumentchecktls set untagged_type=null, " +
                   ...
                   " where groupdocumentchecklist_gid='" + lsgroupdocumentchecklist_gid + "'";
                    ...
                    msSQL = " update agr_trn_tsuprgroupdocumentchecklist set untagged_type=null, " +
        ...
        //Individual Document
        public bool DaGetIndividualTypeList(MdlMstCAD values, string credit_gid)
        {
            msSQL = " SELECT individualdocument_gid,individualdocument_name,documenttypes_gid,documenttype_name,covenant_type " +
                  ...
                  " from agr_trn_tsuprdocumentchecktls where credit_gid = '" + credit_gid + "' and untagged_type is null group by individualdocument_gid)";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            var getDocList = new List<CADDocument>();
            if (dt_datatable.Rows.Count != 0)
            {
        ...
        public bool DaGetCovenantIndividualDocumentList(MdlMstCADCompany values, string credit_gid)
        {
            if (credit_gid != "")
            {
                msSQL = " SELECT individualdocument_gid,individualdocument_name,documenttypes_gid,documenttype_name,covenant_type " +
                  ...
                  " from agr_trn_tsuprcovanantdocumentcheckdtls where credit_gid = '" + credit_gid + "' and untagged_type is null group by individualdocument_gid)";
            }
            else
            {
                msSQL = " SELECT individualdocument_gid,individualdocument_name,documenttypes_gid,documenttype_name,covenant_type " +
            ...
            foreach (string i in values.document_gid)
            {
                ...
                if (lsgroupdocumentchecklist_gid != "")
                {
                   ...
                   " where groupdocumentchecklist_gid='" + lsgroupdocumentchecklist_gid + "'";
                    ...
                    msSQL = " update agr_trn_tsuprgroupdocumentchecklist set untagged_type=null where groupdocumentchecklist_gid='" + lsgroupdocumentchecklist_gid + "'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                }
                else
                {
        ...
        public bool DaGetGroupTypeList(MdlMstCAD values, string credit_gid)
        {
            msSQL = " SELECT groupdocument_gid,groupdocument_name,documenttypes_gid,documenttype_name,covenant_type " +
                  ...
                  " from agr_trn_tsuprdocumentchecktls where credit_gid = '" + credit_gid + "' and untagged_type is null group by groupdocument_gid)";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            var getDocList = new List<CADDocument>();
            if (dt_datatable.Rows.Count != 0)
            {
        ...
        public bool DaGetCovenantGroupDocumentList(MdlMstCADCompany values, string credit_gid)
        {
            if (credit_gid != "")
            {
                msSQL = " SELECT groupdocument_gid,groupdocument_name,documenttypes_gid,documenttype_name,covenant_type " +
                        ...
                        " from agr_trn_tsuprcovanantdocumentcheckdtls where credit_gid = '" + credit_gid + "' and untagged_type is null group by groupdocument_gid)";
            }
            else
            {
                msSQL = " SELECT groupdocument_gid,groupdocument_name,documenttypes_gid,documenttype_name,covenant_type " +
        ...
        //{
        //    msSQL = "select institution2documentupload_gid from agr_mst_tsuprinstitution2documentupload where institution_gid='" + credit_gid + "'";
        //    string lsinstitution = objdbconn.GetExecuteScalar(msSQL);
        //    msSQL = "select contact2document_gid from agr_mst_tsuprcontact2document where contact_gid='" + credit_gid + "'";
        //    string lsindividual = objdbconn.GetExecuteScalar(msSQL);
        //    msSQL = "select group2document_gid from agr_mst_tsuprgroup2document where group_gid='" + credit_gid + "'";
        //    string lsgroup = objdbconn.GetExecuteScalar(msSQL);
        //    if (lsinstitution != "")
        //    {
        ...
        //           " (SELECT COUNT(*) FROM agr_trn_tsuprdocumentchecktls x" +
        ...
        //           " FROM agr_trn_tsuprdocumentchecktls a" +
        ...
        //            " from agr_mst_tsuprcontact2document a " +
        //            " left join ocs_mst_tindividualdocument b on a.individualdocument_gid = b.individualdocument_gid " +
        ...
        //            " document_title as documenttype_name, document_name, document_path, b.covenant_type " +
        //            " from agr_mst_tsuprgroup2document a " +
        //            " left join ocs_mst_tgroupdocument b on a.groupdocument_gid = b.groupdocument_gid " +
        ...
        public bool DaGetCADTrnCovenantTaggedDocList(MdlMstCAD values, string credit_gid)
        {
            ...
            if (lsinstitution != "")
            {
                msSQL = " SELECT groupcovdocumentchecklist_gid,a.companydocument_gid,documenttype_gid,documenttype_code,a.documenttype_name,a.covenant_periods, " +
                        " a.covenant_type,application_gid,a.tagged_name, a.tagged_by as taggedby  " +
                        " FROM agr_trn_tsuprcovanantdocumentcheckdtls a" +
            ...
            else if (lsindividual != "")
            {
                msSQL = " SELECT groupcovdocumentchecklist_gid,a.individualdocument_gid as companydocument_gid,documenttype_gid,documenttype_code, " +
                        " a.documenttype_name,a.covenant_periods, a.covenant_type,application_gid,a.tagged_name, a.tagged_by as taggedby  " +
                       " FROM agr_trn_tsuprcovanantdocumentcheckdtls a" +
                       " WHERE credit_gid = '" + credit_gid + "' and a.covenant_type='Y' and a.untagged_type is null or a.untagged_type = 'N'" +
        ...
        public bool DaGetCADTaggedDocList(MdlMstCAD values, string credit_gid)
        {
            ...
            if (lsinstitution != "")
            {
                msSQL = " SELECT groupdocumentchecklist_gid,documentcheckdtl_gid,documenttype_gid,documenttype_code,a.documenttype_name, " +
                       " a.tagged_by, b.covenant_type,application_gid FROM agr_trn_tsuprdocumentchecktls a " +
                       " left join ocs_mst_tcompanydocument b on a.companydocument_gid = b.companydocument_gid " +
                       " WHERE credit_gid = '" + credit_gid + "' and a.untagged_type is null or a.untagged_type = 'N' GROUP BY a.companydocument_gid";
            }
            else if (lsindividual != "")
        ...
        public bool DaGetCADCovenantTaggedDocList(MdlMstCAD values, string credit_gid)
        {
            ...
            if (lsinstitution != "")
            {
                msSQL = " SELECT groupcovdocumentchecklist_gid,covenantdocumentcheckdtl_gid,a.companydocument_gid,documenttype_gid, " +
                    " documenttype_code,a.documenttype_name,a.covenant_periods, b.covenant_type, application_gid, a.tagged_by as taggedby  " +
                    " FROM agr_trn_tsuprcovanantdocumentcheckdtls a" +
                    " left join ocs_mst_tcompanydocument b on a.companydocument_gid = b.companydocument_gid " +
        ...
        //            " (SELECT COUNT(*) FROM agr_trn_tsuprcovanantdocumentcheckdtls x" +
        //            " WHERE x.documenttype_name = a.documenttype_name AND x.credit_gid = a.credit_gid GROUP BY x.documenttype_name) as documenttype_count" +
        //            " FROM agr_trn_tsuprcovanantdocumentcheckdtls a" +
        //            " left join ocs_mst_tcompanydocument b on a.companydocument_gid = b.companydocument_gid " +
        ...
        public void DaUnTagDocument(string documentcheckdtl_gid, result objResult, string user_gid)
        {
            ...
            msSQL = "select groupdocumentchecklist_gid from agr_trn_tsuprgroupdocumentchecklist where groupdocumentchecklist_gid='" + documentcheckdtl_gid + "'";
            string lsdeferral = objdbconn.GetExecuteScalar(msSQL);
            if (lsdeferral != "")
            {
                msSQL = " update agr_trn_tsuprdocumentchecktls set untagged_type='Y', " +
                     " untagged_by='" + user_gid + "', " +
                     " untagged_date=current_timestamp" +
                     " where groupdocumentchecklist_gid='" + documentcheckdtl_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                msSQL = " update agr_trn_tsuprgroupdocumentchecklist set untagged_type='Y' where groupdocumentchecklist_gid='" + documentcheckdtl_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
            }
            else
            {
                msSQL = " update agr_trn_tsuprcovanantdocumentcheckdtls set untagged_type='Y', " +
                    " untagged_by='" + user_gid + "', " +
                    " untagged_date=current_timestamp" +
                    " where groupcovdocumentchecklist_gid='" + documentcheckdtl_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                msSQL = " update agr_trn_tsuprgroupcovenantdocumentchecklist set untagged_type='Y' where groupcovdocumentchecklist_gid='" + documentcheckdtl_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
            }
            //msSQL = "select covenantdocumentcheckdtl_gid from agr_trn_tsuprcovanantdocumentcheckdtls where covenantdocumentcheckdtl_gid='" + documentcheckdtl_gid + "'";
            //string lscovenant = objdbconn.GetExecuteScalar(msSQL);
            //if (lscovenant != "")
            //    msSQL = "DELETE FROM agr_trn_tsuprcovanantdocumentcheckdtls WHERE covenantdocumentcheckdtl_gid='" + documentcheckdtl_gid + "'";
            //else
            //    msSQL = "DELETE FROM agr_trn_tsuprdocumentchecktls WHERE documentcheckdtl_gid='" + documentcheckdtl_gid + "'";
            //mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

            //msSQL = "select institution2documentupload_gid from agr_mst_tsuprinstitution2documentupload where institution2documentupload_gid='" + documentcheckdtl_gid + "'";
            //string lsinstitution = objdbconn.GetExecuteScalar(msSQL);
            //msSQL = "select contact2document_gid from agr_mst_tsuprcontact2document where contact2document_gid='" + documentcheckdtl_gid + "'";
            //string lsindividual = objdbconn.GetExecuteScalar(msSQL);
            //msSQL = "select group2document_gid from agr_mst_tsuprgroup2document where group2document_gid='" + documentcheckdtl_gid + "'";
            //string lsgroup = objdbconn.GetExecuteScalar(msSQL);
            //if (lsinstitution != "")
            //{
            ...
            //else if (lsindividual != "")
            //{
            //    msSQL = " update agr_mst_tsuprcontact2document set untagged_type='Y', " +
            //       " untagged_by='" + user_gid + "', " +
            //       " untagged_date=current_timestamp" +
            ...
            //else if (lsgroup != "")
            //{
            //    msSQL = " update agr_mst_tsuprgroup2document set untagged_type='Y', " +
            //       " untagged_by='" + user_gid + "', " +
            //       " untagged_date=current_timestamp" +
        ...
        public void DaCheckALLDocumentList(MdlMstCAD values, string user_gid)
        {
            ...
            {
                msSQL = " select institution_gid from agr_mst_tsuprinstitution where application_gid = '" + values.application_gid + "' and institution_gid<> '" + values.credit_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow dt in dt_datatable.Rows)
                    {
                        msSQL = " select documenttype_gid, documenttype_code, documenttype_name from agr_trn_tsuprdocumentchecktls where credit_gid='" + values.credit_gid + "'";
                        dt_child = objdbconn.GetDataTable(msSQL);
                        if (dt_child.Rows.Count != 0)
                        {
            ...
            {
                msSQL = " select contact_gid from agr_mst_tsuprcontact where application_gid = '" + values.application_gid + "' and contact_gid<> '" + values.credit_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow dt in dt_datatable.Rows)
            ...
            {
                msSQL = " select group_gid from agr_mst_tsuprgroup where application_gid = '" + values.application_gid + "' and group_gid<> '" + values.credit_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow dt in dt_datatable.Rows)
        ...
        public void DaGetSanctionEdit(string application2sanction_gid, cadsanctiondetails values)
        {
            try
            {
                msSQL = " select application2sanction_gid, sanction_refno,  sanction_date, sanction_amount,  entity, " +
                        ...
                        " from agr_trn_tsuprapplication2sanction " +
                ...
                {
                    msSQL = " select applicant_type from agr_mst_tsuprapplication where application_gid = '" + values.application_gid + "'";
                    string lsapplicant_type = objdbconn.GetExecuteScalar(msSQL);
                    if (lsapplicant_type == "Institution")
                    {
                        msSQL = " select company_name, institution_gid from agr_mst_tsuprinstitution where application_gid = '" + values.application_gid + "'" +
                                " and stakeholder_type='Applicant'";
                        dt_datatable = objdbconn.GetDataTable(msSQL);
                        var getsanctionto_list = new List<sanctionto_list>();
                        if (dt_datatable.Rows.Count != 0)
                ...
                msSQL = " select contact_gid from agr_mst_tsuprcontact where contact_gid = '" + values.sanctionto_gid + "'";
                objODBCDataReader = objdbconn.GetDataReader(msSQL);
                if (objODBCDataReader.HasRows == true)
                {
                    objODBCDataReader.Close();
                    msSQL = "select concat(first_name,',',middle_name,',',last_name) as contactperson_name from agr_mst_tsuprcontact where contact_gid = '" + values.sanctionto_gid + "'";
                    objODBCDataReader1 = objdbconn.GetDataReader(msSQL);
                    if (objODBCDataReader1.HasRows == true)
                    {
                        values.contactperson_name = objODBCDataReader1["contactperson_name"].ToString();
                    ...
                    msSQL = " select concat(addressline1,',',addressline2) as address, contact2address_gid as address_gid from agr_mst_tsuprcontact2address where contact_gid = '" + values.sanctionto_gid + "'";
                    dt_datatable = objdbconn.GetDataTable(msSQL);
                    var geaddress_list = new List<cadaddress_list>();
                    if (dt_datatable.Rows.Count != 0)
                    {
                    ...
                    msSQL = " select mobile_no, contact2mobileno_gid as mobileno_gid from agr_mst_tsuprcontact2mobileno where contact_gid = '" + values.sanctionto_gid + "'";
                    dt_datatable = objdbconn.GetDataTable(msSQL);
                    var getmobileno_list = new List<cadmobileno_list>();
                    if (dt_datatable.Rows.Count != 0)
                    {
                    ...
                    msSQL = " select email_address, contact2email_gid as email_gid from agr_mst_tsuprcontact2email where contact_gid = '" + values.sanctionto_gid + "'";
                    dt_datatable = objdbconn.GetDataTable(msSQL);
                    var getemail_list = new List<cademail_list>();
                    if (dt_datatable.Rows.Count != 0)
                    {
                ...
                {
                    objODBCDataReader.Close();
                    msSQL = "select concat(contactperson_firstname,',',contactperson_middlename,',',contactperson_lastname) as contactperson_name from agr_mst_tsuprinstitution where institution_gid = '" + values.sanctionto_gid + "'";
                    objODBCDataReader1 = objdbconn.GetDataReader(msSQL);
                    if (objODBCDataReader1.HasRows == true)
                    {
                        values.contactperson_name = objODBCDataReader1["contactperson_name"].ToString();
                    ...
                    //}
                    //objODBCDataReader1.Close();
                    msSQL = " select concat(addressline1,',',addressline2) as address, institution2address_gid as address_gid from agr_mst_tsuprinstitution2address where institution_gid = '" + values.sanctionto_gid + "'";
                    dt_datatable = objdbconn.GetDataTable(msSQL);
                    var geaddress_list = new List<cadaddress_list>();
                    if (dt_datatable.Rows.Count != 0)
                    {
                    ...
                    msSQL = " select mobile_no, institution2mobileno_gid as mobileno_gid from agr_mst_tsuprinstitution2mobileno where institution_gid = '" + values.sanctionto_gid + "'";
                    dt_datatable = objdbconn.GetDataTable(msSQL);
                    var getmobileno_list = new List<cadmobileno_list>();
                    if (dt_datatable.Rows.Count != 0)
                    {
                    ...
                    msSQL = " select email_address, institution2email_gid as email_gid from agr_mst_tsuprinstitution2email where institution_gid = '" + values.sanctionto_gid + "'";
                    dt_datatable = objdbconn.GetDataTable(msSQL);
                    var getemail_list = new List<cademail_list>();
                    if (dt_datatable.Rows.Count != 0)
                    {
        ...
        public bool DaPostUpdateSanction(string employee_gid, cadsanctiondetails values)
        {
            ...
            msSQL = " update agr_trn_tsuprapplication2sanction set " +
                     " sanction_date='" + Convert.ToDateTime(values.sanction_date).ToString("yyyy-MM-dd") + "'," +
        ...
        public void DaGetCADDocChecklistFollowupMakerSummary(string employee_gid, MdlMstCAD values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customerref_name,a.customer_urn,a.creditgroup_name, " +
                     ...
                     " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
                     " left join agr_trn_tsuprprocesstype_assign d on d.application_gid = a.application_gid " +
                     " left join agr_trn_tsuprapplication2docchecklist f on f.application_gid = a.application_gid " +
        ...
        public void DaPostDocChecklistMakerSubmit(string employee_gid, MdlDocChecklistdetails values)
        {
            ...
            msSQL = " insert into agr_trn_tsuprapplication2docchecklist(" +
            ...
            if (mnResult != 0)
            {
                msSQL = " update agr_mst_tsuprapplication set docchecklist_makerflag='Y' where application_gid='" + values.application_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
        ...
        {
            public const string
                 DocumentChecklist = "AGDSMPSDC",
                 Sanction = "CADMGTSAN",
                 ScannedDocument = "AGDSMPSSD",
                 Physical_document = "AGDSMPSPD";
        }
        ...
        public void DaGetCADDocChecklistCheckerSummary(string employee_gid, MdlMstCAD values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customerref_name,a.customer_urn,a.creditgroup_name, " +
                     ...
                     " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as cadaccepted_by, " +
                     " date_format(a.processupdated_date, '%d-%m-%Y %h:%i %p') as cadaccepted_date, a.created_by, " +
                     " a.creditgroup_gid, e.cadgroup_name from agr_mst_tsuprapplication a " +
                     " left join hrm_mst_temployee b on b.employee_gid = a.processupdated_by " +
                     " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
        ...
        {
            msSQL = " update agr_trn_tsuprapplication2docchecklist set checkerapproved_by='" + employee_gid + "', " +
            ...
            if (mnResult != 0)
            {
                msSQL = " update agr_mst_tsuprapplication set docchecklist_checkerflag='Y' where application_gid='" + values.application_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
        ...
        public void DaGetCADDocChecklistApprovalSummary(string employee_gid, MdlMstCAD values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customerref_name,a.customer_urn,a.creditgroup_name, " +
                     ...
                     " a.creditgroup_gid, docchecklist_approvalflag, d.approval_status as approval, g.cadgroup_name from agr_mst_tsuprapplication a " +
                     " left join hrm_mst_temployee b on b.employee_gid = a.processupdated_by " +
                     " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
                     " left join agr_trn_tsuprapplication2docchecklist d on d.application_gid = a.application_gid " +
                     ...
                     " a.application_gid in (Select application_gid from agr_trn_tsuprprocesstype_assign where menu_gid = '" + getMenuClass.DocumentChecklist + "' and checker_flag='Y' " +
        ...
        {
            msSQL = " update agr_trn_tsuprapplication2docchecklist set approved_by='" + employee_gid + "', " +
            ...
            if (mnResult != 0)
            {
                msSQL = " update agr_mst_tsuprapplication set docchecklist_approvalflag='Y' where application_gid='" + values.application_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
        ...
        public void DaGetSanctionMakerSummary(string employee_gid, MdlMstCAD values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customerref_name,a.customer_urn,a.creditgroup_name, " +
                     ...
                     " left join hrm_mst_temployee b on b.employee_gid = a.processupdated_by " +
                     " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
                     " left join agr_trn_tsuprapplication2sanction d on d.application_gid = a.application_gid " +
                      " left join agr_trn_tsuprprocesstype_assign e on e.application_gid = a.application_gid " +
                     " where a.process_type = 'Accept' and " +
                     " a.application_gid in (Select application_gid from agr_trn_tsuprprocesstype_assign where menu_gid = '" + getMenuClass.Sanction + "' " +
        ...
        public void DaCADSanctionSummaryCount(string user_gid, string employee_gid, CadSanctionCount values)
        {
            ...
            msSQL = " select(select count(*) from agr_trn_tsuprprocesstype_assign a  " +
                   ...
                   "  (select count(*) from agr_trn_tsuprprocesstype_assign a " +
                   "  left join agr_mst_tsuprapplication b on a.application_gid = b.application_gid " +
                   " left join agr_trn_tsuprapplication2sanction c on a.application_gid = c.application_gid " +
        ...
        public void DaCADDocChecklistSummaryCount(string user_gid, string employee_gid, CadSanctionCount values)
        {
            ...
            msSQL = " select count(a.application_gid) as cadsanction_count from agr_mst_tsuprapplication a " +
                   " left join agr_trn_tsuprapplication2docchecklist d on d.application_gid = a.application_gid " +
            ...
            msSQL = " select count(a.application_gid) as cadchecker_count from agr_mst_tsuprapplication a " +
                     " left join agr_trn_tsuprapplication2docchecklist d on d.application_gid = a.application_gid " +
                     " where a.process_type = 'Accept' and d.maker_flag='Y' and d.checker_flag='N' and " +
                     " a.application_gid in (Select application_gid from agr_trn_tsuprprocesstype_assign where menu_gid = 'AGDSMPSDC' " +
                     " and checker_gid = '" + employee_gid + "')";
            ...
            msSQL = " select count(application_gid) as cadcheckerapproval_count from agr_mst_tsuprapplication a " +
                     " where a.process_type = 'Accept' and docchecklist_checkerflag='Y' and docchecklist_approvalflag='N' and " +
                     " a.application_gid in (Select application_gid from agr_trn_tsuprprocesstype_assign where menu_gid = 'AGDSMPSDC' " +
                     " and approver_gid = '" + employee_gid + "')";
            values.cadcheckerapproval_count = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " select count(application_gid) as approvalcompleted_count from agr_mst_tsuprapplication a " +
        ...
        public void DaGetAssignmentView(string application_gid, MdlMstAssignmentview values)
        {
            msSQL = " select a.processtypeassign_gid, a.application_gid, a.processtype_name, a.cadgroup_name,a.cadgroup_gid, a.menu_name, a.menu_gid," +
                    ...
                    " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as created_by from agr_trn_tsuprprocesstype_assign a " +
        ...
        public void DaGetSentbackToCCUnderwritingView(string application_gid, MdlMstAssignmentview values)
        {
            msSQL = " select a.application_gid, a.processtype_remarks, a.process_type, " +
                    ...
                    " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as processupdated_by from agr_mst_tsuprapplication a " +
                    " left join hrm_mst_temployee b on b.employee_gid = a.processupdated_by " +
                    " left join adm_mst_tuser c on b.user_gid = c.user_gid " +
                    " where a.application_gid = '" + application_gid + "' ";
            objODBCDatareader = objdbconn.GetDataReader(msSQL);
        ...
        public void DaGetMenu(string application_gid, MdlMstCADGetMenu objmenu)
        {
            try
            {
                msSQL = " select module_code as menu_gid,module_name as menu_name from adm_mst_tmodule where module_gid in ('AGDSMPSDC','AGDMGTDTS','AGDMGTPYD', 'AGDMGTCMS')" +
                        " and module_gid not in (select menu_gid from agr_trn_tsuprprocesstype_assign where application_gid = '" + application_gid + "')";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaPostRevertCADtoCC(string employee_gid, MdlCADRevert values)
        {
            ...
            msSQL = " insert into agr_trn_tsuprcadtoccmeetinglog(" +
                    " cadtocclog_gid," +
                    " application_gid," +
                    " reason," +
                    " created_by," +
            ...
            msSQL = " select ccmeeting2members_gid, application_gid, ccschedulemeeting_gid, ccmember_name, " +
                    " ccgroup_name, ccmember_gid, created_by, created_date, updated_by, updated_date, " +
                    " attendance_status, approval_remarks, approval_status from agr_mst_tsuprccmeeting2members " +
                    " where application_gid ='" + values.application_gid + "'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dt in dt_datatable.Rows)
                {
                    ...
                    msSQL = " insert into agr_mst_tsuprccmeeting2memberslog(" +
            ...
            msSQL = " insert into agr_mst_tsuprccschedulemeetinglog (select ccschedulemeeting_gid, " +
                ...
                " from agr_mst_tsuprccschedulemeeting where " +
                " application_gid='" + values.application_gid + "')";
            mnResult1 = objdbconn.ExecuteNonQuerySQL(msSQL);

            //msSQL = " insert into agr_mst_tsuprccmeeting2othermemberslog (select application_gid from agr_mst_tsuprccmeeting2othermembers where " +
            //   " application_gid='" + values.application_gid + "')";

            msSQL = "insert into agr_mst_tsuprccmeeting2othermemberslog ( application_gid,ccschedulemeeting_gid, employee_name, " +
                " employee_gid, created_by, created_date, updated_by,updated_date, attendance_status, approval_remarks, approval_status) " +
                " select  application_gid, ccschedulemeeting_gid, employee_name, employee_gid, created_by, created_date, updated_by, " +
                " updated_date, attendance_status, approval_remarks, approval_status from agr_mst_tsuprccmeeting2othermembers " +
                " where application_gid ='" + values.application_gid + "'";
            ...
            {
                msSQL = " delete from agr_mst_tsuprccmeeting2members " +
                    " where application_gid='" + values.application_gid + "' ";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                msSQL = " delete from agr_mst_tsuprccschedulemeeting " +
                    " where application_gid='" + values.application_gid + "' ";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                msSQL = " delete from agr_mst_tsuprccmeeting2othermembers " +
                    " where application_gid='" + values.application_gid + "' ";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                msSQL = " UPDATE agr_mst_tsuprapplication SET approval_status = 'Sent Back to CC', " +
        ...
        public void DaPostRevertCADtoCredit(string employee_gid, MdlCADRevert values)
        {
            ...
            msSQL = " insert into agr_trn_tsuprcadtocreditlog(" +
                    " cadtocreditlog_gid," +
                    " application_gid," +
                    " reason," +
                    " created_by," +
            ...
            msSQL = " select application_gid, appcreditapproval_gid, approval_gid, approval_name, approval_type, " +
                    ...
                    " hierary_level, created_by, created_date, initiate_flag from agr_trn_tsuprAppcreditapproval " +
                    " where application_gid ='" + values.application_gid + "'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dt in dt_datatable.Rows)
                {
                    ...
                    msSQL = " insert into agr_trn_tsuprAppcreditapprovallog(" +
            ...
            if (mnResult != 0 && mnResult1 != 0 && mnResult2 != 0)
            {
                msSQL = " update agr_trn_tsuprAppcreditapproval set " +
                ...
                msSQL = " delete from agr_trn_tsuprAppcreditapproval " +
                        " where application_gid='" + values.application_gid + "' and hierary_level<>'0' ";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                ...
                msSQL = " UPDATE agr_mst_tsuprapplication SET approval_status = 'Sent Back to Credit', " +
                        " ccsubmit_flag = 'N', meeting_status = 'Pending', " +
                        " ccgroup_name = null, mom_description = null, " +
                        " momapproval_flag = 'N', mom_flag = 'N', " +
                        " momupdated_by = null, momupdated_date = null, " +
                        " momdocumentupload_flag = 'N', cc_remarks = null, cccompleted_flag ='N', " +
                        " cccompleted_date = null, cccompleted_by = null, creditheadapproval_status = 'Pending', " +
                ...
                try
                {
                    msSQL = "select relationshipmanager_name,relationshipmanager_gid,creditassigned_date,application_no,clustermanager_gid,creditregionalmanager_gid,creditnationalmanager_gid,customerref_name from agr_mst_tsuprapplication where application_gid = '" + values.application_gid + "'";
                    objODBCDatareader1 = objdbconn.GetDataReader(msSQL);
                    if (objODBCDatareader1.HasRows == true)
                    {
                    ...
                    msSQL = "select concat(c.user_firstname,' ',c.user_lastname,' / ',c.user_code) as allocated_by from agr_mst_tsuprapplication a left join hrm_mst_temployee b" +
                    ...
                    msSQL = "select approval_gid from agr_trn_tsuprAppcreditapproval where hierary_level='0' and application_gid = '" + values.application_gid + "'";
        ...
        public bool DaGetCovenantDocumentTypeList(MdlMstCADCompany values, string credit_gid)
        {
            if (credit_gid != "")
            {
                msSQL = " SELECT companydocument_gid,companydocument_name,documenttypes_gid,documenttype_name,covenant_type " +
                        ...
                        " from agr_trn_tsuprcovanantdocumentcheckdtls where credit_gid = '" + credit_gid + "' and untagged_type is null group by companydocument_gid)";
            }
            else
            {
                msSQL = " SELECT companydocument_gid,companydocument_name,documenttypes_gid,documenttype_name,covenant_type " +
        ...
        public void DaPostCovenantPeriods(MdlCovenantPeriodlist values, result objResult, string user_gid, string employee_gid)
        {
            ...
            if (caddocchecklist.Count > 0)
            {
                ...
                {
                    msSQL = " update agr_trn_tsuprgroupcovenantdocumentchecklist set covenant_periods='" + i.covenantperiod + "', " +
                    ...
                    msSQL = " update agr_trn_tsuprcovanantdocumentcheckdtls set covenant_periods='" + i.covenantperiod + "', " +
                            " covenantperiod_updatedby='" + user_gid + "', " +
                            " untagged_type=null," +
                            " covenantperiod_updateddate=current_timestamp" +
                            " where groupcovdocumentchecklist_gid='" + i.groupcovdocumentchecklist_gid + "'";
            ...
            if (untaggedcaddocchecklist.Count > 0)
            {
                ...
                //msSQL = "DELETE FROM agr_trn_tsuprcovanantdocumentcheckdtls WHERE covenantdocumentcheckdtl_gid in (" + getdocumentid + ")";
                //mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                msSQL = " UPDATE agr_trn_tsuprgroupcovenantdocumentchecklist SET covenant_periods = '', " +
                        " untagged_type = 'Y', " +
                        " covenantperiod_updatedby = '" + user_gid + "', " +
                        " covenantperiod_updateddate = current_timestamp() " +
                ...
                msSQL = " UPDATE agr_trn_tsuprcovanantdocumentcheckdtls SET covenant_periods = '', " +
                      " untagged_type = 'Y', " +
                      " untagged_by='" + user_gid + "', " +
                      " untagged_date=current_timestamp," +
                      " covenantperiod_updatedby = '" + user_gid + "', " +
        ...
        {
            msSQL = " UPDATE agr_mst_tsuprapplication SET pslcompleted_flag = 'Y', " +
        ...
        {
            msSQL = " select application_gid from agr_mst_tsuprapplication" +
                " where application_gid='" + application_gid + "'" +
        ...
        {
            msSQL = " update agr_trn_tsuprapplication2sanction set sanctionletter_status='Saved'," +
                    " template_content='" + values.template_content.Replace("'", "''") + "', makersubmitted_by='" + employee_gid + "'," +
                    " makersubmitted_on='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where application2sanction_gid='" + values.sanction_gid + "' ";
            mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
        ...
        public void DaGetCADGroupDtl(string employee_gid, string application_gid, string menu_gid, MdlMstCAD values)
        {
            msSQL = " select application_gid,cadgroup_gid,cadgroup_name,menu_gid,menu_name, maker_gid,maker_name, " +
                    " checker_gid,checker_name,approver_gid,approver_name from agr_trn_tsuprprocesstype_assign" +
        ...
        public void DaPostReassignCADApplication(string employee_gid, MdlReassignCadApplication values)
        {
            ...
            if (objODBCDatareader.HasRows == true)
            {
                ...
                msSQL = " insert into agr_trn_tsuprprocesstype_assignlog(" +
            ...
            if (mnResult != 0)
            {
                ...
                msSQL = " update agr_trn_tsuprprocesstype_assign set " +
                         " application_gid='" + values.application_gid + "'," +
        ...
        public void DaGetReassignApplicationView(string application_gid, string menu_gid, MdlMstAssignmentview values)
        {
            msSQL = " select a.processtypeassignlog_gid, a.application_gid, " +
                    ...
                    " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as created_by from agr_trn_tsuprprocesstype_assignlog a " +
                    " left join hrm_mst_temployee b on b.employee_gid = a.created_by " +
                    " left join adm_mst_tuser c on b.user_gid = c.user_gid " +
                    " where application_gid = '" + application_gid + "' and menu_gid='" + menu_gid + "'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        {
            msSQL = "delete from agr_trn_tsuprprocesstype_assign where processtypeassign_gid='" + processtypeassign_gid + "'";
            mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
        ...
        public bool DaGetTempDelete(string employee_gid, string application_gid, result value)
        {
            ...
            msSQL = " update agr_mst_tsuprapplication2loan set limit_product='N' where application_gid='" + application_gid + "'";
            mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
        ...
        public void DaGetCADtoCCMeetingLog(string application_gid, string employee_gid, MdlCADtoCCMeetingLog values)
        {
            msSQL = " select application_gid,cadtocclog_gid,reason as cadtoccmeeting_reason," +
                    ...
                    " from agr_trn_tsuprcadtoccmeetinglog a" +
                    " left join hrm_mst_temployee b on b.employee_gid=a.created_by " +
                    " left join adm_mst_tuser c on c.user_gid=b.user_gid " +
        ...
        public void DaGetCADtoCreditLog(string application_gid, string employee_gid, MdlCADtoCreditLog values)
        {
            msSQL = " select application_gid,cadtocreditlog_gid,reason as cadtocredit_reason," +
                    ...
                    " from agr_trn_tsuprcadtocreditlog a" +
                    " left join hrm_mst_temployee b on b.employee_gid=a.created_by " +
                    " left join adm_mst_tuser c on c.user_gid=b.user_gid " +
                    " where a.application_gid = '" + application_gid + "' order by a.created_date desc ";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetDocChecklistApprovalDtls(string application_gid, MdlSanctionApprovalDetails values)
        {
            try
            {
                msSQL = " select application_gid,maker_name,checker_name,approver_name, " +
                        ...
                        " from agr_trn_tsuprprocesstype_assign a where processtype_name = 'Accept' and menu_gid = 'AGDSMPSDC' and application_gid='" + application_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
                    values.application_gid = objODBCDatareader["application_gid"].ToString();
        ...
        public void DaGetDocChecklistApprovalCompletedSummary(string employee_gid, MdlMstCAD values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customerref_name,a.customer_urn,a.creditgroup_name,h.sanction_refno, " +
                     ...
                     " left join adm_mst_tuser f on f.user_gid = e.user_gid " +
                     " left join agr_trn_tsuprprocesstype_assign g on g.application_gid = a.application_gid " +
                     " left join agr_trn_tsuprapplication2sanction h on h.application_gid = a.application_gid " +
                     " where a.process_type = 'Accept' and docchecklist_approvalflag='Y' and " +
                     " a.application_gid in (Select application_gid from agr_trn_tsuprprocesstype_assign where menu_gid = 'AGDSMPSDC' and approver_approvalflag='Y' " +
                     " and (maker_gid='" + employee_gid + "' or checker_gid='" + employee_gid + "' or approver_gid='" + employee_gid + "'))" +
                     " group by a.application_gid order by a.updated_date desc ";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            var getapplicationadd_list = new List<cadapplicationlist>();
