...
namespace ems.mastersamagro.Controllers
{
    ...
    /// <summary>
    /// This Controllers will provide access to business approval process in aplication creation flow.
    /// </summary>
    /// <remarks>Written by Sherin Augusta, Logapriya, Abilash.A, Premchander.K </remarks>

    [RoutePrefix("api/AgrTrnApplicationApproval")]
    [Authorize]
    public class AgrTrnApplicationApprovalController : ApiController
    {
        DaAgrTrnApplicationApproval objDaAgrTrnApplicationApproval = new DaAgrTrnApplicationApproval();
        ...
        [ActionName("Getproceedapprovalflag")]
        [HttpGet]
        public HttpResponseMessage Getproceedapprovalflag(string application_gid)
        {
           
            Mdlproceedapproval values = new Mdlproceedapproval();
            objDaAgrTrnApplicationApproval.DaGetproceedapprovalflag(values, application_gid);
            return Request.CreateResponse(HttpStatusCode.OK, values);
        }
    }
}
