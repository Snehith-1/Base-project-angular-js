...
namespace ems.system.DataAccess
{
    public class DaSysMstHRDocument
    {
...

        {
            try
            {
                msSQL = " SELECT a.hrdocument_gid,a.hrdocument_name,lms_code, a.bureau_code, date_format(a.created_date,'%d-%m-%Y %h:%i %p') as created_date, " +
        ...
        public void DaEditSysHRDocument(string hrdocument_gid, hrdocument values)
        {
            try
            {
                msSQL = " SELECT hrdocument_gid,hrdocument_name,lms_code, bureau_code, status as status FROM sys_mst_thrdocument " +
            ...
            if (objODBCDatareader.HasRows == true)
            {
                ...
                if (!(String.IsNullOrEmpty(lsUpdatedBy)) && !(String.IsNullOrEmpty(lsUpdatedDate)))
                {
                    ...
                    msSQL = " insert into sys_mst_thrdocumentlog(" +
                              ...
                              "'" + objODBCDatareader["hrdocument_name"].ToString().Replace("'", @"\'") + "'," +
                              "'" + employee_gid + "'," +
                              "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')";
        ...
        public void DaUploadDocumenttoDigio(MdlFileDetailsEsign values)
        {
            try
            {
                ...
                if (format == "pdf")
                {
                    ...
                    if (objODBCDatareader.HasRows)
                    {
                        ObjMdlUploadDocumenttoDigioRequest.expire_in_days = objODBCDatareader["expire_in_days"].ToString();
                        ObjMdlUploadDocumenttoDigioRequest.comment = objODBCDatareader["comment"].ToString();
                        ObjMdlUploadDocumenttoDigioRequest.display_on_page = objODBCDatareader["display_on_page"].ToString();
                        ObjMdlUploadDocumenttoDigioRequest.send_sign_link = objODBCDatareader["send_sign_link"].ToString();
                        ObjMdlUploadDocumenttoDigioRequest.notify_signers = objODBCDatareader["notify_signers"].ToString();
                    }
                    objODBCDatareader.Close();
                    ...
                    if (objODBCDatareader.HasRows)
                    {
                        lsname = objODBCDatareader["name"].ToString();
        ...
        public void DaGetDocumentDetails(MdlFileDetailsEsign values, string user_gid)
        {
            try
            {
                ...
                if (objODBCDatareader.HasRows)
                {
                    ...
                    lscreated_at = objODBCDatareader["created_at"].ToString();
                    objODBCDatareader.Close();
                }
                else
                {
        ...
        public void DaUpdateExpiryDate(hrdoc_list values, string employee_gid)
        {
            ...
            foreach (var lshrdoc_id in HRDocumentSummary_List)
            {
                ...
                if (objODBCDatareader.HasRows)
                {
                    lsexpire_on = objODBCDatareader["expire_on"].ToString();
                    lscreated_at = objODBCDatareader["created_at"].ToString();
                }
                objODBCDatareader.Close();
        ...
        public void DaGetESignReportSummaryCount(hrdoc_list values)
        {
            ...
            if (objODBCDatareader.HasRows == true)
            {
                values.pendingesign_count = objODBCDatareader["unsigneddocument_count"].ToString();
            ...
            if (objODBCDatareader.HasRows == true)
            {
                values.completedesign_count = objODBCDatareader["signeddocument_count"].ToString();
            ...
            if (objODBCDatareader.HasRows == true)
            {
                values.expiredesign_count = objODBCDatareader["expireddocument_count"].ToString();
