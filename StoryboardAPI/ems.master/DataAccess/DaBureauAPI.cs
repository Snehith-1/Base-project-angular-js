...
using ems.master.TransUnionService;
using ems.master.TransUnionCommService;
    ...
    public class DaBureauAPI
    {
        ...
        public MdlHighmarkResponse DaGetHighmarkCreditInfo(string employee_gid, string contact_gid)
        {
            ...
            try
            {
                ...
                msSQL = " select first_name,middle_name,last_name,pan_no,individual_dob from ocs_mst_tcontact where " +
                ...
                msSQL = " select addressline1,addressline2,city,state,postal_code from ocs_mst_tcontact2address where " +
                ...
                msSQL = " select mobile_no from ocs_mst_tcontact2mobileno where " +
                          " contact_gid='" + contact_gid + "' and primary_status='Yes'";
            ...
            thread.Join();
                ...
                msSQL = " insert into ocs_mst_tindividual2cicdocumentupload( " +
        ...
        public void DaGetHighmarkHTMLContent(string tmpcicdocument_gid, MdlHighmarkResponse values)
        {
            try
            {
                msSQL = "select document_content from ocs_mst_tindividual2cicdocumentupload" +
        ...
        public MdlTransUnionResponse DaGetTransUnionConsumerCreditInfo(string employee_gid, string contact_gid)
        {
            ...
            try
            {
                ...
                bureau_type = "Consumer";
                BureauLogResponse bureauLogResponse = new BureauLogResponse();
                LogForAuditTransUnion("Logging started for TransUnion Consumer with Contact GID - " + contact_gid + "at " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), bureau_type, contact_gid);

                msSQL = " select first_name,middle_name,last_name,pan_no,individual_dob,gender_name from ocs_mst_tcontact where " +
                       " contact_gid='" + contact_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
                ...
                msSQL = " select addressline1,addressline2,city,postal_code,state from ocs_mst_tcontact2address where " +
                       " contact_gid='" + contact_gid + "' and primary_status = 'Yes'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                ...
                if (TransUnionConsumerValidator(transUnionConsumer) == false)
                {
                    ...
                    LogForAuditHighmark("Input fields not validated - " + validation_message, bureau_type, contact_gid);
                    return ObjMdlTransUnionResponse;
                }

                LogForAuditTransUnion("Input Fields validated..!", bureau_type, contact_gid);
                ...
                LogForAuditTransUnion("XML Request Contents obtained - " + xmlreqContents + " - End of XML Request Contents", bureau_type, contact_gid);
                ...
                LogForAuditTransUnion("Response obtained for the Bureau Request..!", bureau_type, contact_gid);
                LogForAuditTransUnion(xmlResponse, bureau_type, contact_gid);
                LogForAuditTransUnion("End of Response..!", bureau_type, contact_gid);
                ...
                if (m.Success)
                {
                    ...
                    LogForAuditTransUnion("Bureau Score obtained..!", bureau_type, contact_gid);
                    if(ObjMdlTransUnionResponse.bureau_score == "0-1")
                    {
                        ...
                        bureauLogResponse.failure_reason = ObjMdlTransUnionResponse.message;
                        LogTransUnionConsumerResponse(contact_gid, employee_gid, bureauLogResponse, failedStatus);
                        LogForAuditTransUnion("Consumer not in CIBIL Database or history older than 36 months!", bureau_type, contact_gid);
                    }
                }
                else
                {
                    ...
                    bureauLogResponse.failure_reason = ObjMdlTransUnionResponse.message;
                    LogTransUnionConsumerResponse(contact_gid, employee_gid, bureauLogResponse, failedStatus);
                    LogForAuditTransUnion("Bureau Score not found in the Response..!", bureau_type, contact_gid);
                }
                ...
                if (mappid.Success == false || mdocid.Success == false)
                {
                    ...
                    bureauLogResponse.failure_reason = ObjMdlTransUnionResponse.message;
                    LogTransUnionConsumerResponse(contact_gid, employee_gid, bureauLogResponse, failedStatus);
                    LogForAuditTransUnion("Application ID/Document ID not found in the Response..!", bureau_type, contact_gid);
                    return ObjMdlTransUnionResponse;
                }
                else
                {
                    LogForAuditTransUnion("Application ID and Document ID were found in the Response..!", bureau_type, contact_gid);
                }
                ...
                {
                    LogForAuditTransUnion("Report File Content obtained in Response..!", bureau_type, contact_gid);
                } else
                {
                    ObjMdlTransUnionResponse.status = false;
                    ...
                    bureauLogResponse.failure_reason = ObjMdlTransUnionResponse.message;
                    LogTransUnionConsumerResponse(contact_gid, employee_gid, bureauLogResponse, failedStatus);
                    LogForAuditTransUnion("Report File Content was not obtained in the Response..!", bureau_type, contact_gid);
                    return ObjMdlTransUnionResponse;
                }
                ...
                string lsreportabspath = ConfigurationManager.AppSettings["file_path"] + "/erp_documents" + "/" + fetchCompanyCode() + "/" + "Master/TransUnionReport/" + DateTime.Now.Year + "/" + DateTime.Now.Month + "/";

                if ((!System.IO.Directory.Exists(lsreportabspath)))
                    System.IO.Directory.CreateDirectory(lsreportabspath);

                fileName = "TransUnionReport" + document_id + ".xml";
                ...
                string lsreportrelpath = "../../erp_documents" + "/" + fetchCompanyCode() + "/" + "Master/TransUnionReport/" + DateTime.Now.Year + "/" + DateTime.Now.Month + "/";
                ...
                bureauLogResponse.bureau_score = ObjMdlTransUnionResponse.bureau_score;
                bureauLogResponse.document_content = xmlresponseContents;
                LogTransUnionConsumerResponse(contact_gid, employee_gid, bureauLogResponse, successStatus);
                ...
                if(mnResult == 1)
                {
                    //TransUnionConsumerHighRiskAlert transUnionConsumerHighRiskAlert = new TransUnionConsumerHighRiskAlert();

                    //transUnionConsumerHighRiskAlert.noofphones_rep3months = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001DM201S&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+)&lt;/Value&gt;\r\n").Groups[1].Value;
                    //transUnionConsumerHighRiskAlert.noofaddresses_rep3months = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001DM211S&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+)&lt;/Value&gt;\r\n").Groups[1].Value;

                    //transUnionConsumerHighRiskAlert.noofdistphones_rep3months = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001DM207E&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+)&lt;/Value&gt;\r\n").Groups[1].Value;
                    //transUnionConsumerHighRiskAlert.noofdistddresses_rep3months = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001DM217E&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+)&lt;/Value&gt;\r\n").Groups[1].Value;
                    //transUnionConsumerHighRiskAlert.noofdistids_rep3months = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001DM247S&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+)&lt;/Value&gt;\r\n").Groups[1].Value;
                    //transUnionConsumerHighRiskAlert.noofdistpincodes_rep3months = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001DM257S&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+)&lt;/Value&gt;\r\n").Groups[1].Value;

                    //transUnionConsumerHighRiskAlert.enqdifflenders_30days = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001INQ_VEL_6&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+[.][0-9]{2})&lt;/Value&gt;\r\n").Groups[1].Value;
                    //transUnionConsumerHighRiskAlert.newloansopened_30days = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P001TRD_VEL_1&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+[.][0-9]{2})&lt;/Value&gt;\r\n").Groups[1].Value;

                    //transUnionConsumerHighRiskAlert.distunsecuredenq_3months = Regex.Match(xmlResponse, "&lt;Id&gt;SPD020P002A169&lt;/Id&gt;\r\n&lt;Value&gt;([0-9]+)&lt;/Value&gt;\r\n").Groups[1].Value;

                    //transUnionConsumerHighRiskAlert.ranksegment_hml = Regex.Match(xmlResponse, "&lt;HRARuleAttributes&gt;\r\n&lt;Attribute&gt;\r\n&lt;Value&gt;([A-Za-z]+)&lt;/Value&gt;\r\n&lt;Id&gt;RankSegment&lt;/Id&gt;").Groups[1].Value;

                    //if(TransUnionConsumerHRAValidator(transUnionConsumerHighRiskAlert) == false)
                    //{
                    //    ObjMdlTransUnionResponse.status = false;
                    //    ObjMdlTransUnionResponse.message = validation_message;
                    //    LogForAuditTransUnion("High Risk Alert Details not obtained in the Response..!", bureau_type, contact_gid);
                    //}
                    //else
                    //{
                    //    msGetGid = objcmnfunctions.GetMasterGID("C2HA");
                    //    msSQL = " insert into ocs_mst_tcontact2tuhighriskalert( " +
                    ...
                    //    ObjMdlTransUnionResponse.message = validation_message;
                    //    LogForAuditTransUnion("High Risk Alert Details obtained and inserted into DB successfully..!", bureau_type, contact_gid);
                    //}

                } else
                {
                    ObjMdlTransUnionResponse.status = false;
                    ObjMdlTransUnionResponse.message = "Error Occured in inserting TransUnion Consumer details into DB..!";
                }
            ...
            catch (Exception ex)
            {
                ...
                LogForAuditTransUnion("Exception occured - ExString - " + ex.ToString() + " - End of Exception String", bureau_type, contact_gid);
                LogForAuditTransUnion("Exception occured - ExStackTrace - " + ex.StackTrace + " - End of Exception StackTrace", bureau_type, contact_gid);

            }
            LogForAuditTransUnion("Logging ended for TransUnion Consumer..!", bureau_type, contact_gid);
            LogForAuditTransUnion("", bureau_type, contact_gid);
            return ObjMdlTransUnionResponse;
        }
        ...
        public MdlHighmarkResponse DaGetHighmarkInstitutionCreditInfo(string employee_gid, string institution_gid)
        {
            ...
            try
            {
                ...
                msSQL = " select mobile_no from ocs_mst_tinstitution2mobileno where " +
                         " institution_gid='" + institution_gid + "' and primary_status='Yes'";
                highmarkCommercial.mobile_no = objdbconn.GetExecuteScalar(msSQL);

                msSQL = " select addressline1,addressline2,city,state,postal_code from ocs_mst_tinstitution2address where " +
                           " institution_gid='" + institution_gid + "' and primary_status='Yes'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
        ...
        public void DaGetHighmarkInstitutionHTMLContent(string tmpcicdocument_gid, MdlHighmarkResponse values)
        {
            try
            {
                msSQL = "select document_content from ocs_mst_tinstitution2cicdocumentupload" +
        ...
        public void DaGetHighRiskAlertDetails(string contact2bureau_gid, string employee_gid, MdlHighRiskAlertDetails values)
        {
            try
            {
                msSQL = " select noofph_rep3mon,noofad_rep3mon,noofdistph_rep3mon,noofdistad_rep3mon,noofdistid_rep3mon,noofdistpin_3mon,enqdifflend_30days,newloanopened_30days," +
                        ...
                        " from ocs_mst_tcontact2tuhighriskalert a" +
        ...
        public MdlTransUnionResponse DaGetTransUnionCommercialCreditInfo(string employee_gid, string institution_gid)
        {
            ...
            XmlDocument xmlDoc = new XmlDocument();
            LogForAuditTransUnion("Logging started for TransUnion Commercial with GID - " + institution_gid + "at " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), bureau_type, institution_gid);
            try
            {
                msSQL = " select company_name,companypan_no from ocs_mst_tinstitution where " +
                       " institution_gid='" + institution_gid + "'";
                ...
                msSQL = " select addressline1,addressline2,city,postal_code,state,taluka from ocs_mst_tinstitution2address where " +
                       " institution_gid='" + institution_gid + "' and primary_status = 'Yes'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                ...
                if (objODBCDatareader.HasRows == true)
                {
                    ...
                    transUnionCommercial.city = objODBCDatareader["taluka"].ToString();
                ...
                if (TransUnionCommercialValidator(transUnionCommercial) == false)
                {
                    ObjMdlTransUnionResponse.status = false;
                    ObjMdlTransUnionResponse.message = validation_message;
                    LogForAuditTransUnion("Input fields not validated - " + validation_message, bureau_type, institution_gid);
                    return ObjMdlTransUnionResponse;
                }
                LogForAuditTransUnion("Input fields validated", bureau_type, institution_gid);
                templatePath = ConfigurationManager.AppSettings["file_path"] + "/templates/TemplateRequestTransUnionCommercial.xml";
                xmlDoc.Load(templatePath);
                string xmlreqContents = xmlDoc.InnerXml;
                LogForAuditTransUnion("Template loaded successfully", bureau_type, institution_gid);
                ...
                LogForAuditTransUnion("Bureau Request: " + xmlreqContents, bureau_type, institution_gid);
                LogForAuditTransUnion("Response obtained for the Bureau Request..!", bureau_type, institution_gid);
                LogForAuditTransUnion(xmlResponse, bureau_type, institution_gid);
                LogForAuditTransUnion("End of Response..!", bureau_type, institution_gid);
                ...
                LogForAuditTransUnion("Document metadata Template loaded successfully", bureau_type, institution_gid);
                ...
                LogForAuditTransUnion("Document metadata Request: " + xmlDocMetaDataContents, bureau_type, institution_gid);
                LogForAuditTransUnion("Response obtained for the document meta data Request..!", bureau_type, institution_gid);
                LogForAuditTransUnion(xmlMetaDataResponse, bureau_type, institution_gid);
                LogForAuditTransUnion("End of Response..!", bureau_type, institution_gid);
                ...
                LogForAuditTransUnion("Document download Template loaded successfully", bureau_type, institution_gid);
                ...
                LogForAuditTransUnion("Download Document Request: " + xmlDownloadDocContents, bureau_type, institution_gid);
                ...
                if (mnResult == 1)
                {
                    ...
                    LogForAuditTransUnion(ObjMdlTransUnionResponse.message, bureau_type, institution_gid);
                    LogForAuditTransUnion("Logging ended", bureau_type, institution_gid);
                    return ObjMdlTransUnionResponse;
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                LogForAuditTransUnion("Exception occured while generating TransUnion details: " + ex.ToString(), bureau_type, institution_gid);
        ...
        public string removeXmlContent(string xmlContent)
        {
            string htmlContent = xmlContent.Remove(0, 264);
            htmlContent = htmlContent.Replace("font-family:font-family:", "font-family:");
            return htmlContent;
        }
