...
/// <summary>
/// (It's used for Admin ) Admin DataAccess Class accessed by API methods from related Controller class and is returning relevant response to client.
/// </summary>
/// <remarks>Written by Logapriya</remarks>

namespace ems.master.DataAccess
{
    public class DaMstAdminApplication
    {
        ...
        int mnResult, mnResultapprovallog, mnResultapprovalreasonlog,mnResulthierarchyupdateapprovallog;
        string msSQL, msGetGid;
        string sToken = string.Empty;
        Random rand = new Random();
        string msGetGidapprovallog, msGetGidcreditapprovallog, msGetGidcreditmanagerapprovallog, msGetGidcreditmanagerapprovalreasonlog, lsapproved_date;
        ...
        public void DaGetBusinessRejectedApplSummary(string employee_gid, MdlRejectedAppl values)
        {
            msSQL = " select a.application_gid,application_no,customer_name,customer_urn,vertical_name,applicant_type, region, " +
                    ...
                    " left join ocs_trn_tapplicationapprovallog d on d.application_gid = a.application_gid " +
                    " left join ocs_trn_tbusinessrejectrevokelog e on e.application_gid = a.application_gid " +
        ...
        {
            msSQL = "select approval_status from ocs_mst_tapplication where application_gid='" + values.application_gid + "'";
            ...
            if (values.approval_status == "Rejected By Business")
            {
                msSQL = " select applicationapproval_gid,hierary_level,(hierary_level-1) as hierarylevel_count, " +
                        ...
                        " from ocs_trn_tapplicationapproval where application_gid='" + values.application_gid + "'" +
            ...
            msSQL = "select approved_date from ocs_trn_tapplicationapproval where application_gid='" + values.application_gid + "' and hierary_level='" + values.hierarylevel_count + "'";
            objODBCDatareader = objdbconn.GetDataReader(msSQL);
            if (objODBCDatareader.HasRows == true)
            {
            ...
            msSQL = "select applicationapproval_gid from ocs_trn_tapplicationapproval where application_gid='" + values.application_gid + "'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dt in dt_datatable.Rows)
                {
                    ...
                    msSQL = " insert into ocs_trn_tapplicationapprovallog " +
                            ...
                            " from ocs_trn_tapplicationapproval " +
            ...
            if (mnResultapprovallog != 0)
            {
                if (values.approval_status == "Rejected By Business")
                {
                    ...
                    msSQL = " insert into ocs_trn_tbusinessrejectrevokelog ( " +
                ...
                if (mnResult != 0)
                {
                    ...
                    {
                        msSQL = " update ocs_mst_tapplication set approval_status='Submitted to Approval', headapproval_status='Pending',headapproval_date=null" +
                                " where application_gid='" + values.application_gid + "'";
                        mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                    }
                    else
                    {
                        msSQL = " update ocs_mst_tapplication set approval_status='Submitted to Approval', headapproval_status='L" + values.hierarylevel_count + " - Approved',headapproval_date='" + Convert.ToDateTime(lsapproved_date).ToString("yyyy-MM-dd HH:mm:ss") + "'" +
                                " where application_gid='" + values.application_gid + "'";
                        mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                    }

                    msSQL = " update ocs_trn_tapplicationapproval set approval_status='Pending',approval_remarks=null," +
                            " rejected_date=null,hold_date=null where applicationapproval_gid='" + values.applicationapproval_gid + "'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                    msSQL = " update ocs_trn_tapplicationapproval set approval_status='Pending' " +
        ...
        public void DaGetBusinessRevokedApplSummary(string employee_gid, MdlRevokedAppl values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customer_name,a.customer_urn,a.vertical_name,a.applicant_type, a.region, " +
                     ...
                     " date_format(a.submitted_date,'%d-%m-%Y %h:%i %p') as submitted_date, " +
                     " (select f.approval_status from ocs_trn_tbusinessrejectrevokelog f where " +
                     " f.created_date = (select max(k.created_date) from ocs_trn_tbusinessrejectrevokelog k where k.application_gid = a.application_gid)) " +
                     " as approval_status" +
                     " from ocs_mst_tapplication a " +
                     " left join hrm_mst_temployee b on b.employee_gid=a.created_by  " +
                     " left join ocs_trn_tapplicationapprovallog d on d.application_gid=a.application_gid " +
        ...
        public void DaGetBusinessPendingHistoryLog(string application_gid, string employee_gid, MdlRevokedAppl values)
        {
            msSQL = " select a.application_gid, a.approval_remarks,approval_status,applicationapproval_gid, " +
                     ...
                     " from ocs_trn_tapplicationapproval a " +
        ...
        public void DaGetBusinessHistoryLog(string application_gid, string employee_gid, MdlRevokedAppl values)
        {
            msSQL = " select a.application_gid,a.reason as revoked_remarks,businessrejectrevokelog_gid,  " +
                    ...
                    " from ocs_trn_tbusinessrejectrevokelog a  " +
                    " left join hrm_mst_temployee b on b.employee_gid=a.created_by   " +
                    " left join adm_mst_tuser c on c.user_gid=b.user_gid " +
        ...
        {
            msSQL = " select count(application_gid) as businessreject_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(application_gid) as businesshold_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(distinct a.application_gid) as businessrevoked_count from ocs_mst_tapplication a " +
                    " left join ocs_trn_tapplicationapprovallog b on b.application_gid=a.application_gid  " +
        ...
        public void DaGetCreditRejectedApplSummary(string employee_gid, MdlCreditRejectedAppl values)
        {
            msSQL = " select a.application_gid,application_no,customer_name,customer_urn,vertical_name,applicant_type, region, " +
                    ...
                    " left join hrm_mst_temployee b on b.employee_gid=a.created_by " +
                    " left join adm_mst_tuser c on c.user_gid=b.user_gid " +
                    " left join ocs_trn_tappcreditrevokeapprovallog d on d.application_gid = a.application_gid " +
                    " left join ocs_trn_tcreditrevokelog e on e.application_gid = a.application_gid " +
        ...
        {
            msSQL = " select count(application_gid) creditreject_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(application_gid) as credithold_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(distinct a.application_gid) as creditrevoked_count from ocs_mst_tapplication a " +
                    " left join ocs_trn_tappcreditrevokeapprovallog b on b.application_gid=a.application_gid  " +
                    " left join ocs_trn_tcreditmanagerrevokelog f on f.application_gid = a.application_gid " +
                    " where b.application_gid=a.application_gid  or f.application_gid=a.application_gid ";
        ...
        public bool DaPostCreditRevokeApplication(Mdlcreditrevoke values, string user_gid, string employee_gid)
        {
            ...
            if (values.approval_status == "Rejected By Credit")
            {
                msSQL = " select appcreditapproval_gid,hierary_level,(hierary_level-1) as hierarylevel_count, " +
                        ...
                        " from ocs_trn_tappcreditapproval where application_gid='" + values.application_gid + "'" +
                        " and approval_status='Rejected'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
            ...
            msSQL = "select approved_date from ocs_trn_tappcreditapproval where application_gid='" + values.application_gid + "' and hierary_level='" + values.hierarylevel_count + "'";
            objODBCDatareader = objdbconn.GetDataReader(msSQL);
            if (objODBCDatareader.HasRows == true)
            {
                lsapproved_date = objODBCDatareader["approved_date"].ToString();
            ...
            msSQL = "select appcreditapproval_gid from ocs_trn_tappcreditapproval where application_gid='" + values.application_gid + "'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dt in dt_datatable.Rows)
                {
                    ...
                    msSQL = " insert into ocs_trn_tappcreditrevokeapprovallog " +
            ...
            if (mnResultapprovallog != 0)
            {
                if (values.approval_status == "Rejected By Credit")
                {
                    ...
                    msSQL = " insert into ocs_trn_tcreditrevokelog ( " +
                            " creditrevokelog_gid, " +
                            " application_gid, " +
                            " reason, " +
                            " created_by, " +
                ...
                if (mnResult != 0)
                {
                    if (values.hierarylevel_count == "0")
                    {
                        msSQL = " update ocs_mst_tapplication set approval_status='Submitted to Credit Approval', creditheadapproval_status='Pending',creditheadapproval_date=null" +
                                " where application_gid='" + values.application_gid + "'";
                        mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                    }
                    else
                    {
                        msSQL = " update ocs_mst_tapplication set approval_status='Submitted to Credit Approval', creditheadapproval_status='L" + values.hierarylevel_count + " - Approved', " +
                                " creditheadapproval_date='" + Convert.ToDateTime(lsapproved_date).ToString("yyyy-MM-dd HH:mm:ss") + "'" +
                                " where application_gid='" + values.application_gid + "'";
                        mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                    }

                    msSQL = " update ocs_trn_tappcreditapproval set approval_status='Pending',approval_remarks=null," +
                            " rejected_date=null,hold_date=null where appcreditapproval_gid='" + values.appcreditapproval_gid + "'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                    msSQL = " update ocs_trn_tappcreditapproval set approval_status='Pending' " +
        ...
        public void DaGetCreditRevokedApplSummary(string employee_gid, MdlCreditRevokedAppl values)
        {
            msSQL = " select a.application_gid,a.application_no,a.customer_name,a.customer_urn,a.vertical_name,a.applicant_type, a.region, " +
                     ...
                     " date_format(a.created_date,'%d-%m-%Y %h:%i %p') as created_date,a.creditgroup_gid,a.creditgroup_name,d.created_date, " +
                     " date_format(a.submitted_date,'%d-%m-%Y %h:%i %p') as submitted_date, " +
                     " (select f.approval_status from ocs_trn_tcreditrevokelog f where " +
                     " f.created_date = (select max(k.created_date) from ocs_trn_tcreditrevokelog k where k.application_gid = a.application_gid)) " +
                     " as approval_status" +
                     " from ocs_mst_tapplication a " +
                     " left join hrm_mst_temployee b on b.employee_gid=a.created_by  " +
                     " left join ocs_trn_tappcreditrevokeapprovallog d on d.application_gid=a.application_gid " +
                     ...
                     " left join adm_mst_tuser c on c.user_gid=b.user_gid  " +
                     " left join ocs_trn_tcreditmanagerrevokelog f on f.application_gid = a.application_gid " +
                     " where d.application_gid=a.application_gid or f.application_gid=a.application_gid " +
                     " group by(d.application_gid) order by d.created_date desc ";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetCreditPendingHistoryLog(string application_gid, string employee_gid, MdlCreditPendingHistoryAppl values)
        {
            msSQL = " select a.application_gid, a.approval_remarks,approval_status,appcreditapproval_gid, " +
                     " concat(c.user_firstname,' ',c.user_lastname,' / ',c.user_code) as rejecttedhold_by,hierary_level, " +
                     " case when a.rejected_date is not null then date_format(a.rejected_date,'%d-%m-%Y %h:%i %p') when date_format(a.hold_date,'%d-%m-%Y %h:%i %p') is not null then a.hold_date else null end as rejecttedhold_date " +
                     " from ocs_trn_tAppcreditapproval a " +
                     " left join hrm_mst_temployee b on b.employee_gid=a.approval_gid  " +
                     " left join adm_mst_tuser c on c.user_gid=b.user_gid  " +
                     " where a.application_gid='" + application_gid + "' and (a.approval_status = 'Rejected' or a.approval_status = 'Hold') group by a.application_gid";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetCreditHistoryLog(string application_gid, string employee_gid, MdlCreditHistoryLog values)
        {
            msSQL = " select a.application_gid,a.reason as revoked_remarks,creditrevokelog_gid,  " +
                    ...
                    " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as revoked_by, " +
                    " date_format(a.created_date, '%d-%m-%Y %h:%i %p') as revoked_date  " +
                    " from ocs_trn_tcreditrevokelog a  " +
                    " left join hrm_mst_temployee b on b.employee_gid=a.created_by   " +
                    " left join adm_mst_tuser c on c.user_gid=b.user_gid " +
                    " left join hrm_mst_temployee d on d.employee_gid=a.rejected_by  " +
                    " left join adm_mst_tuser e on e.user_gid=d.user_gid   " +
        ...
        public void DaGetBusinessStageSummary(string employee_gid, MdlBusinessHierarchyUpdate values)
        {
            msSQL = " select a.application_gid,application_no,customer_name,customer_urn,a.vertical_name,applicant_type, region, " +
                    ...
                    " left join hrm_mst_temployee b on b.employee_gid=a.created_by " +
                    " left join adm_mst_tuser c on c.user_gid=b.user_gid " +
                    " left join ocs_trn_tbusinesshierarchyupdatelog d on d.application_gid=a.application_gid " +
        ...
        {
            msSQL = " select count(application_gid) incompletestage_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(application_gid) businessstage_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(application_gid) as creditstage_count from ocs_mst_tapplication a " +
                    " where a.approval_status='Submitted to Underwriting' or a.approval_status='Submitted to Credit Approval'" +
            ...
            msSQL = " select count(distinct a.application_gid) as ccstage_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(distinct a.application_gid) as cadpendingstage_count from ocs_mst_tapplication a " +
            ...
            msSQL = " select count(distinct a.application_gid) as cadacceptedstage_count from ocs_trn_tcadapplication a " +
            ...
            int totalcount = incompletestage_count + businessstage_count + creditstage_count + ccstage_count + cadpendingstage_count + cadacceptedstage_count;
            values.lstotalcount = Convert.ToInt16(totalcount);
        ...
        public bool DaPostBusinessHierarchyUpdate(MdlBusinessHierarchyUpdateDtl values, string employee_gid)
        {
            ...
            if (values.application_stage == "IncompleteStage")
            {
                ...
                msSQL = " insert into ocs_trn_tbusinesshierarchyupdatelog ( " +
                ...
                if (mnResult != 0)
                {  
                    msSQL = " update ocs_mst_tapplication set vertical_gid='" + values.submitvertical_gid + "', vertical_name='" + values.submitvertical_name + "'," +
            ...
            else if (values.application_stage == "BusinessStage")
            {
                ...
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow dt in dt_datatable.Rows)
                    {
                        ...
                        msSQL = " insert into ocs_trn_tbusinessapprovalhierarchylog " +
                ...
                if (mnResult != 0)
                {
                    ...
                    msSQL = " update ocs_trn_tapplicationapproval set approval_status='Pending',approval_remarks=null,approved_date=null " +
                            " where application_gid='" + values.application_gid + "'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                    msSQL = " update ocs_trn_tapplicationapproval set approval_gid='" + values.updateddrm_gid + "', approval_name='" + values.updateddrm_name + "'," +
                            " initiate_flag='Y'" +
                            " where application_gid='" + values.application_gid + "' and hierary_level='1'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                    msSQL = " update ocs_trn_tapplicationapproval set approval_gid='" + values.updatedclusterhead_gid + "', approval_name='" + values.updatedclusterhead_name + "'," +
                            " initiate_flag='N'" +
                            " where application_gid='" + values.application_gid + "' and hierary_level='2'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                    msSQL = " update ocs_trn_tapplicationapproval set approval_gid='" + values.updatedregionhead_gid + "', approval_name='" + values.updatedregionhead_name + "'," +
                            " initiate_flag='N'" +
                            " where application_gid='" + values.application_gid + "' and hierary_level='3'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                    msSQL = " update ocs_trn_tapplicationapproval set approval_gid='" + values.updatedzohalhead_gid + "', approval_name='" + values.updatedzonalhead_name + "'," +
                            " initiate_flag='N'" +
                            " where application_gid='" + values.application_gid + "' and hierary_level='4'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                    msSQL = " update ocs_trn_tapplicationapproval set approval_gid='" + values.updatedbusinesshead_gid + "', approval_name='" + values.updatedbusinesshead_name + "'," +
                            " initiate_flag='N'" +
            ...
            else if (values.application_stage == "CadAcceptedStage")
            {
                ...
                if (mnResult != 0)
                {
                    ...
                    msSQL = " update ocs_trn_tcadapplication set vertical_gid='" + values.submitvertical_gid + "', vertical_name='" + values.submitvertical_name + "'," +
                            " program_gid='" + values.submitprogram_gid + "', program_name='" + values.submitprogram_name + "'," +
                            " relationshipmanager_gid='" + values.updatedrelationshipmanager_gid + "', relationshipmanager_name='" + values.updatedrelationshipmanager_name + "'," +
                            " created_by= '" + values.updatedrelationshipmanager_gid + "', drm_gid ='" + values.updateddrm_gid + "'," +
                            " drm_name= '" + values.updateddrm_name + "', clustermanager_gid ='" + values.updatedclusterhead_gid + "'," +
                    ...
                    //Bank alert unallocated RM inactive user to Allocated table
                    ...
                    if (lscustomer_urn != "" || lscustomer_urn != null)
                    {
                        msSQL = "select a.application_gid from ocs_trn_tcadapplication a " +
                                "where a.customer_urn = '" + lscustomer_urn + "' order by a.created_date desc limit 1";

                        string lsapplication_gid = objdbconn.GetExecuteScalar(msSQL);

                        if (lsapplication_gid == values.application_gid)
                        {
                            msSQL = "select a.customer_name from ocs_trn_tcadapplication a " +
                               "where a.customer_urn = '" + lscustomer_urn + "' and a.application_gid= '" + values.application_gid + "'";
                            ...
                            msSQL = " select bankalert2notallocated_gid,ecollectionresponsefromsambtrn_gid,ecollectionresponsefromsambtrndtls_gid,kotakAPI_flag,ticketref_no,email_to,email_from," +
                                ...
                                " from osd_trn_tbankalert2notallocated where ( reason = 'RM Inactive' or reason = 'RM Empty' ) and " +
                                " customer_urn='" + lscustomer_urn + "' and kotakAPI_flag='Y'";
                                ...
                                foreach (DataRow dr_datarow in dt_datatable.Rows)
                                {
                                    ...
                                    msSQL = "insert into osd_trn_tbankalert2allocated (" +
                                             ...
                                             " businessheadlevel_name, businessheadlevel_gid," +
                                               ...
                                               " 'Re-Allocated from RM Inactive or Empty'," +
                                               " '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'," +
                                                 "'" + values.updatedrelationshipmanager_name + "'," +
                                                 "'" + values.updatedrelationshipmanager_gid + "'," +
                                                 "'" + values.updateddrm_gid + "'," +
        ...
        public void DaGetHierarchyUpdateHistoryLog(string application_gid, MdlHierarchyUpdateHistoryLog values)
        {
            msSQL = " select a.businesshierarchyupdatelog_gid,a.vertical_gid ,a.vertical_name,a.program_gid,a.program_name, " +
                    ...
                    " date_format(a.created_date,'%d-%m-%Y %h:%i %p') as created_date, " +
                    " concat(c.user_firstname,' ',c.user_lastname,' / ',c.user_code) as created_by " +
                    " from ocs_trn_tbusinesshierarchyupdatelog a" +
        ...
        public void DaGetCreditManagerRejectedApplSummary(string employee_gid, MdlCreditRejectedAppl values)
        {
            msSQL = " select a.application_gid,application_no,customer_name,customer_urn,vertical_name,applicant_type, region, " +
                    ...
                    " left join hrm_mst_temployee b on b.employee_gid=a.created_by " +
                    " left join adm_mst_tuser c on c.user_gid=b.user_gid " +
                    " left join ocs_trn_tappcreditmanagerrevokeapprovallog d on d.application_gid = a.application_gid " +
                    " left join ocs_trn_tcreditmanagerrevokelog e on e.application_gid = a.application_gid " +
        ...
        public bool DaPostCreditManagerRevokeApplication(Mdlcreditrevoke values, string user_gid, string employee_gid)
        {
            ...
            msSQL = "select creditmanagerrejectlog_gid from ocs_trn_tcreditmanagerrejectlog where application_gid='" + values.application_gid + "'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dt2 in dt_datatable.Rows)
                {
                    ...
                    msSQL = " insert into ocs_trn_tcreditmanagerrejectrevokereasonlog " +
                            ...
                            " from ocs_trn_tcreditmanagerrejectlog " +
                            " where application_gid='" + values.application_gid + "'";
            ...
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dt in dt_datatable.Rows)
                {
                    ...
                    msSQL = " insert into ocs_trn_tappcreditmanagerrevokeapprovallog " +
            ...
            {
                msSQL = " delete from ocs_trn_tcreditmanagerrejectlog " +
                        " where application_gid='" + values.application_gid + "' ";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                ...
                if (values.approval_status == "Rejected by Credit Manager")
                {
                    ...
                    msSQL = " insert into ocs_trn_tcreditmanagerrevokelog ( " +
                ...
                if (mnResult != 0)
                {
                    msSQL = " update ocs_mst_tapplication set approval_status='Submitted to Underwriting'" +
                           " where application_gid='" + values.application_gid + "'";
                    mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);                  
        ...
        public void DaGetCreditManagerHistoryLog(string application_gid, string employee_gid, MdlCreditHistoryLog values)
        {
            msSQL = " select a.application_gid,a.approval_remarks as rejected_remarks,creditmanagerrejectrevokereasonlog_gid,  " +
                    " concat(e.user_firstname, ' ', e.user_lastname, ' / ', e.user_code) as rejected_by, " +
                    " date_format(a.rejected_date, '%d-%m-%Y %h:%i %p') as rejected_date, a.approval_status, " +
                    " concat(h.user_firstname, ' ', h.user_lastname, ' / ', h.user_code) as revoked_by, " +
                    " date_format(f.created_date, '%d-%m-%Y %h:%i %p') as revoked_date , " +
                    " f.reason as revoked_reason,f.creditmanagerrevokelog_gid," +
                    " concat(h.user_firstname, ' ', h.user_lastname, ' / ', h.user_code) as revoked_by " +
                    " from ocs_trn_tcreditmanagerrejectrevokereasonlog a  " +
                    " left join hrm_mst_temployee d on d.employee_gid=a.rejected_by  " +
                    " left join adm_mst_tuser e on e.user_gid=d.user_gid " +
                    " left join ocs_trn_tcreditmanagerrevokelog f on f.application_gid=a.application_gid " +
                    " left join hrm_mst_temployee g on g.employee_gid=f.created_by  " +
                    " left join adm_mst_tuser h on h.user_gid=g.user_gid " +
                    " where a.application_gid = '" + application_gid + "' group by f.creditmanagerrevokelog_gid " +
                    " order by a.creditmanagerrejectrevokereasonlog_gid";
            dt_datatable = objdbconn.GetDataTable(msSQL);
            var getcreditmanagerhistorylog_list = new List<creditmanagerhistorylog_list>();
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dt in dt_datatable.Rows)
                {
                    getcreditmanagerhistorylog_list.Add(new creditmanagerhistorylog_list
                    {
                        application_gid = dt["application_gid"].ToString(),
                        rejected_by = dt["rejected_by"].ToString(),
                        rejected_date = dt["rejected_date"].ToString(),
                        revoked_by = dt["revoked_by"].ToString(),
                        revoked_date = dt["revoked_date"].ToString(),
                        revoked_reason = dt["revoked_reason"].ToString(),
                        rejected_remarks = dt["rejected_remarks"].ToString(),
                        approval_status = dt["approval_status"].ToString(),
                        creditmanagerrejectrevokereasonlog_gid = dt["creditmanagerrejectrevokereasonlog_gid"].ToString(),
                        creditmanagerrevokelog_gid = dt["creditmanagerrevokelog_gid"].ToString()
                    });

                }
            }
            values.creditmanagerhistorylog_list = getcreditmanagerhistorylog_list;
            dt_datatable.Dispose();
        }
        public void DaGetCreditManagerRejectedRemarksView(string creditmanagerrejectrevokereasonlog_gid, MdlCreditHistoryApplLog values)
        {
            try
            {
                msSQL = " select creditmanagerrejectrevokereasonlog_gid,approval_remarks,application_gid," +
                        " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as created_by, " +
                        " date_format(a.created_date, '%d-%m-%Y %h:%i %p') as created_date" +
                        " from ocs_trn_tcreditmanagerrejectrevokereasonlog a " +
                        " left join hrm_mst_temployee b on b.employee_gid = a.created_by " +
                        " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
                        " where creditmanagerrejectrevokereasonlog_gid = '" + creditmanagerrejectrevokereasonlog_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
                    values.creditmanagerrejectrevokereasonlog_gid = objODBCDatareader["creditmanagerrejectrevokereasonlog_gid"].ToString();
        ...
        public void DaGetCreditManagerRejectedRevokeRemarksView(string creditmanagerrevokelog_gid, MdlCreditHistoryApplLog values)
        {
            try
            {
                msSQL = " select creditmanagerrevokelog_gid,reason,application_gid," +
                        " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as created_by, " +
                        " date_format(a.created_date, '%d-%m-%Y %h:%i %p') as created_date" +
                        " from ocs_trn_tcreditmanagerrevokelog a " +
                        " left join hrm_mst_temployee b on b.employee_gid = a.created_by " +
                        " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
                        " where creditmanagerrevokelog_gid = '" + creditmanagerrevokelog_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
                    values.creditmanagerrevokelog_gid = objODBCDatareader["creditmanagerrevokelog_gid"].ToString();
                    values.reason = objODBCDatareader["reason"].ToString();
                    values.application_gid = objODBCDatareader["application_gid"].ToString();
                    values.created_by = objODBCDatareader["created_by"].ToString();
                    values.created_date = objODBCDatareader["created_date"].ToString();
