(function () {
    ...
    angular
        .module('angle')
        .controller('MstTransferCallSummaryController', MstTransferCallSummaryController);

    MstTransferCallSummaryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstTransferCallSummaryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstTransferCallSummaryController';
        activate();

        function activate() {
            var url = 'api/TeleCalling/GetEmpTransferredIBCallSummary';
            SocketService.get(url).then(function (resp) {
                $scope.ibcalltransfer_list = resp.data.ibcall_list;
                unlockUI();
            });           
        ...
        $scope.view = function (inboundcall_gid) {
            $location.url("app/MstAssignedCallView?hash=" + cmnfunctionService.encryptURL("inboundcall_gid=" + inboundcall_gid + '&lspage=TransferCall'));
        }
