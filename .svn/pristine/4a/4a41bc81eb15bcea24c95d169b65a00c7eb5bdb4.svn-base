(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADCompanyCrimeCheckRecordAPIController', MstCADCompanyCrimeCheckRecordAPIController);

        MstCADCompanyCrimeCheckRecordAPIController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce'];

    function MstCADCompanyCrimeCheckRecordAPIController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADCompanyCrimeCheckRecordAPIController';
        ...
        function activate() {
            ...
            var url = 'api/CADCrimeCheckAPI/GetTaggedCaseInstitutionSummary';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
        ...
        $scope.crimecheck_record = function () {
            $location.url('app/MstCADCompanyCrimeCheckRecordAPI?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&lsapi_name=PROPERTYTAX' + '&lspage=' + lspage);
        }
        ...
        $scope.CrimeCheck_Tag = function (data) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    ...
                    lockUI();
                    var url = 'api/CADCrimeCheckAPI/TagCaseInstitution';
        ...
        $scope.TaggedCase_Delete = function (crimecasetaggedinstitution_gid) {
            ...
            var url = 'api/CADCrimeCheckAPI/DeleteTaggedCaseInstitution';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.search_crimecheck = function () {                    
            ...
            if($scope.cboaddress == undefined || $scope.rdbSearchmode == undefined || searchIn == '') {
               ...
               var url = 'api/CADCrimeCheckAPI/GetCrimeRecordsCompany';
               lockUI();
               SocketService.post(url, params).then(function (resp) {               
                setTimeout(function() {
