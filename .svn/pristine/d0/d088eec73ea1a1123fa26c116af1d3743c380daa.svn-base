...
(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAVerificationMakerIndividualPendingController', MstSAVerificationMakerIndividualPendingController);

    MstSAVerificationMakerIndividualPendingController.$inject = ['DownloaddocumentService', '$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function MstSAVerificationMakerIndividualPendingController(DownloaddocumentService, $rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAVerificationMakerIndividualPendingController';
        ...
        function activate() {
            ...
            var url = 'api/MstSAOnboardingIndividual/TempChequeDocuments';
            SocketService.get(url).then(function (resp) {
...
//
        ...
        $scope.back = function () {
            ...
            if (lspage == 'IndividualPending') {
                $location.url('app/MstSAVerificationMakerPendingSummary');
            }
        ...
        $scope.makerindividualraisequery = function (sacontact_gid) {
            var modalInstance = $modal.open({
                templateUrl: '/makerindividualClose.html',
                controller: ModalInstanceCtrl,
                backdrop: 'static',
                keyboard: false,
                size: 'md'
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submit = function () {
                    ...
                    var url = 'api/MstSAOnboardingIndividual/PostMakerIndividualRaiseQuery';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        function makerquery_list(sacontact_gid) {
            var params = {
                sacontact_gid: sacontact_gid,
        ...
        $scope.final_approval = function () {
            ...
            if (($scope.cbopanstatus == 'Customer Submitting PAN') && ($scope.txtpan_no == '' || $scope.txtpan_no == undefined || $scope.txtpan_no == null)) {
            ...
            else if (($scope.rbointerview_evalution_form == undefined) || ($scope.rbointerview_evalution_form == '') || ($scope.rboapplication_form == undefined) || ($scope.rboapplication_form == '') || ($scope.rbocancelledcheckleaf == undefined) || ($scope.rbocancelledcheckleaf == '') || ($scope.rbokycdocuments == undefined) || ($scope.rbokycdocuments == '') || ($scope.rbophotographs == undefined) || ($scope.rbophotographs == '') || ($scope.rbovettingstatus == undefined) || ($scope.rbovettingstatus == '') || ($scope.rboaddressproof == undefined) || ($scope.rboaddressproof == '')) {
                ...
                var url = 'api/MstSAOnboardingIndividual/MakerIndividualInitated';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.makerindividualsaveas_draft = function () {
            ...
            if (($scope.cbopanstatus == 'Customer Submitting PAN') && ($scope.txtpan_no == '' || $scope.txtpan_no == undefined || $scope.txtpan_no == null)) {
                ...
                var url = 'api/MstSAOnboardingIndividual/MakerIndividualsaveasdraftInitated';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.credit_amountChange = function () {
            ...
            if (output == "NaN") {
                ...
                $scope.txtcredited_amount = output;
                document.getElementById('words_creditamt').innerHTML = lswords_annualturnover;
            }
        }
        ...
        $scope.reject = function () {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.rejectSubmit = function () {
                    ...
                    var url = 'api/MstSAOnboardingIndividual/IndividualMakerRejected';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
