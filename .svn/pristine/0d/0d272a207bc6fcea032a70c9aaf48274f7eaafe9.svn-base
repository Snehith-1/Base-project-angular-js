(function () {
    ...
    angular
        .module('angle')
        .controller('MstGradingToolViewController', MstGradingToolViewController);

    MstGradingToolViewController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];


    function MstGradingToolViewController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstGradingToolViewController';
        ...
        $scope.ViewAssessmentCriteria = function (val) {
            $location.url('app/MstAssessmentCriteriaDetailsView?application2gradingassesment_gid=' + val + '&application2gradingtool_gid=' + application2gradingtool_gid + '&application_gid=' + application_gid);
