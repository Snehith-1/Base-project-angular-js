(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstSuprUDCMakerViewController', AgrMstSuprUDCMakerViewController);

    AgrMstSuprUDCMakerViewController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function AgrMstSuprUDCMakerViewController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstSuprUDCMakerViewController';
        ...
        $scope.Back = function () {
            ...
            if (lspage == 'makerfollowup') {
                $location.url('app/AgrMstSuprChequeMakerFollowDtls?lsUdcManagement_gid=' + $scope.UdcManagement_gid + '&application_gid=' + application_gid + '&lspage=' + lspage);
            }
