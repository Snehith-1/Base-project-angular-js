(function () {
    ...
    angular
        .module('angle')
        .controller('MstApplCreationVisitReportViewController', MstApplCreationVisitReportViewController);

    MstApplCreationVisitReportViewController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'SweetAlert', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', '$cookies', '$filter', 'ngTableParams', '$resource', '$timeout', 'ngTableDataService', '$anchorScroll', '$sce', 'DownloaddocumentService', 'cmnfunctionService'];

    function MstApplCreationVisitReportViewController($rootScope, $scope, $modal, $state, SweetAlert, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, $cookies, $filter, ngTableParams, $resource, $timeout, ngTableDataService, $anchorScroll, $sce, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstApplCreationVisitReportViewController';
        ...
        function activate() {
           ...
           var params = {
            ...
            var url =  'api/MstApplicationView/GetVisitReportDtls';
        ...
        $scope.visitreportmobileno_view = function (applicationvisit2person_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.ok = function () {
                    ...
                    lockUI();
                    var url = 'api/MstApplicationView/GetVisitContactList';
                    SocketService.getparams(url, params).then(function (resp) {
                        unlockUI();
