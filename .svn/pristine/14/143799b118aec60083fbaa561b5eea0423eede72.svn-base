(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstProductRejectHoldRevokeController', AgrMstProductRejectHoldRevokeController);

    AgrMstProductRejectHoldRevokeController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function AgrMstProductRejectHoldRevokeController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstProductRejectHoldRevokeController';
        ...
        $scope.Back = function () {
            if (lspage == 'ProductHoldRevokeAppl') {
                $state.go('app.AgrMstProductHoldRevokeSummary');
            }
            else if (lspage == 'ProductRejectRevokeAppl') {
        ...
        $scope.revoke_submit = function () {
            ...
            var url = 'api/AgrAdminApplication/PostProductRevokeApplication';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
