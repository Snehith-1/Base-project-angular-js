(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprStartScheduledMeetingController', AgrTrnSuprStartScheduledMeetingController);

    AgrTrnSuprStartScheduledMeetingController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', '$sce', '$anchorScroll', 'DownloaddocumentService','cmnfunctionService'];

    function AgrTrnSuprStartScheduledMeetingController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, $sce, $anchorScroll, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprStartScheduledMeetingController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnSuprCC/GetApprovalList';
            ...
            var url = 'api/AgrTrnSuprCC/GetApprovalShowFlag';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
            ...
            var url = 'api/AgrTrnSuprCC/GetMomFlag';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.mom_flag = resp.data.mom_flag;
            });
            var url = 'api/AgrTrnSuprCC/GetMOMDescriptions';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.mom_descflag = resp.data.mom_descflag;
            });
            var url = 'api/AgrTrnSuprCC/GetMOMReapproval';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.mom_reapprove = resp.data.mom_reapprove;
            });
            var url = 'api/AgrTrnSuprCC/GetMOMRemail';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
            ...
            var url = 'api/AgrSuprCrimeCheckAPI/GetCCCaseTaggedIndividualSummary';
            ...
            var url = 'api/AgrSuprCrimeCheckAPI/GetCCCaseTaggedInstitutionSummary';
            SocketService.getparams(url, paramcrimetc).then(function (resp) {
                unlockUI();
                $scope.cccasetaggedinstitution_list = resp.data.cccasetaggedinstitution_list;
            });
            var url = 'api/AgrSuprCrimeCheckAPI/GetCCReportInstitutionSummary';
            SocketService.getparams(url, paramcrimetc).then(function (resp) {
                unlockUI();
                $scope.ccreportinstitution_list = resp.data.ccreportinstitution_list;
            });
            var url = 'api/AgrSuprCrimeCheckAPI/GetCCReportIndividualSummary';
            SocketService.getparams(url, paramcrimetc).then(function (resp) {
                unlockUI();
        ...
        $scope.Back = function () {          
            if (lspage == 'CCMmeetingScheduledcompleted') {
                $location.url('app/AgrTrnSuprCcScheduledMeetingSummary');
            }             
            else {
       ...
       var url = "api/AgrMstSuprApplicationEdit/GetGroupSummary";
        ...
        $scope.MOM_resubmit=function()
        {
            ...
            lockUI();
            var url = "api/AgrTrnSuprCC/PostReMOMSubmit";
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    Notify.alert(resp.data.message, {
        ...
        $scope.getReApprovalRequest = function (ccmeeting2members_gid) {
            var modalInstance = $modal.open({
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/AgrTrnSuprCC/GetApprovalInitiate';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                ...
                $scope.getreapprovalclick = function () {
                    var params = {
                        ...
                        ccmember_gid: $scope.ccmember_gid,                     
                        ccmeeting2members_gid: ccmeeting2members_gid,
                        application_gid: application_gid
                       
                    }
                    lockUI();
                    var url = "api/AgrTrnSuprCC/PostApprovalInitiate";
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
                            modalInstance.close('closed');
        ...
        $scope.cancelapprovalinitiate = function (ccmeeting2members_gid) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    var url = "api/AgrTrnSuprCC/CancelApprovalInitiate"
                    lockUI();
                    SocketService.getparams(url, params).then(function (resp) {
                        if (resp.data.status == true) {
