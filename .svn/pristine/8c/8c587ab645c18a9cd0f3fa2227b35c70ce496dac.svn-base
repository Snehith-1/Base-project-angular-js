(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditRejectandHoldSummaryController', AgrTrnSuprCreditRejectandHoldSummaryController);

    AgrTrnSuprCreditRejectandHoldSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrTrnSuprCreditRejectandHoldSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditRejectandHoldSummaryController';
        lockUI();
        activate();
        function activate() {
            var url = 'api/AgrTrnSuprCreditApproval/GetMyAppcreditRejectHoldSummary';
        ...
        $scope.applcreation_view = function (val) {
            $location.url('app/AgrTrnSuprCcCommitteeApplicationView?application_gid=' + val + '&lstab=CreditRejectHold');
        }
        ...
        $scope.credit_approval = function (application_gid, employee_gid, appcreditapproval_gid, initiate_flag) {
            $location.url('app/AgrTrnSuprCreditApproval?application_gid=' + application_gid + '&employee_gid=' + employee_gid + '&lspage=CreditRejectHold' + '&initiate_flag=N' + '&lsflag=N');
        }
