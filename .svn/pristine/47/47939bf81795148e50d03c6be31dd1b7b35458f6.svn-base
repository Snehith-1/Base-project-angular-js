...
{
    public class DaLglTrnDNTrackerFPO
    {
        ...
        int mnResult;
        public bool DaGetFPOPendingList(string employee_gid, MdlDNpendingList values)
        {
            msSQL = " (select max(od_days) as max_od_days,urn,acknowledgement_status,a.Customer_name,GurantorCustomerId,Guarantor_Name,Vertical,DN_status" +
                    " from lgl_trn_tmisdata a where vertical='FPO' and natureof_credit_amount >= 5000 " +
                    ...
                    " vertical='FPO' and natureof_credit_amount >= 5000 and " +
        ...
        public bool DaGetFPOGeneratedList(string employee_gid, MdlDNGeneratedList values)
        {
            msSQL = " select max(od_days) as max_od_days,urn,acknowledgement_status,Customer_name,GurantorCustomerId,Guarantor_Name,Vertical,DN_status from lgl_trn_tmisdata  " +
                    " where vertical='FPO'" +
                    " and natureof_credit_amount >=5000 and dn_status not in ('Pending','Legal SR','DN Skip') and exclusion_flag = 'N'  group by urn order by od_days desc";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public bool DaGetFPOSkippedList(string employee_gid, MdlDNSkippedList values)
        {
            msSQL = " select max(od_days) as max_od_days,a.urn,Customer_name,GurantorCustomerId,Guarantor_Name,Vertical," +
                    ...
                    " left join lgl_tmp_tmisdata b on a.urn = b.urn where  vertical='FPO'" +
        ...
        public bool DaGetFPOExclusionList(string employee_gid, MdlDNexclusionList values)
        {
            msSQL = "select account_no,count(urn),max(od_days) as max_od_days,urn,acknowledgement_status," +
                      ...
                      " concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as excluded_by from lgl_trn_tmisdata a" +
                       ...
                       " left join adm_mst_tuser c on b.user_gid = c.user_gid where vertical ='FPO' " +
        ...
        public bool DaGetFPOLegalSRList(MdlDNlegalsrList values, string employee_gid)
        {
            ...
            msSQL = " select a.customer_gid, a.templegalsr_gid,d.email,d.mobileno,a.auth_status,a.approval_status," +
                    ...
                    " where a.created_by='" + employee_gid + "' and vertical='FPO'" +
        ...
        public bool DaGetFPO_Count(string employee_gid, count_dtl values)
        {
