(function () {
    ...
    angular
        .module('angle')
        .controller('MstCadDocumentChecklistAddController', MstCadDocumentChecklistAddController);

    MstCadDocumentChecklistAddController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function MstCadDocumentChecklistAddController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCadDocumentChecklistAddController';
        ...
        function activate() {
            ...
            var geturl = "api/MstCADCreditAction/GetCADTrnCovenantTaggedDocList";
            SocketService.getparams(geturl, params).then(function (resp) {
                ...
                if (resp.data.TaggedDocument != null) {
                    ...
                    angular.forEach($scope.covenant_documentlist, function (value, key) {
                        var getselected = $scope.covenant_taggeddoclist.find(function (v) { return v.companydocument_gid === value.document_gid })
                        if (getselected != null) {
        ...
        $scope.Back = function () {
            if (lspage == "CadDocumentChecklist") {
                $location.url('app/MstCadGuarantorDetails?application_gid=' + application_gid + '&lspage=' + lspage + '&lsfollowup=' + lsfollowup);
            }
            else if (lspage == "CADAfterApproval") {
                $location.url('app/MstCadGuarantorApproval?application_gid=' + application_gid + '&lspage=' + lspage + '&lsfollowup=' + lsfollowup);
            }
            else  if(lspage=="RMDocChecklist"){
