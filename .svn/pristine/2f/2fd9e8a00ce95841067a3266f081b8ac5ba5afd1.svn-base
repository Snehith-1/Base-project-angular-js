(function () {
    ...
    angular
        .module('angle')
        .controller('MstAssignedInboundCallSummaryController', MstAssignedInboundCallSummaryController);

    MstAssignedInboundCallSummaryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstAssignedInboundCallSummaryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstAssignedInboundCallSummaryController';
        ...
        function activate() {
            ...
            var url = 'api/TeleCalling/GetAssignedCallSummary';
            SocketService.get(url).then(function (resp) {
                $scope.ibassignedcall_list = resp.data.ibcall_list;
                unlockUI();
            });
        }
        ...
        $scope.view = function (inboundcall_gid) {
            $location.url('app/MstTeleCallingAssignView?hash=' + cmnfunctionService.encryptURL('lsinboundcall_gid=' + inboundcall_gid + '&lspage=AssignedInboundCall'));
        }
