...
{
    /// <summary>
    ///To upload SBI Statement it digitize with LMS trnascation to get matched cases and bank transaction summary
    /// </summary>
    /// <remarks>Written by Niranchana</remarks>
    public class DaSbiReconcillation
    {
        ...
        string lspath, endRange, lsremarks, excelRange;
        int mnresult;
        int rowCount, columnCount,lsvalue_datecolumn;
        string lsbankconfig_gidrange, lstransc_idrange, lsrepayment_amount,lstrn_daterange, lsaccount_number, lsurn_no, lstransactionid, lstransaction_date, lsrepayment_date,
         ...
         lscustref_norange, lsbranch_namerange, lsbalance_amtrange, lschq_norange, lscr_drrange, lsbankremaining_amount,lsknockoff_status, lsknockoffby_financerange;
        string lstransact_particulars, lscheqref, lstransact_val, lsdrcr, lsbalance,lsvalue_date,lscredit_amt,lsdebit_amt;
        string lstrn_date, lstrndate,lspayment_date, lscustref_no, lsacc_no, lsbranch, lsbranchcode, lsknockofffin;
        ...
        int lscustref_norow, lscustref_nocolumn, lsacc_norow, lsacc_nocolumn, lsbranch_namerow, lsbranch_namecolumn, lsbranch_coderow,lsbranch_codecolumn, lsknockofffin_column;
        public bool DaPostBRSExcelSample(HttpRequest httpRequest, MdlSbiReconcillation objfilename, string employee_gid, string user_gid)
        {
            ...
            path = ConfigurationManager.AppSettings["file_path"] + "/erpdocument" + "/" + lscompany_code + "/" + "BRS/Sbireconcildoc/" + DateTime.Now.Year + "/" + DateTime.Now.Month;
            {
            ...
            try
            {
                if (httpRequest.Files.Count > 0)
                {
                    ...
                    for (int i = 0; i < httpFileCollection.Count; i++)
                    {
                        ...
                        string[] splits8 = lsremarksrange.Split(',');
                        string[] splits9 = lsbranch_namerange.Split(',');
                        string[] splits12 = lsknockoffby_financerange.Split(',');


                        lsbranch_coderow = (Convert.ToInt32(splits9[0]) == 0) ? 1 : Convert.ToInt32(splits9[0]);
                        lsbranch_codecolumn = (Convert.ToInt32(splits9[1]) == 0) ? 1 : Convert.ToInt32(splits9[1]);
...

                        {
                            ...
                            {
                                if (worksheet.Cells[lsbranch_coderow, lsbranch_codecolumn].Value != null)
                                {
                                    if ((!string.IsNullOrWhiteSpace(worksheet.Cells[lsbranch_coderow, lsbranch_codecolumn].Value.ToString()) || (worksheet.Cells[lsbranch_coderow, lsbranch_codecolumn].Value.ToString() != "")))
                                    {
                                        lsbranchcode = worksheet.Cells[lsbranch_coderow, lsbranch_codecolumn].Value.ToString();
                                    }
                                }
                ...
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow datarow in dt_datatable.Rows)
                    {
                        ...
                        if ((convertDecimalrepaymentamount < convertDecimalbankremainingamount) && (lsknockoff_status == "Pending"))
                        {
                            ...
                            //if (lstransact_val == null || lstransact_val == "")
                            //{
                            //    lstransact_val = "0";
                            //}
                            //var convertDecimaltransactvalremaining = Convert.ToDecimal(lstransact_val);
