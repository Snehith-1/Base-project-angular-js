(function () {
    ...
    angular
        .module('angle')
        .controller('AgrCreditAssessedScoreAddController', AgrCreditAssessedScoreAddController);

    AgrCreditAssessedScoreAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrCreditAssessedScoreAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrCreditAssessedScoreAddController';
        ...
        $scope.editAssessmentCriteria = function (val) {
            //    $location.url('app/AgrAssessmentCriteriaDetailsEdit?application2gradingassesment_gid=' + val + '&application_gid=' + application_gid + '&lspage=add');
            var params = {
        ...
        function assessmentcriteria_list() {
            var url = 'api/AgrMstApplicationGradingTool/GettmpGradingTool';
            SocketService.get(url).then(function (resp) {
        ...
        $scope.save_gradingtool = function () {
            ...
            var url = 'api/AgrMstApplicationGradingTool/SaveasDraftGradingToolDetails';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.Back = function () {
            if (lstab == 'ApplAssignmnet') {
                ...
                $state.go('app.AgrMyApplicationsSummary');
            }
            else {
        ...
        $scope.gradingtooldetails_edit = function (val) {
            $location.url('app/AgrCreditAssessedScoreEdit?application2gradingtool_gid=' + val + '&application_gid=' + application_gid + '&lstab=' + lstab);
        }
        ...
        $scope.gradingtooldetails_view = function (application2gradingtool_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrCreditAssessedScoreView";
        ...
        $scope.RMassessedscore_view = function (application2gradingtool_gid) {
            ...
            localStorage.setItem('application2gradingtool_gid', application2gradingtool_gid);
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrRMAssessedScoreView";
            window.open(URL, '_blank');
        }
