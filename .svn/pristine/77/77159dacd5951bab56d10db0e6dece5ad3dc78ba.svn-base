(function () {
    ...
    angular
        .module('angle')
        .controller('MstCadPendingApplicationViewController', MstCadPendingApplicationViewController);

    MstCadPendingApplicationViewController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function MstCadPendingApplicationViewController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCadPendingApplicationViewController';
        ...
        const lspagename = 'MstCadPendingApplicationView';
        ...
        function refreshproductdetails() {
            ...
            var url = 'api/MstCAD/GetAppLimitInfoDtl';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
        ...
        $scope.Back = function () {
            if (lspage == 'PendingCADReview') {
                ...
                $location.url('app/MstCadUrnAcceptedCustomerDtls?application_gid=' + application_gid + '&lspage=CADUrnAcceptanceCustomer');
            }
            else if (lspage == 'CadPendingStage') {
                $location.url('app/MstCadPendingStageSummary');
            }
            else {
        ...
        $scope.institution_view = function (institution_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCcCommitteeInstitutionView?application_gid=" + application_gid + '&institution_gid='+ institution_gid + '&lspage='+ lspage +'&lspagetype='+ lspagetype; //+ '&lscompany='+ 'Company'
            window.open(URL, '_blank');
        }
        ...
        $scope.individual_view = function (contact_gid) {
            ...
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCcCommitteeIndividualView?application_gid=" + application_gid + '&contact_gid='+ contact_gid + '&lspage='+ lspage +'&lspagetype='+ lspagetype; //+ '&lsindividual='+ 'Individual'
            window.open(URL, '_blank');
        }
        ...
        $scope.group_view = function (group_gid) {
            ...
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCcCommitteeGroupView?application_gid=" + application_gid + '&group_gid='+ group_gid + '&lspage='+ lspage +'&lspagetype='+ lspagetype; //+ '&lsgroup='+ 'Group'
            window.open(URL, '_blank');
        }
