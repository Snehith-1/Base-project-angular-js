(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnApplCCApprovedController', AgrTrnApplCCApprovedController);

    AgrTrnApplCCApprovedController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrTrnApplCCApprovedController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnApplCCApprovedController';
        lockUI();
        activate();
        function activate() {
            var url = 'api/AgrMstApplicationAdd/GetApplCCApprovedSummary';
        ...
        $scope.applcreation_view = function (val) {
            $location.url('app/AgrApplicationCreationView?application_gid=' + val + '&lstab=CCApproved');
        }
        ...
        $scope.ccmeetingskip_history = function (application_gid) {
            $location.url('app/AgrTrnCcMeetingSkipHistory?application_gid=' + application_gid + '&lspage=CCApproved');
        }
