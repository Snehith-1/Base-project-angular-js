(function () {
    ...
    angular
        .module('angle')
        .controller('MstTaggedMemberSummaryController', MstTaggedMemberSummaryController);

    MstTaggedMemberSummaryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstTaggedMemberSummaryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstTaggedMemberSummaryController';
        activate();

        function activate() {
            var url = 'api/TeleCalling/GetEmpTaggedSummary';
            SocketService.get(url).then(function (resp) {
                $scope.ibcalltagged_list = resp.data.ibcall_list;
                unlockUI();
            });           
        ...
        $scope.view = function (inboundcall_gid) {
            $location.url("app/MstTaggedMemberView?hash=" + cmnfunctionService.encryptURL("inboundcall_gid=" + inboundcall_gid));
        }
