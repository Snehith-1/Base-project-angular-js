(function () {
    ...
    angular
        .module('angle')
        .controller('MstCadDeferralQueryController', MstCadDeferralQueryController);

    MstCadDeferralQueryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService', '$anchorScroll','cmnfunctionService'];

    function MstCadDeferralQueryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, DownloaddocumentService, $anchorScroll, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCadDeferralQueryController';
        ...
        $scope.updatedocumentsubmissiondate = function () {
            ...
            var url = 'api/MstScannedDocument/GetUpdateDocumentSubmission';
            SocketService.post(url, params).then(function (resp) {
                if (resp.data.status == true) {  
                    unlockUI();
                    Notify.alert(resp.data.message, {
        ...
        $scope.deletependingquery = function(tagquery_gid){
            ...
            var url = 'api/MstScannedDocument/GetPendingQueryDelete';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) { 
                if (resp.data.status == true) {
                    unlockUI();
