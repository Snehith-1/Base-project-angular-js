(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditProductChargesDtlEditController', AgrTrnSuprCreditProductChargesDtlEditController);

    AgrTrnSuprCreditProductChargesDtlEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function AgrTrnSuprCreditProductChargesDtlEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditProductChargesDtlEditController';
        ...
        function activate() {
            ...
            var url = 'api/AgrMstSuprApplicationEdit/GetEditproduct';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
        ...
        $scope.creditproductchargesloan_edit = function (application2loan_gid, product_gid, variety_gid) {
            $location.url('app/AgrTrnSuprCreditLoanDtlEdit?application2loan_gid=' + application2loan_gid + '&application_gid=' + $scope.application_gid + '&lspage=' + lspage + '&product_gid=' + product_gid + '&variety_gid=' + variety_gid);
        }
        ...
        $scope.servicedtl_edit = function (application2servicecharge_gid) {
            $location.url('app/AgrTrnSuprCreditServicesDtlEdit?application2servicecharge_gid=' + application2servicecharge_gid + '&application_gid=' + $scope.application_gid + '&lspage=' + lspage + '&product_gid=' + product_gid + '&variety_gid=' + variety_gid);
        }
    }
})();
