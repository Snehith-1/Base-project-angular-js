(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCadDeferralQueryController', AgrTrnSuprCadDeferralQueryController);

    AgrTrnSuprCadDeferralQueryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

        function AgrTrnSuprCadDeferralQueryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCadDeferralQueryController';
        ...
        function activate() {
            lockUI();
            var url = 'api/AgrMstSuprScannedDocument/tmpclearQueryuploaded';
            ...
            var url = 'api/AgrMstSuprScannedDocument/GetAppcadQuerySummary';
            SocketService.getparams(url, param).then(function (resp) {
            ...
            var url = 'api/AgrMstSuprScannedDocument/GetMakerCheckerConversation';
        ...
        $scope.query_add = function () {
            ...
            var url = 'api/AgrMstSuprScannedDocument/PostAppcadqueryadd';
            lockUI();
        ...
        $scope.QueryDocumentUpload = function (val, val1, name) {
            ...
            if ($scope.uploadfrm != undefined) {
                var url = 'api/AgrMstSuprScannedDocument/QueryDocumentUpload';
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                    ...
                    var url = 'api/AgrMstSuprScannedDocument/GetTmpQueryDocument';
                    SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.defdoc_delete = function (tagquerydocument_gid, tagquery_gid) {
            ...
            var url = 'api/AgrMstSuprScannedDocument/cancelQueryuploaddocument';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.defferaldoc_view = function (tagquery_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/AgrMstSuprScannedDocument/GetQueryDocument';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
        ...
        $scope.Back = function () {
            $location.url('app/AgrTrnSuprCadDeferralDochecklist?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
        }
        ...
        $scope.responseclick = function () {
            ...
            var url = 'api/AgrMstSuprScannedDocument/postMakerCheckerConversation';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
