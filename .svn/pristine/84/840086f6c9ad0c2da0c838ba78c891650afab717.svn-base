(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADGroupDtlAddController', MstCADGroupDtlAddController);

    MstCADGroupDtlAddController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', '$sce', '$anchorScroll', 'DownloaddocumentService','cmnfunctionService'];

    function MstCADGroupDtlAddController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, $sce, $anchorScroll, DownloaddocumentService, cmnfunctionService) {

        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADGroupDtlAddController';
        ...
        function groupaddress_list() {
            var url = 'api/MstCADApplication/GetGroupAddressList';
            lockUI();
            SocketService.get(url).then(function (resp) {
                unlockUI();
        ...
        $scope.groupaddress_delete = function (group2address_gid) {
            ...
            var url = 'api/MstCADApplication/DeleteGroupAddressDetail';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        function groupbank_list() {
            var url = 'api/MstCADApplication/GetGroupBankList';
            lockUI();
            SocketService.get(url).then(function (resp) {
                unlockUI();
        ...
        function groupdocument_list() {
            var url = 'api/MstCADApplication/GetGroupDocumentList';
            lockUI();
            SocketService.get(url).then(function (resp) {
                unlockUI();
        ...
        $scope.group_submit = function () {
            ...
            if (($scope.URN_yes == true) && (($scope.txtgroup_urn == '') || ($scope.txtgroup_urn == undefined))) {
                ...
                var url = 'api/MstCADApplication/SubmitGroupDtlAdd';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        function equipmentholding_list() {
            var url = 'api/MstCADApplication/GetGroupEquipmentHoldingList';
            SocketService.get(url).then(function (resp) {
        ...
        function livestock_list() {
            var url = 'api/MstCADApplication/GetGroupLivestockList';
            SocketService.get(url).then(function (resp) {
