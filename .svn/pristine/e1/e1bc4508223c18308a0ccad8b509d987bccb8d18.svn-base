(function () {
    ...
    angular
        .module('angle')
        .controller('MstApplCCApprovedController', MstApplCCApprovedController);

    MstApplCCApprovedController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstApplCCApprovedController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstApplCCApprovedController';
        lockUI();
        activate();
        function activate() {
            var url = 'api/MstApplicationAdd/GetApplCCApprovedSummary';
        ...
        $scope.applcreation_view = function (val) {
            $location.url('app/MstApplicationCreationView?application_gid=' + val + '&lstab=CCApproved');
        }
        ...
        $scope.ccmeetingskip_history = function (application_gid) {
            $location.url('app/MstCcMeetingSkipHistory?application_gid=' + application_gid + '&lspage=CCApproved');
        }
