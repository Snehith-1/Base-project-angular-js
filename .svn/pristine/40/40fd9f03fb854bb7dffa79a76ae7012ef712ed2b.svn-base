(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAVerificationMakerPendingSummaryController', MstSAVerificationMakerPendingSummaryController);

    MstSAVerificationMakerPendingSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstSAVerificationMakerPendingSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAVerificationMakerPendingSummaryController';

        activate();

        function activate() {
            var url = 'api/MstSAOnboardingIndividual/GetSAVerfiyMakerindividualPending';
            lockUI();
        ...
        $scope.saonboardingverification = function (sacontact_gid) {
            ...
            $location.url('app/MstSAVerificationMakerIndividualPending?hash=' + cmnfunctionService.encryptURL('lssacontact_gid=' + sacontact_gid + '&lspage=IndividualPending'));
        }
    }
})();
