(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstAppEditOverallLimitAdd', AgrMstAppEditOverallLimitAdd);

    AgrMstAppEditOverallLimitAdd.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function AgrMstAppEditOverallLimitAdd($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstAppEditOverallLimitAdd';
                ...
                SocketService.post(url, params).then(function (resp) {
                    ...
                    if (resp.data.status == true) {
                        ...
                        if (lstab == 'add') {
                            ...
                            $location.url('app/AgrMstAppEditProductAdd?lsapplication_gid=' + $scope.application_gid + '&lstab=edit');
                        }
                    }
                    else {
                        Notify.alert(resp.data.message, {
