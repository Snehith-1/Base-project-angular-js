(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADCreditIndividualAPIController', MstCADCreditIndividualAPIController);

    MstCADCreditIndividualAPIController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce'];

    function MstCADCreditIndividualAPIController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADCreditIndividualAPIController';
        ...
        function activate() {
            ...
            var url = 'api/CADCrimeCheckAPI/GetIndividualReportList';
            SocketService.getparams(url, paramcrime).then(function (resp) {
                unlockUI();
        ...
        $scope.PropertyTaxdelete = function (propertytax_gid) {
         ...
         $scope.raise_crimereportrequest = function () {
            ...
            if($scope.cboaddress == '' || $scope.cboaddress == null || $scope.cboaddress == undefined ) {
                ...
                var url = 'api/CADCrimeCheckAPI/RequestCrimeReportIndividual';
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
        ...
        $scope.crimereport_view = function (crimereportcontact_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCrimeReportIndividualView";
            window.open(URL, '_blank');
        }
