(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditAssessedScoreAddController', MstCreditAssessedScoreAddController);

    MstCreditAssessedScoreAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstCreditAssessedScoreAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditAssessedScoreAddController';
    ...
    /*    var lstab = $scope.lstab;*/
        ...
        $scope.Back = function () {
         /*   $location.url('app/MstAppassignedAssignmentSummary?application_gid=' + application_gid);*/
            if (lstab == 'AssignmentSummary') {
                $state.go('app.MstAppassignedAssignmentSummary');
            }
            else if (lstab == 'AssessedScore') {
                $state.go('app.MstAppassignedAssignmentSummary');
            }
            else if (lstab == 'gradingtool') {
                $state.go('app.MstMyApplicationsSummary');
            }
        }
      
        $scope.gradingtooldetails_edit = function (val) {
            $location.url('app/MstCreditAssessedScoreEdit?application2gradingtool_gid=' + val + '&application_gid=' + application_gid + '&lstab='+ lstab );
        }
        ...
        $scope.gradingtooldetails_view = function (application2gradingtool_gid) { 
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCreditAssessedScoreView";
        ...
        $scope.RMassessedscore_view = function (application2gradingtool_gid) {
            ...
            localStorage.setItem('application2gradingtool_gid', application2gradingtool_gid);
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstRMAssessedScoreView";
            window.open(URL, '_blank');
        }
