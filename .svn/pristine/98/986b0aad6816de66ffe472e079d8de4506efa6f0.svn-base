...
/// <summary>
/// (It's used for Application Creation View) ApplicationView DataAccess Class accessed by API methods from related Controller class and is returning relevant response to client.
/// </summary>
/// <remarks>Written by Sumala,Logapriya and Abilash</remarks>

namespace ems.master.DataAccess
{
    public class DaMstApplicationView
    {
        ...
        string msSQL, lsapplication_gid, lsapplication_gidcontact, lsapplication2hypothecation_gid, hypo_flag;
        int mnResult;
        ...
        public void DaGetApplicationBasicView(string application_gid, MdlMstApplicationView values)
        {
            try
            {
                msSQL = " select a.application_gid, a.application_no, customer_urn, customerref_name as customer_name, vertical_name, verticaltaggs_name,a.cccompleted_flag, " +
                        ...
                        " case when e.urn = '' then d.urn else e.urn end as customer_urnno from ocs_mst_tapplication a" +
                        ...
                        " left join ocs_mst_tinstitution d on d.application_gid = a.application_gid " +
                        " left join ocs_mst_tcontact e on e.application_gid = a.application_gid " +
                ...
                msSQL = " select application2hypothecation_gid from ocs_mst_tapplication2hypothecation " +
                ...
                msSQL = "select count(*) from ocs_trn_tscorecardgrouptitle where application_gid='" + application_gid + "'";
                values.scorecard_submit = objdbconn.GetExecuteScalar(msSQL);

                msSQL = " select application_gid from ocs_mst_tinstitution " +
                ...
                {
                    msSQL = " select application_gid from ocs_mst_tcontact " +
                            " where application_gid='" + application_gid + "' and (stakeholder_type='Applicant' or stakeholder_type='Borrower')";
        ...
        {
            msSQL = "select mobile_no from ocs_mst_tapplication2contactno where application_gid='" + application_gid + "' and primary_mobileno = 'Yes'";
            values.primary_mobileno = objdbconn.GetExecuteScalar(msSQL);

            msSQL = "select email_address from ocs_mst_tapplication2email where application_gid='" + application_gid + "' and primary_emailaddress = 'Yes'";
            ...
            try
            {
                msSQL = " select application2contact_gid, application_gid, mobile_no, whatsapp_mobileno " +
                        " from ocs_mst_tapplication2contactno " +
                ...
                msSQL = " select application2email_gid, application_gid, email_address " +
                        " from ocs_mst_tapplication2email " +
        ...
        public void DaGetGeneticDetailsView(string application_gid, MdlMstApplicationView values)
        {
            ...
            try
            {
                msSQL = " select geneticcode_name, genetic_status, genetic_remarks, application_gid, geneticcode_gid " +
                        " from ocs_mst_tapplication2geneticcode " +
                        " where application_gid = '" + application_gid + "' ";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetBorrowerInstitutionView(string application_gid, MdlMstInstitutionDtlView values)
        {
            try
            {
                msSQL = " select institution_gid, application_gid, company_name, companypan_no, date_format(date_incorporation, '%d-%m-%Y') as date_incorporation, " +
                        ...
                        " date_format(businessstart_date, '%d-%m-%Y') as businessstart_date, nearsamunnatiabranch_gid,nearsamunnatiabranch_name, " +
                        " udhayam_registration,tan_number,business_description,tanstate_gid,tanstate_name,internalrating_gid,internalrating_name, " +
                        " sales, purchase, credit_summation, cheque_bounce, numberof_boardmeetings, farmer_count, crop_cycle,calamities_prone " +
                        " from ocs_mst_tinstitution " +
                        " where application_gid = '" + application_gid + "' and (stakeholder_type = 'Applicant' or stakeholder_type='Borrower')";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
               ...
               msSQL = " select GROUP_CONCAT(distinct(b.city_name) SEPARATOR ', ') as city_name " +
                       ...
                       " left join ocs_mst_tinstitution2fpocacity b on b.institution_gid = a.institution_gid " +
                ...
                msSQL = " select institution2receivable_gid,institution_gid,receivable_date,onetothirty_days, " +
                   " thirtyonetosixty_days,sixtyonetoninety_days,ninety_days from ocs_mst_tinstitution2receivable " +
                ...
                msSQL = "select institution2branch_gid,gst_state,gst_no, gst_registered,headoffice_status from ocs_mst_tinstitution2branch where institution_gid='" + values.institution_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                msSQL = "  select institution2address_gid,addresstype_name,primary_status, addressline1, addressline2, taluka, district, state, country, landmark," +
                    " postal_code, city from ocs_mst_tinstitution2address where institution_gid='" + values.institution_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                msSQL = " select institution_gid, mobile_no, whatsapp_no " +
                        " from ocs_mst_tinstitution2mobileno " +
                ...
                msSQL = " select institution_gid, email_address " +
                        " from ocs_mst_tinstitution2email " +
                        " where institution_gid = '" + values.institution_gid + "' and primary_status = 'No' ";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                msSQL = " select institution2documentupload_gid,institution_gid,document_name,document_path, " +
                        " document_title,document_id, migration_flag from ocs_mst_tinstitution2documentupload " +
                        " where institution_gid='" + values.institution_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                objODBCDatareader.Close();
                msSQL = "select cicdocument_name,cicdocument_path from ocs_mst_tinstitution2cicdocumentupload" +
                        " where institution_gid='" + values.institution_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
        ...
        public void DaGetBorrowerIndividualView(string application_gid, MdlMstIndividualDtlView values)
        {
            try
            {
                ...
                msSQL = " SELECT panabsencereason" +
                  " from ocs_mst_tcontact2panabsencereason where contact_gid='" + values.contact_gid + "'";
                ...
                msSQL = " select contact2address_gid,addresstype_name,primary_status, addressline1, addressline2, taluka, district, state, country," +
                   " postal_code, landmark, city from ocs_mst_tcontact2address where contact_gid='" + values.contact_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                msSQL = "select contact2idproof_gid,idproof_name,idproof_no,document_name, document_path from ocs_mst_tcontact2idproof where " +
                        " contact_gid='" + values.contact_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                ...
                msSQL = " select document_title,document_name,document_path,migration_flag from ocs_mst_tcontact2document " +
                ...
                msSQL = "select cicdocument_name,cicdocument_path from ocs_mst_tindividual2cicdocumentupload " +
                        " where contact_gid='" + values.contact_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
        ...
        public void DaGetGurantorIndividualList(string application_gid, MdlMstGurantorView values)
        {
            msSQL = " select a.contact_gid, a.application_gid, concat_ws(' ', first_name, last_name, middle_name) as individual_name, " +
                        " a.pan_no, concat('XXXX-XXXX-',RIGHT(aadhar_no,4)) as aadhar_no, date_format(individual_dob, '%d-%m-%Y') as individual_dob," +
                        " main_occupation, date_format(a.created_date, '%d-%m-%Y') as created_date," +
                        "concat(c.user_firstname,' ',c.user_lastname,' / ',c.user_code) as created_by,stakeholder_type from ocs_mst_tcontact a" +
                        " left join hrm_mst_temployee b on b.employee_gid = a.created_by" +
                        " left join adm_mst_tuser c on b.user_gid = c.user_gid" +
        ...
        public void DaGetGurantorInstitutionList(string application_gid, MdlMstGurantorView values)
        {
            msSQL = " select a.institution_gid, a.application_gid, cin_no, companytype_name, " +
                        ...
                        "concat(c.user_firstname,' ',c.user_lastname,' / ',c.user_code) as created_by,stakeholder_type from ocs_mst_tinstitution a" +
                        " left join hrm_mst_temployee b on b.employee_gid = a.created_by" +
                        " left join adm_mst_tuser c on b.user_gid = c.user_gid" +
                        " where application_gid = '" + application_gid + "' and stakeholder_type!='Applicant' and stakeholder_type!='Borrower'";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetGurantorInstitutionView(string institution_gid, MdlMstInstitutionDtlView values)
        {
            try
            {
                ...
                msSQL = "select institution2branch_gid,gst_state,gst_no, gst_registered,headoffice_status from ocs_mst_tinstitution2branch where institution_gid='" + institution_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getmstgst_list = new List<mstgst_list>();
                if (dt_datatable.Rows.Count != 0)
                {
                ...
                msSQL = "  select institution2address_gid,addresstype_name,primary_status, addressline1, addressline2, taluka, district, state, country, landmark," +
                    " postal_code, city from ocs_mst_tinstitution2address where institution_gid='" + institution_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getmstaddress_list = new List<mstaddress_list>();
                if (dt_datatable.Rows.Count != 0)
                {
        ...
        public void DaGetGurantorIndividualView(string contact_gid, MdlMstIndividualDtlView values)
        {
            try
            {
              ...
              " from ocs_mst_tcontact2panabsencereason where contact_gid='" + contact_gid + "'";
                ...
                msSQL = " select contact2address_gid,addresstype_name,primary_status, addressline1, addressline2, taluka, district, state, country," +
                   " postal_code, landmark, city from ocs_mst_tcontact2address where contact_gid='" + contact_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getcontactaddress_list = new List<contactaddress_list>();
                if (dt_datatable.Rows.Count != 0)
                {
                ...
                msSQL = "select mobile_no from ocs_mst_tcontact2mobileno where contact_gid='" + contact_gid + "' and primary_status = 'Yes'";
                values.primaryindividual_mobileno = objdbconn.GetExecuteScalar(msSQL);

                msSQL = "select email_address from ocs_mst_tcontact2email where contact_gid='" + contact_gid + "' and primary_status = 'Yes'";
                values.primaryindividual_email = objdbconn.GetExecuteScalar(msSQL);
        ...
        public void DaGetProductChargesDtl(string application_gid, MdlMstProductChargesView values)
        {
            try
            {
                ...
                msSQL = " select overalllimit_amount, validityoveralllimit_year, validityoveralllimit_month, validityoveralllimit_days, " +
                                " calculationoveralllimit_validity,csa_applicability,csaactivity_gid,csaactivity_name,percentageoftotal_limit from ocs_mst_tapplication " +
                                " where application_gid='" + application_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
                ...
                msSQL = " select application2loan_gid, source_type, guideline_value, date_format(guideline_date, '%d-%m-%Y') as guideline_date, " +
                        " date_format(marketvalue_date, '%d-%m-%Y') as marketvalue_date, market_value, " +
                        " forcedsource_value, collateralSSV_value, " +
                        " date_format(forcedvalueassessed_on, '%d-%m-%Y') as forcedvalueassessed_on, " +
                        " collateralobservation_summary,product_type,productsub_type from ocs_mst_tapplication2loan  " +
                ...
                msSQL = " select producttype_gid,product_type, processing_fee,processing_collectiontype, " +
                        " doc_charges," +
                        " doccharge_collectiontype,fieldvisit_charges, " + 
                        " fieldvisit_charges_collectiontype, adhoc_fee,adhoc_collectiontype," +
                        " life_insurance,lifeinsurance_collectiontype,acct_insurance, " +
                        " total_collect,  total_deduct," +
                ...
                msSQL = " select application2hypothecation_gid, security_type, security_description, format(security_value,0,'en_IN') as security_value, " +
                        ...
                        " from ocs_mst_tapplication2hypothecation " +
                        " where application_gid='" + application_gid + "'";
                objODBCDatareader = objdbconn.GetDataReader(msSQL);
                if (objODBCDatareader.HasRows == true)
                {
        ...
        public void DaGetPurposeofLoan(string application2loan_gid, MdlMstProductChargesView values)
        {
            try
            {
                msSQL = " select enduse_purpose from ocs_mst_tapplication2loan where application2loan_gid='" + application2loan_gid + "'";
        ...
        public void DaGetGrouptoMemberList(string group_gid, MdlMstGroupMember values)
        {
            msSQL = "select a.contact_gid,a.pan_no,a.aadhar_no,concat(first_name, ' ',middle_name,' ',last_name) as individual_name,stakeholder_type,credit_status," +
                    " (select count(*) from ocs_trn_tgroupdocumentchecklist where credit_gid =a.contact_gid " +
                    " and(untagged_type is null or untagged_type = 'N')) as OverallDeferralcount, " +
                    " (select count(*) from ocs_trn_tgroupcovenantdocumentchecklist where credit_gid = a.contact_gid " +
        ...
        public void DaGetPrimaryAndOtherMobileNumbers(string contact_gid, MdlMstContactMobileNumbers values)
        {
            ...
            try
            {
                ...
                msSQL = " select contact_gid, mobile_no, whatsapp_no " +
                        " from ocs_mst_tcontact2mobileno " +
        ...
        {
              msSQL = " select a.contact_gid, a.application_gid, concat_ws(' ', first_name, last_name, middle_name) as individual_name," +
                      " (select crimecheck_flag from ocs_trn_tcrimechecksearchrecord xx where " +
                      " xx.contact_gid = a.contact_gid order by (xx.created_date) desc limit 1) as crimecheck_flag," +
                      " (select crimecheck_flag from ocs_trn_tcrimecheckrtsearchrecord cc where " +
                      ...
                      " and(untagged_type is null or untagged_type = 'N') and overall_docstatus in ('Waived','Document Verified')) as verifieddeferraldoc, " +
                      " (select count(*) from ocs_trn_tgroupcovenantdocumentchecklist where credit_gid = a.contact_gid " +
                      " and(untagged_type is null or untagged_type = 'N') and overall_docstatus in ('Waived','Document Verified'))  as verifiedcovenantdoc, f.vertical_gid, f.vertical_name, g.bre_status, " +
                       " case when  a.stakeholder_type !='Applicant'  then 'Y' else 'N' end as creditguarantordelete_flag " +
                      " from ocs_mst_tcontact a" +
                      " left join hrm_mst_temployee b on b.employee_gid = a.created_by" +
                      " left join adm_mst_tuser c on b.user_gid = c.user_gid" +
                      " left join ocs_mst_tapplication f on f.application_gid = a.application_gid" +
                      " left join ocs_trn_tverticalscorecardgrouptitle g on g.editruletype_gid = a.contact_gid" +
            ...
            if (dt_datatable.Rows.Count != 0)
            {
                foreach (DataRow dr_datarow in dt_datatable.Rows)
                {
                    getindividualList.Add(new individual_List
                    {
                        ...
                        creditguarantordelete_flag = (dr_datarow["creditguarantordelete_flag"].ToString()),
                    });
                }
        ...
        public void DaGetInstitutionList(string application_gid, MdlCreditView values)
        {
            msSQL = " select a.institution_gid, a.application_gid, cin_no, companytype_name, " +
                     ...
                     " (select count(*) from ocs_trn_tgroupdocumentchecklist where credit_gid =a.institution_gid " +
                     " and(untagged_type is null or untagged_type = 'N')) as OverallDeferralcount, " +
                     " (select count(*) from ocs_trn_tgroupcovenantdocumentchecklist where credit_gid = a.institution_gid " +
                     " and(untagged_type is null or untagged_type = 'N')) as overallCovenantCount, " +
                     " (select count(*) from ocs_trn_tgroupdocumentchecklist where credit_gid = a.institution_gid " +
                     " and(untagged_type is null or untagged_type = 'N') and overall_docstatus in ('Waived','Document Verified')) as verifieddeferraldoc, " +
                     " (select count(*) from ocs_trn_tgroupcovenantdocumentchecklist where credit_gid = a.institution_gid " +
                     ...
                     " left join ocs_trn_tverticalscorecardgrouptitle e on e.editruletype_gid = a.institution_gid" +
        ...
        public void DaGetRMDetailsView(string application_gid, MdlRMDtlView values)
        {
            try
            {
                msSQL = " select d.department_name,concat(c.user_firstname, ' ', c.user_lastname, ' / ', c.user_code) as RM_Name, " +
                        ...
                        " concat(f.user_firstname, ' ', f.user_lastname, ' / ', f.user_code) as ccsubmitted_by from ocs_mst_tapplication a " +
                        " left join hrm_mst_temployee b on b.employee_gid = a.created_by " +
                        " left join adm_mst_tuser c on c.user_gid = b.user_gid " +
        ...
        public void DaGetLoanProgramValueChain(string application2loan_gid, MdlMstProductChargesView values)
        {
            try
            {
                ...
                msSQL = " select application2product_gid,product_gid,product_name,variety_gid,variety_name,sector_name,category_name," +
                        " botanical_name,alternative_name,csacommodity_average,csapercentageoftotal_limit from ocs_mst_tapplication2product where application2loan_gid='" + application2loan_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetIndividualImportLog(string application_gid, MdlExcelImportApplication values)
        {
            try
            {
                msSQL = " select concat(first_name, ' ', last_name) as individual_name, contactimportlog_status as reason " +
                        " from ocs_trn_tcontactimportlog" +
                        " where application_gid = '" + application_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetInstitutionImportLog(string application_gid, MdlExcelImportApplication values)
        {
            try
            {
                msSQL = " select company_name, institutionimportlog_status as reason " +
                        " from ocs_trn_tinstitutionimportlog" +
                        " where application_gid = '" + application_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        {
            msSQL = " select group_concat(ifnull(c.urn,''))   from ocs_mst_tapplication a " +
            ...
            msSQL = " select group_concat(ifnull(c.urn,''))   from ocs_mst_tapplication a " +
                    " left join ocs_mst_tcontact c on c.application_gid = a.application_gid " +
                    " where a.application_gid = '" + application_gid + "'";
            ...
            msSQL = " select group_concat(ifnull(c.group_urn,''))   from ocs_mst_tapplication a " +
                    " left join ocs_mst_tgroup c on c.application_gid = a.application_gid " +
                    " where a.application_gid = '" + application_gid + "'";
            ...
            try
            {
                msSQL = " select a.customer_name, b.stakeholder_type, 'Institution' as applicant_type, a.urn,a.account_no,(format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt, " +
                        ...
                        " left join ocs_mst_tinstitution b on a.urn = b.urn " +
                        ...
                        "  WHEN a.ac_status = '3' THEN ' Frozen' " +
                         "END as ac_status,(format((a.ledger * -1),2,'en_IN')) as ledger,a.rbiold_oddays, a.late_charge,a.rpa, " +
                        " a.max_dpd,a.Vertical,a.lastdemandrundate from lgl_tmp_tmisdata a " +
                        " left join ocs_mst_tcontact b on a.urn = b.urn " +
                        ...
                        "  WHEN a.ac_status = '3' THEN ' Frozen' " +
                         "END as ac_status,(format((a.ledger * -1),2,'en_IN')) as ledger,a.rbiold_oddays,a.late_charge,a.rpa,  " +
                        " a.max_dpd,a.Vertical,a.lastdemandrundate from lgl_tmp_tmisdata a " +
                        " left join ocs_mst_tgroup b on a.urn = b.group_urn " +
            ...
            try
            {
                msSQL = "select a.customer_name, c.stakeholder_type, 'Institution' as applicant_type, a.urn, a.account_no, (format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt, " +
                        ...
                        "  (format((a.ledger * -1),2,'en_IN')) as ledger from lgl_tmp_tmisdata a " +
                         "left join ocs_mst_tinstitution c on c.urn = a.urn " +
                         " where a.urn in ('" + institution_urn.Replace(",", "','") + "')" +
                        " union " +
                        ...
                        " END as ac_status, a.Uncollected_Interest,    " +
                        "  (format((a.ledger * -1),2,'en_IN')) as ledger from lgl_tmp_tmisdata a " +
                        " left join ocs_mst_tcontact c on c.urn = a.urn " +
                        " where a.urn in ('" + individual_urn.Replace(",", "','") + "')" +
                        " union " +
                         "select a.customer_name, '-' as stakeholder_type, 'Group' as applicant_type, a.urn, a.account_no,  (format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt," +
                        " CASE " +
                        ...
                        "  WHEN a.ac_status = '3' THEN ' Frozen' " +
                         "END as ac_status, a.Uncollected_Interest,    " +
                         " (format((a.ledger * -1),2,'en_IN')) as ledger from lgl_tmp_tmisdata a " +
                         "left join ocs_mst_tgroup c on c.group_urn = a.urn " +
                         " where a.urn in ('" + group_urn.Replace(",", "','") + "')";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaGetNOCView(string application_gid, MdlMstApplicationView values)
        {
            ...
            try
            {
                msSQL = "select a.customer_name, c.stakeholder_type, 'Institution' as applicant_type, a.urn, a.account_no,(format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt ," +
                        ...
                        "  (format((a.ledger * -1),2,'en_IN')) as ledger from lgl_tmp_tmisdata a " +
                         "left join ocs_trn_tcadinstitution c on c.urn = a.urn " +
                         " where a.urn in ('" + institution_urn.Replace(",", "','") + "')" +
                        " union " +
                        " select a.customer_name, c.stakeholder_type,  'Individual' as applicant_type,a.urn, a.account_no,(format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt,  " +
                        " CASE " +
                        ...
                        " END as ac_status, a.Uncollected_Interest,    " +
                        "  (format((a.ledger * -1),2,'en_IN')) as ledger from lgl_tmp_tmisdata a " +
                        " left join ocs_trn_tcadcontact c on c.urn = a.urn " +
                        " where a.urn in ('" + individual_urn.Replace(",", "','") + "')" +
                        " union " +
                         "select a.customer_name, '-' as stakeholder_type, 'Group' as applicant_type, a.urn, a.account_no, (format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt , " +
                        " CASE " +
                        ...
                        "  WHEN a.ac_status = '3' THEN ' Frozen' " +
                         "END as ac_status, a.Uncollected_Interest,    " +
                         " (format((a.ledger * -1),2,'en_IN')) as ledger from lgl_tmp_tmisdata a " +
                         "left join ocs_trn_tcadgroup c on c.group_urn = a.urn " +
                         " where a.urn in ('" + group_urn.Replace(",", "','") + "')";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var tds_List = new List<alldatamodified_List>();
                if (dt_datatable.Rows.Count != 0)
        ...
        public void DaGetNDCView(string application_gid, MdlMstApplicationView values)
        {
            ...
            try
            {
                msSQL = "select a.customer_name, c.stakeholder_type, 'Institution' as applicant_type, a.urn, a.account_no,(format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt, " +
                        ...
                        "  (format((a.ledger * -1),2,'en_IN')) as ledger from lgl_tmp_tmisdata a " +
                         "left join ocs_trn_tcadinstitution c on c.urn = a.urn " +
                         "left join ocs_trn_tcadapplication b on b.application_gid = c.application_gid " +
                         " where a.urn in ('" + institution_urn.Replace(",", "','") + "')" +
                        " union " +
                        " select a.customer_name, c.stakeholder_type,  'Individual' as applicant_type,a.urn, a.account_no, (format((a.Net_Payoff_Amt ),2,'en_IN')) as Net_Payoff_Amt  ," +
                        " CASE " +
        ...
        {
            msSQL = " select a.group_gid,a.group2bank_gid,ifsc_code,bank_accountno,accountholder_name,bank_name,bank_branch,group_name from ocs_mst_tgroup2bank a " +
                    " left join ocs_mst_tgroup b on a.group_gid = b.group_gid " +
                    " left join ocs_mst_tapplication c on c.application_gid = b.application_gid " +
