(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditFsaDetailAddController', MstCreditFsaDetailAddController);

        MstCreditFsaDetailAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','$sce', '$resource'];

    function MstCreditFsaDetailAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, $resource) {
        ...
        const lspagename = 'MstCreditFsaDetailAdd';
        ...
        $scope.uploadExcel_balancesheet1 = function () {
            if ($scope.fileinputvalue == '' || $scope.fileinputvalue == undefined || $scope.fileinputvalue == null) {
                ...
                var url = 'api/MstAppCreditUnderWriting/ImportExcelBalanceSheetTemplate1';
                lockUI();
        ...
        $scope.uploadExcel_balancesheet2 = function () {
            if ($scope.fileinputvalue == '' || $scope.fileinputvalue == undefined || $scope.fileinputvalue == null) {
                ...
                var url = 'api/MstAppCreditUnderWriting/ImportExcelBalanceSheetTemplate2';
                lockUI();
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
        ...
        $scope.uploadexcel_pltemp1 = function () {
            if ($scope.fileinputpathPL == '' || $scope.fileinputpathPL == undefined || $scope.fileinputpathPL == null) {
                ...
                var url = 'api/MstAppCreditUnderWriting/ImportProfitLoss';
                lockUI();
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
        ...
        if ($scope.fileinputvaluePL2 == '' || $scope.fileinputvaluePL2 == undefined || $scope.fileinputvaluePL2 == null) {
            ...
            var url = 'api/MstAppCreditUnderWriting/ImportProfitLossTemp2';
            lockUI();
            SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
        ...
        $scope.uploadExcel_summarytemp1 = function () {
            if ($scope.fileinputvalue == '' || $scope.fileinputvalue == undefined || $scope.fileinputvalue == null) {
                ...
                var url = 'api/MstAppCreditUnderWriting/ImportExcelSummaryTemplate1';
                lockUI();
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
        ...
        $scope.uploadExcel_summarytemp2 = function () {
            if ($scope.fileinputvalue == '' || $scope.fileinputvalue == undefined || $scope.fileinputvalue == null) {
                ...
                var url = 'api/MstAppCreditUnderWriting/ImportExcelSummaryTemplate2';
                lockUI();
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
        ...
        $scope.view = function (application_gid, credit_gid, template_name) {
            $location.url('app/MstCreditFSAView?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&template_name=' + template_name + '&lspage=' + lspage + '&lspagename=' + lspagename);
        }
