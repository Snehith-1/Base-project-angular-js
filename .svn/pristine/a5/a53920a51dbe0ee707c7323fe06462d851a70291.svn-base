(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnCCRejectedApplicationsController', AgrTrnCCRejectedApplicationsController);

        AgrTrnCCRejectedApplicationsController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrTrnCCRejectedApplicationsController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnCCRejectedApplicationsController';

        activate();
        lockUI();
        function activate() {
            var url = 'api/AgrTrnCAD/GetCCRejectedSummary';
        ...
        $scope.view = function (val) {
            $location.url('app/AgrMstCadApplicationView?application_gid=' + val + '&lspage=CCRejectedApplications');
        }
