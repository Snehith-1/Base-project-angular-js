...
(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstApplcreationBasicdtlEditController', AgrMstApplcreationBasicdtlEditController);

        AgrMstApplcreationBasicdtlEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrMstApplcreationBasicdtlEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstApplcreationBasicdtlEditController';
        ...
        $scope.renewal_flag = $location.search().renewal_flag;
        var renewal_flag = renewal_flag;
        $scope.amendment_flag = $location.search().amendment_flag;
        var amendment_flag = amendment_flag;
        activate();
        function activate() {
            ...
            $scope.onboarding_disable = true;
            $scope.onboarding_enable = false;
            ...
            var url = 'api/AgrMstApplicationEdit/EditAppBasicDetail';
            unlockUI();
            SocketService.getparams(url, param).then(function (resp) {
                ...
                if ($scope.rdbOnboarding=='Proposal') {
                    $scope.RenewalOnboarding = 'Credit';
                }
                else {
                    $scope.RenewalOnboarding = 'Advance';
                }
            ...
            var url = 'api/AgrMstApplicationEdit/GetAppMobileNoList';
            SocketService.getparams(url, param).then(function (resp) {
                $scope.mstmobileno_list = resp.data.mstmobileno_list;
            });

            var url = 'api/AgrMstApplicationEdit/GetAppEmailAddressList';
            SocketService.getparams(url, param).then(function (resp) {
                $scope.mstemailaddress_list = resp.data.mstemailaddress_list;
            });

            var url = 'api/AgrMstApplicationEdit/GetAppGeneticCodeList';
            SocketService.getparams(url, param).then(function (resp) {
            ...
            var url = 'api/AgrMstApplicationEdit/GetAppProductList';
            SocketService.getparams(url, param).then(function (resp) {
        ...
        $scope.mobileno_add = function () {
            ...
            if (($scope.txtmobile_no == undefined) || ($scope.txtmobile_no == '') || ($scope.rdbprimary_mobileno == undefined) || ($scope.rdbwhatsapp_mobileno == undefined) || ($scope.rdbprimary_mobileno == '') || ($scope.rdbwhatsapp_mobileno == '')) {
                ...
                var url = 'api/AgrMstApplicationEdit/PostAppMobileNo';
                lockUI();
        ...
        function mobileno_templist() {
            ...
            var url = 'api/AgrMstApplicationEdit/GetAppMobileNoTempList';
            SocketService.getparams(url, param).then(function (resp) {
                $scope.mstmobileno_list = resp.data.mstmobileno_list;
            });
        }
        ...
        $scope.mobileno_edit = function (application2contact_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/AgrMstApplicationEdit/EditAppMobileNo';
                SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.update_mobileno = function () {
                    ...
                    var url = 'api/AgrMstApplicationEdit/UpdateAppMobileNo';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.mobileno_delete = function (application2contact_gid) {
            ...
            var url = 'api/AgrMstApplicationEdit/DeleteAppMobileNo';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.emailaddress_add = function () {
            ...
            if (($scope.txtemail_address == undefined) || ($scope.txtemail_address == '') || ($scope.rdbprimary_emailaddress == undefined) || ($scope.rdbprimary_emailaddress == '')) {
                ...
                var url = 'api/AgrMstApplicationEdit/PostAppEmailAddress';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        function emailaddress_templist() {
            ...
            var url = 'api/AgrMstApplicationEdit/GetAppEmailAddressTempList';
            SocketService.getparams(url, param).then(function (resp) {
                $scope.mstemailaddress_list = resp.data.mstemailaddress_list;
            });
        }
        ...
        $scope.emailaddress_edit = function (application2email_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/AgrMstApplicationEdit/EditAppEmailAddress';
                SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.update_emailaddress = function () {
                    ...
                    var url = 'api/AgrMstApplicationEdit/UpdateAppEmailAddress';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.emailaddress_delete = function (application2email_gid) {
            ...
            var url = 'api/AgrMstApplicationEdit/DeleteAppEmailAddress';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.geneticcode_add = function () {
            if (($scope.cboGeneticCode.geneticcode_name == '') || ($scope.cboGeneticCode.geneticcode_name == undefined) || ($scope.rdbgenetic_status == undefined) || ($scope.txtgenetic_remarks == '')) {
                ...
                var url = 'api/AgrMstApplicationEdit/PostAppGeneticCode';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        function geneticcode_templist() {
            ...
            var url = 'api/AgrMstApplicationEdit/GetAppGeneticCodeTempList';
            SocketService.getparams(url, param).then(function (resp) {
                $scope.mstgeneticcode_list = resp.data.mstgeneticcode_list;
            });
        }
        ...
        $scope.geneticcode_edit = function (application2geneticcode_gid, application_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/AgrMstApplicationEdit/EditAppGeneticCode';
                SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.update_geneticcode = function () {
                    ...
                    var url = 'api/AgrMstApplicationEdit/UpdateAppGeneticCode';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.geneticcode_delete = function (application2geneticcode_gid) {
            ...
            var url = 'api/AgrMstApplicationEdit/DeleteAppGeneticCode';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.basicdetail_update = function () {
            ...
            var url = 'api/AgrMstApplicationEdit/UpdateAppBasicDetail';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.Save_generaldetails = function () {
            ...
            var url = 'api/AgrMstApplicationAdd/SaveGeneralDtl';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.Submitgeneraldetails = function () {
            ...
            var url = 'api/AgrMstApplicationAdd/SubmitGeneralDtl';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.generdtl_Back = function () {
            if (lstab == 'add') {
                ...
                $location.url('app/AgrMstApplicationGeneralEdit?application_gid=' + application_gid);
            }
        }
        ...
        $scope.productdtl_add = function () {
            if (($scope.cboproduct_name == undefined) || ($scope.cboproduct_name == '') || ($scope.cboproduct_name == null) ||
               ($scope.cbovariety_name == undefined) || ($scope.cbovariety_name == undefined) || ($scope.cbovariety_name == '')) {
                ...
                var url = 'api/AgrMstApplicationAdd/PostProductDetailAdd';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        function Tempproductdetaillist() {
            ...
            var url = 'api/AgrMstApplicationEdit/GetAppProductTempList';
            SocketService.getparams(url, params).then(function (resp) {
                $scope.mstproduct_list = resp.data.mstproduct_list;
            });
        }
