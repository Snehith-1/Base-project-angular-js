(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstBureauUpdateInstitutionController', AgrMstBureauUpdateInstitutionController);

    AgrMstBureauUpdateInstitutionController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function AgrMstBureauUpdateInstitutionController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstBureauUpdateInstitutionController';
        ...
        $scope.bureau_edit = function (institution2bureau_gid, application_gid) {
            $location.url('app/AgrMstApplcreationCICUploadInstEdit?lsinstitution2bureau_gid=' + institution2bureau_gid + '&lsinstitution_gid=' + $scope.institution_gid + '&lsapplication_gid=' + application_gid + '&lstab=' + lstab + '&lsstatus=' + lsstatus);
        }

        $scope.bureau_view = function (institution2bureau_gid, application_gid) {
            $location.url('app/AgrMstApplcreationCICUploadInstView?lsinstitution2bureau_gid=' + institution2bureau_gid + '&lsinstitution_gid=' + $scope.institution_gid + '&lsapplication_gid=' + application_gid + '&lstab=' + lstab + '&lsstatus=' + lsstatus);
        }

        $scope.CICUploads_Back = function (application_gid) {
            if (tab == 'edit') {
                $location.url('app/AgrMstApplicationEditCICUploadAdd?lsapplication_gid=' + application_gid + '&lstab=' + tab + '&lsstatus=' + status);
            }
            else if (tab == 'add') {
                $location.url('app/AgrMstApplicationCICUploadAdd?lsapplication_gid=' + application_gid + '&lstab=' + tab + '&lsstatus=' + status );
            }
            else {
                $location.url('app/AgrMstApplicationGeneralEdit?lsapplication_gid=' + application_gid + '&lstab=' + tab + '&lsstatus=' + status);
            }
        }
