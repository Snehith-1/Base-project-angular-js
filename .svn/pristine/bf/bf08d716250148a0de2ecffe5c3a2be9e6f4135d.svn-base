(function () {
    ...
    angular
        .module('angle')
        .controller('MstCCMeetingScheduleController', MstCCMeetingScheduleController);

    MstCCMeetingScheduleController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', '$sce','SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstCCMeetingScheduleController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http,$sce,SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCCMeetingScheduleController';
        ...
        function activate() {
           ...
           var application_gid = $location.search().application_gid;
            ...
            lockUI();
            var url = 'api/MstCC/GetApplicantInfo';
        ...
        $scope.onchangeccgroup=function()
        {
            ...
            var url = 'api/MstCC/Getcc2member';
            SocketService.post(url, params).then(function (resp) {
                $scope.ccmember_list = resp.data.ccmember_list;
            });
            var url = 'api/MstCC/OtherEmployee';
            SocketService.post(url, params).then(function (resp) {
        ...
        $scope.schedule = function () {
            if ((($scope.txtstart_time != undefined) || ($scope.txtend_time != undefined)) && ($scope.cboccadmin_name == undefined)) {
                ...
                if (($scope.txtstart_time == undefined)|| ($scope.txtend_time == undefined))
                {
                    ...
                    var url = 'api/MstCC/PostScheduleMeeting';
        ...
        $scope.reschedule = function (application_gid) {
            $location.url('app/MstCCMeetingReschedule?application_gid=' + application_gid + '&ccschedulemeeting_gid=' + $scope.ccschedulemeeting_gid);
        }
        ...
        $scope.cancel = function (application_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submit = function () {
                    ...
                    var url = 'api/MstCC/CancelMeeting';
                    SocketService.post(url, params).then(function (resp) {
        ...
        $scope.pastdatecheck = function (val) {
            ...
            var url = 'api/MstCC/PastDateCheck';
            SocketService.getparams(url, params).then(function (resp) {
