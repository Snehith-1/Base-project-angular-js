...
namespace ems.master.DataAccess
{
    public class DaMstTemplate
    {
        ...
        public void DaGetTemplateSummary(MdlTemplateDtlsList values,string vertical_gid,string template_type)
        {
            try
            {
                msSQL = " select a.template_gid as template_gid,a.api_code ,a.template_name as template_name,program_name, " +
        ...
        public void DaPostTemplateDtl(string user_gid, MdlTemplateDtls values)
        {
            try
            {
                foreach (var z in values.template_programlist)
                {
                    ...
                    msGetAPICode = objcmnfunctions.GetApiMasterGID("TEMP");
                    ...
                    foreach (var i in values.templateinputtype_list)
                    {
                        ...
                        if (lsdbfield == "Y")
                        {
                            ...
                            msSQL = " insert into ocs_mst_ttemplatedbinputdtl (templatedbinputdtl_gid,fieldmapping_gid, template_gid, " +
                                    ...
                                    " from ocs_mst_tfieldmapping where fieldmapping_gid = '" + i.DBfield_type + "')";
                            mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                        }
        ...
        public void DaGetVerticalProgramTemplate(string application_gid, MdlTemplatelist values)
        {
            try
            {
        ...
        public void DaPostTrnTemplateConfirm(MdlTemplateGeneratedtl values, string user_gid)
        {
            ...
            if (values.lstemplatefrom == getTemplateClass.Sanction)
            {
                msSQL = " update ocs_trn_tapplication2sanction set template_name='" + lstemplate_name + "', " +
        ...
        public void DaGetFieldMappingSummary(MdlFieldMappinglist values)
        {
            try
            {
                msSQL = " select a.fieldmapping_gid,a.api_code, a.fieldmapping_code,fieldmapping_name, " +
                        " concat(b.user_firstname,' ',b.user_lastname,' / ', b.user_code) as created_by, " +
                        " date_format(a.created_date, '%d-%m-%Y') as created_date " +
                        " from ocs_mst_tfieldmapping a " +
                        " left join adm_mst_tuser b on a.created_by = b.user_gid " + 
        ...
        public void DaPostFieldMappingDtl(string user_gid, MdlFieldMappingdtl values)
        {
            try
            {
                msGetGID = objcmnfunctions.GetMasterGID("FMPG");
                msGetAPICode = objcmnfunctions.GetApiMasterGID("FIEM");
                string msGetCode = objcmnfunctions.GetMasterGID("FMC");

                msSQL = "INSERT INTO ocs_mst_tfieldmapping(" +
                        " fieldmapping_gid," +
                        " api_code," +
                        " fieldmapping_code," +
                        " fieldmapping_name, " +
