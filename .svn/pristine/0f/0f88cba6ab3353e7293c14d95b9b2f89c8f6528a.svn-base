(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditSuppliersDtlAddController', AgrTrnSuprCreditSuppliersDtlAddController);

    AgrTrnSuprCreditSuppliersDtlAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce','cmnfunctionService'];

    function AgrTrnSuprCreditSuppliersDtlAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditSuppliersDtlAddController';
        ...
        $scope.add_creditsupplierdtl = function () {
            ...
            ($scope.txtBankDebitamt == '') || ($scope.txtrelationship_supplier == '')
                ...
                var url = 'api/AgrTrnSuprAppCreditUnderWriting/PostCreditSupplier';
                lockUI();
        ...
        $scope.creditsupplier_delete = function (creditsupplier_gid) {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/DeleteCreditSupplier';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    $scope.institutionsupplier_list = resp.data.supplier_list;
        ...
        $scope.creditsupplier_edit = function (creditsupplier_gid) {
            $location.url('app/AgrTrnSuprCreditSuppliersDtlEdit?application_gid=' + application_gid + '&creditsupplier_gid=' + creditsupplier_gid + '&institution_gid=' + institution_gid + '&lspage=' + lspage);
        }
