...
{
    public class DaLglTrnDNTrackerOthers
    {
        ...
        int mnResult;
        public bool DaGetOthersPendingList(string employee_gid, MdlDNpendingList values)
        {
            msSQL = " (select max(od_days) as max_od_days,urn,acknowledgement_status,a.Customer_name,GurantorCustomerId,Guarantor_Name,Vertical,DN_status" +
                    " from lgl_trn_tmisdata a where Vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail') and natureof_credit_amount >= 5000 " +
                    ...
                    " vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail') and natureof_credit_amount >= 5000 and " +
        ...
        public bool DaGetOthersGeneratedList(string employee_gid, MdlDNGeneratedList values)
        {
            msSQL = " select max(od_days) as max_od_days,urn,acknowledgement_status,Customer_name,GurantorCustomerId,Guarantor_Name,Vertical,DN_status from lgl_trn_tmisdata  " +
                    " where Vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail') " +
                    " and natureof_credit_amount >=5000 and dn_status not in ('Pending','Legal SR','DN Skip') and"+
                    " exclusion_flag = 'N' group by urn order by od_days desc";
            dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public bool DaGetOthersSkippedList(string employee_gid, MdlDNSkippedList values)
        {
            msSQL = " select max(od_days) as max_od_days,a.urn,Customer_name,GurantorCustomerId,Guarantor_Name,Vertical," +
                    ...
                    " left join lgl_tmp_tmisdata b on a.urn = b.urn where  Vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail') " +
        ...
        public bool DaGetOthersExclusionList(string employee_gid, MdlDNexclusionList values)
        {
            msSQL = "select account_no,count(urn),max(od_days) as max_od_days,urn,acknowledgement_status," +
                      ...
                      " Vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail') " +
        ...
        public bool DaGetOthersLegalSRList(MdlDNlegalsrList values, string employee_gid)
        {
            ...
            msSQL = " select a.customer_gid, a.templegalsr_gid,d.email,d.mobileno,a.auth_status,a.approval_status," +
                    ...
                    " where a.created_by='" + employee_gid + "' and vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail')" +
        ...
        public bool DaGetOthers_Count(string employee_gid, count_dtl values)
        {
            ...
            if(lspending_count==""||lspending_count==null)
            {
                values.lblpending_count = "0";
            }
            else
            {
            ...
            msSQL = " select count( distinct a.urn) " +
                    " from lgl_trn_tdnskipcase a"+
                    " left join lgl_tmp_tmisdata b on a.urn = b.urn where  "+
                    " Vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail')";
            ...
            msSQL = "select count(distinct a.urn) from lgl_trn_tmisdata a" +
                      " left join hrm_mst_temployee b on a.exclusion_updatedby = b.employee_gid" +
                      " left join adm_mst_tuser c on b.user_gid = c.user_gid where  Vertical not in ('AE','Agri Enterprises','Agri Enterprise','AgriEnterprises','FPO','CBO','Retail')" +
