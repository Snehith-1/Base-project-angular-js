...
namespace ems.idas.DataAccess
{
    public class DaTrnCourierAck
    {
        ...
        public void DaGetAcknowledgeForm(string courierMgmt_gid, CourierMgmt values)
        {
            try
            {
                ...
                msSQL = " SELECT courierref_no, couriermgmt_gid,date_format(date_of_courier,'%d-%m-%Y') as date_of_courier," +
                        ...
                        " FROM "+ lscompany_name + ".ids_trn_tcouriermgnt" +
        ...
        public result  DaPostAckStatus(MdlCourierAck values)
        {
            lscompany_name = ConfigurationManager.AppSettings[HttpContext.Current.Request.Url.Host].ToString();
            result objResult = new Models.result();
          
            msSQL = " UPDATE " + lscompany_name + ".ids_trn_tcouriermgnt SET" +
