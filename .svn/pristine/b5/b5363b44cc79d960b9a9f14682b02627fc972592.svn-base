(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnCreditBankStatementAnalysisAddController', AgrTrnCreditBankStatementAnalysisAddController);

        AgrTrnCreditBankStatementAnalysisAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','$sce', '$resource','DownloaddocumentService','cmnfunctionService'];

    function AgrTrnCreditBankStatementAnalysisAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, $resource,DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnCreditBankStatementAnalysisAddController';
        ...
        $scope.upload = function (val, val1, name) {
            ...
            if (!allowedExtensions.exec(filePath)) {
                ...
                if (item.name.includes("Bank Statement Analysis")) {
