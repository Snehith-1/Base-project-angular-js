(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstSuprCcCommitteeApplicationViewController', AgrMstSuprCcCommitteeApplicationViewController);

    AgrMstSuprCcCommitteeApplicationViewController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', '$sce', '$anchorScroll', 'DownloaddocumentService'];

    function AgrMstSuprCcCommitteeApplicationViewController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, $sce, $anchorScroll, DownloaddocumentService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstSuprCcCommitteeApplicationViewController';
        ...
        $scope.Back = function () {
            if (lspage == 'ScheduleMeeting') {
                ...
                $state.go('app.AgrMstSuprSentbackcctoCredit');
            }
            else if (lstab == 'CCApproved') {
                $state.go('app.AgrMstSuprCCApprovedSummary');
            }
            else {
        ...
        $scope.Kyc_view = function () {
            var application_gid = $scope.application_gid;
            $location.url('app/AgrMstSuprCcCommitteeKycView?application_gid=' + application_gid + '&lstab=' + lstab);
        }
