(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditInstitutionBureauViewController', MstCreditInstitutionBureauViewController);

    MstCreditInstitutionBureauViewController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService', 'cmnfunctionService'];

    function MstCreditInstitutionBureauViewController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditInstitutionBureauViewController';
        ...
        $scope.CICUploads_Back = function () {
            if(lspagename=='MstCreditInstitutionDtlAdd')
                ...
                $location.url('app/MstCreditCompanyDtlView?application_gid=' + $scope.application_gid + '&institution_gid=' + $scope.institution_gid + '&lspage=' + $scope.lspage +'&lspagetype='+ lspagetype);
        
            else if(lspagename=='MstCcCommitteeInstitutionView')
                $location.url('app/MstCcCommitteeInstitutionView?application_gid=' + $scope.application_gid + '&institution_gid=' + $scope.institution_gid + '&lspage=' + $scope.lspage +'&lspagetype='+ lspagetype);
