(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAOnboardingApprovalIndSummaryController', MstSAOnboardingApprovalIndSummaryController);

    MstSAOnboardingApprovalIndSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstSAOnboardingApprovalIndSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
    /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAOnboardingApprovalIndSummaryController';

        activate();

        function activate() { 
            var url = 'api/MstSAOnboardingBussDevtVerification/GetSaApprovalInitiateSummary';
            lockUI();
        ...
        $scope.saonboardingverification = function (sacontact_gid) {
            $location.url('app/MstSAOnboardingIndividualApprovalView?hash=' + cmnfunctionService.encryptURL('lssacontact_gid=' + sacontact_gid + '&lspage=IndividualInitiate'));
        }
    }
})();
