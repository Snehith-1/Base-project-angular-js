(function () {
    ...
    angular
        .module('angle')
        .controller('MstWorkInprogressCallSummaryController', MstWorkInprogressCallSummaryController);

    MstWorkInprogressCallSummaryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstWorkInprogressCallSummaryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstWorkInprogressCallSummaryController';

        activate();

        function activate() {
            var url = 'api/TeleCalling/GetEmpInProgressIBCallSummary';
        ...
        $scope.view = function (inboundcall_gid) {
            $location.url("app/MstTeleCallingAssignView?hash=" + cmnfunctionService.encryptURL("inboundcall_gid=" + inboundcall_gid));
        }
        ...
        $scope.acknowledge = function (inboundcall_gid) {
            $location.url("app/MstAssignedCalls?hash=" + cmnfunctionService.encryptURL("inboundcall_gid=" + inboundcall_gid));
        }
