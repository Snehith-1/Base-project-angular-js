(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstProducDesctVerificationController', AgrMstProducDesctVerificationController);

    AgrMstProducDesctVerificationController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function AgrMstProducDesctVerificationController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, DownloaddocumentService,cmnfunctionService) {

        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstProducDesctVerificationController'; 
        ...
        function activate() {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.assign_remarks = resp.data.assign_remarks;
            });
            ...
            var url = 'api/AgrTrnProductApproval/GetApprmquerysSummary';
            SocketService.getparams(url, params).then(function (resp) {
                lockUI();
        ...
        $scope.Back = function () {
            if (lspage == "ProductDescMyAssignment") {
        ...
        $scope.CAM_generate = function () {
            ...
            var application_gid = $scope.application_gid;
            $location.url('app/AgrTrnCAMGenerate?application_gid=' + application_gid);
        }
        ...
        $scope.submitto_approval = function () {
            ...
            var url = 'api/AgrTrnProductApproval/Getappproductmemberapproval';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.creditinstitution_add = function (institution_gid) {
            $location.url('app/AgrTrnDocumentCheckList?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=StartCreditUnderwriting&lstype=Institution');
        }

        $scope.creditindividual_add = function (contact_gid) {
            $location.url('app/AgrTrnIndividualDocCheckList?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&lspage=StartCreditUnderwriting&lstype=Individual');
        }

        $scope.creditgroup_add = function (group_gid) {
            $location.url('app/AgrTrnGroupDocCheckList?application_gid=' + application_gid + '&group_gid=' + group_gid + '&lspage=StartCreditUnderwriting&lstype=Group');
        }
        ...
        $scope.creditgeneraldtl_edit = function (product_gid, variety_gid) {
            $location.url('app/AgrTrnCreditGeneralDtlEdit?application_gid=' + application_gid + '&lspage=StartCreditUnderwriting' + '&product_gid=' + product_gid + '&variety_gid=' + variety_gid);
        }

        $scope.applcreainstitution_edit = function (institution_gid) {
            $location.url('app/AgrTrnCreditInstitutionDtlEdit?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=StartCreditUnderwriting');
        }

        $scope.appcreaindividual_edit = function (contact_gid) {
            $location.url('app/AgrTrnCreditIndividualDtlEdit?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&lspage=StartCreditUnderwriting');
        }

        $scope.appcreagroup_edit = function (group_gid) {
            $location.url('app/AgrTrnCreditGroupDtlEdit?application_gid=' + application_gid + '&group_gid=' + group_gid + '&lspage=StartCreditUnderwriting');
        }
        ...
        $scope.productcharges_edit = function (product_gid, variety_gid) {
            $location.url('app/AgrTrnCreditProductChargesDtlEdit?application_gid=' + application_gid + '&lspage=StartCreditUnderwriting' + '&product_gid=' + product_gid + '&variety_gid=' + variety_gid);
        }

        $scope.hypothecation_edit = function () {
            $location.url('app/AgrTrnCreditHypothecationEdit?application_gid=' + application_gid + '&lspage=StartCreditUnderwriting');
        }
        ...
        $scope.create_Query = function () {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.query_add = function () {
                    ...
                    var url = 'api/AgrTrnProductApproval/PostAppProductqueryadd';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
