(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAOnboardingInstitutionEditController', MstSAOnboardingInstitutionEditController);

    MstSAOnboardingInstitutionEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstSAOnboardingInstitutionEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
    /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAOnboardingInstitutionEditController';
       ...
       $scope.sacontactinstitution_gid = searchObject.lssacontactinstitution_gid;
        ...
        function activate() {  
          ...
          vm.format = vm.formats[0];
            ...
            SocketService.getparams(url, param).then(function (resp) {
                   ...
                   if($scope.txtannual_turnover!=null && $scope.txtannual_turnover!=undefined && $scope.txtannual_turnover!="")                   {   
                       $scope.txtannual_income_edit = (parseInt($scope.txtannual_turnover.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');   
        ...
        $scope.queryclose = function (bdinstitutionraisequery_gid) {
            var modalInstance = $modal.open({
                templateUrl: '/bdinstitutionqueryClose.html',
                controller: ModalInstanceCtrl,
                backdrop: 'static',
                keyboard: false,
                size: 'lg'
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submit = function () {
                    var params = {
                        bdinstitutionraisequery_gid: bdinstitutionraisequery_gid,
                        queryresponse_remarks: $scope.txtcloseremarks,
                        sacontactinstitution_gid: sacontactinstitution_gid
                    }
                    var url = 'api/MstSAOnboardingBussDevtVerification/PostBDInstitutionresponsequery';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.saveasdraft_company = function () {
            ...
            if ($scope.cboDesignation != undefined || $scope.cboDesignation != null) {
              ...
              lsdesignation_type = $('#designation_type :selected').text();
            }
           // if ($scope.cbosatype != undefined || $scope.cbosatype != null) {
            ...
            if ($scope.cbosaentitytype != undefined || $scope.cbosaentitytype != null) {
               ...
               lssaentitytype_name = $('#saentitytype_name :selected').text();
            }
             lssatype_name = $('#satype_name :selected').text();
            ...
            //    lsassessmentagency_name = $scope.cboassessmentagencyrating.assessmentagencyrating_name;
            //}
            ...
            console.log(params);
            var url = 'api/MstSAOnboardingBussDevtVerification/InstitutionEditSaveAsDraft';
        ...
        $scope.uploaddocumentcancel = function (institutionsabureaudocumentupload_gid) {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                ...
                var param = {
                    institutionsabureaudocumentupload_gid: $scope.institutionsabureaudocumentupload_gid
                };
                var url = 'api/MstSAOnboardingInstitution/SAUploadIndividualDocList';
                SocketService.getparams(url, param).then(function (resp) {
                    $scope.lrfilename = resp.data.filename;                    
        ...
        $scope.update_company =function () {
            ...
            var params = {               
                ...
                sa_appcrediteddate: $scope.txtcredited_date,
                sa_appcreditedamount: $scope.txtcredited_amount,
                rm_tagging_id: $scope.cborm,
                rm_tagging_name: empname,                
        ...
        $scope.update_company_buss = function () {
            //var lsassessmentagency_gid = '';
            //var lsassessmentagency_name = '';
            //var lsassessmentagencyrating_gid = '';
            //var lsassessmentagencyrating_name = '';
            ...
            console.log(params);
            //var url = 'api/MstSAOnboardingInstitution/CompanyEditUpdate';
            var url = 'api/MstSAOnboardingBussDevtVerification/CompanyBussDevtVerificationupdateRaise';
