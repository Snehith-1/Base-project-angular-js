(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstContractEditController', AgrMstContractEditController);

    AgrMstContractEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function AgrMstContractEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstContractEditController';
        const lsdynamiclimitmanagementback = 'AgrMstContractEdit';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnContract/ContractTempClear';
        ...
        function refreshloandetails() {
            ...
            var url = 'api/AgrTrnContract/GetProductChargesDtl';
       ...
       // }
        ...
        $scope.Back = function (val) {
            ...
            if (lspage == 'ContractMaker') {
               // $state.go('app.AgrMstContractDtlSummary');
       ...
       //                 }
       //                 lockUI();
       //                 var url = 'api/AgrTrnCad/Postlimitproductinfo';
       //                 SocketService.post(url, params).then(function (resp) {
       //                     if (resp.data.status == true) {
       //                         Notify.alert(resp.data.message, {
       //                             status: 'success',
       ...
       // //Sanction Submit Event
        $scope.ContractUpdate = function () {
          // if ($scope.productdetails != null && $scope.loandtls_list.length == $scope.productdetails.length) {*/
                var params = {
                    contract_date: $scope.txtcontract_date,  
                    application2sanction_gid: application2sanction_gid,
                    application_gid: application_gid
                   
                }
            var url = 'api/AgrTrnContract/UpdateContractDetail';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    if (resp.data.status == true) {
                        unlockUI();
        ...
        $scope.onboardappdetailinfo = function (onboard_gid) {
            $location.url('app/AgrMstOnboardingApplicationInfo?hash=' + cmnfunctionService.encryptURL('onboard_gid=' + onboard_gid + '&application_gid=' + application_gid + '&application2sanction_gid=' + application2sanction_gid + '&selectedIndex=' + $scope.selectedIndex + '&lsdynamiclimitmanagementback=' + lsdynamiclimitmanagementback + '&lspage=' + lspage + '&lsparent=AppRMView'));
        }
