...
{
    /// <summary>
    ///To upload AXIS Statement,It digitize with LMS trnascation to get matched/Nonmatched cases
    /// </summary>
    /// <remarks>Written by Motches</remarks>
    public class DaAxisReconcillation
    {
        string bankrepaytransc_gid;
        cmnfunctions objcmnfunctions = new cmnfunctions();
        Fnazurestorage objcmnstorage = new Fnazurestorage();
        DataTable dt_datatable;
        string msSQL, msGetGid, msGetUTRGid, msGetGid1, lsbrs_gid,lsbanktransac_name, msGetGid2, lsbanktrans_gid
;
        OdbcDataReader objODBCDatareader;
        dbconn objdbconn = new dbconn();
        HttpPostedFile httpPostedFile;
        string lspath, endRange, excelRange;
        int mnresult;
        int rowCount, columnCount;
        string lsbankconfig_gidrange, lsrepayment_amount,lsaccount_number,lsurn_no, lstransactionid, lstransaction_date,lsrepayment_date, lstransc_idrange, lstrn_daterange,
        ...
        string lstrn_date,lspayment_date,lstrndate,lscustref_no, lsacc_no, lsbranch, lsbankremaining_amount, lsknockoff_status;
        //ExcelPackage xlPackage = new ExcelPackage();
        ExcelPackage xlPackage;
        int lstrndatastart_rowrange;
        int lstrn_datecolumn;
        int lspayment_datecolumn,lsvalue_datecolumn;
        int lstransact_particularscolumn;
        int lscheqref_column;
        int lsdebit_amtcolumn, lstransact_valcolumn;
        int lscredit_amtcolumn;
        DateTime date1;
        string lsconverted_date, lsremarks;
        decimal remaining_amount;
        int lsbalance_column, lscr_drcolumn;
        FileStream filexlsx;
        ...
        public bool DaPostBRSExcelSample(HttpRequest httpRequest, MdlAxisReconcillation objfilename, string employee_gid, string user_gid)
        {
            ...
            path = ConfigurationManager.AppSettings["file_path"] + "/erpdocument" + "/" + lscompany_code + "/" + "BRS/Axisreconcildoc/" + DateTime.Now.Year + "/" + DateTime.Now.Month;
            {
            ...
            try
            {
                if (httpRequest.Files.Count > 0)
                {
                    ...
                    for (int i = 0; i < httpFileCollection.Count; i++)
                    {
                        ...
                        bool status;
                        status = objcmnstorage.UploadStream("erpdocument", lscompany_code + "/" + "BRS/Axisreconcildoc/" + DateTime.Now.Year + "/" + DateTime.Now.Month + "/" + msdocument_gid + FileExtension, ms);

                        lspath = "erpdocument" + "/" + lscompany_code + "/" + "BRS/Axisreconcildoc/" + DateTime.Now.Year + "/" + DateTime.Now.Month + "/";
                        file.Close();
                        ms.Close();
                        msxlsx.Close();
                        mscsvxlsx.Close();
                        ...
                        string[] splits10 = lscr_drrange.Split(',');
                        string[] splits11 = lstransact_valrange.Split(',');
                        ...
                        for (int row = lstrndatastart_rowrange; row <= rowCount - 3; row++)
                        ...
                        lscr_drcolumn = Convert.ToInt32(splits10[1]);
                        lstransact_valcolumn = Convert.ToInt32(splits11[1]);
                        lsbranch_namecolumn = Convert.ToInt32(splits9[1]);
                ...
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow datarow in dt_datatable.Rows)
                    {
...

                        {
                            ...
                            msSQL = " update brs_trn_tbanktransactiondetails set knockoff_status='Matched' , rule_number ='R1',fullymatched_flag='Y', knockoff_date='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',knockoff_flag='Y',remaining_amount='0.00', bankrepaytransc_gid = '" + lsrepayment_id + "' ,closed_by= '" + user_gid + "'  " +
                              " where banktransc_gid = '" + msGetGid1 + "' ";
                            mnresult = objdbconn.ExecuteNonQuerySQL(msSQL);
                ...
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow datarow in dt_datatable.Rows)
                    {
                        ...
                        if ((convertDecimalrepaymentamount > convertDecimalbankremainingamount) && (lsknockoff_status == "Pending"))
                        {
                            ...
                            //remaining_amount = convertDecimaltransactvalremaining - convertDecimalrepaymentamountremaining;
                            if (lsrepayment_amount == null || lsrepayment_amount == "")
                            {
                                lsrepayment_amount = "0";
                            }
