(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstIndividualTabController', AgrMstIndividualTabController);

    AgrMstIndividualTabController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

        function AgrMstIndividualTabController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstIndividualTabController';
    ...
    $scope.Passportid_dob =  function (){
        ...
        $scope.individual_save = function () {
            ...
            //else
                ...
                var url = 'api/AgrMstApplicationAdd/IndividualSave';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.individual_submit = function () {
            ...
            //else {
                ...
                //else
                    ...
                    var url = 'api/AgrMstApplicationAdd/IndividualSubmit';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
