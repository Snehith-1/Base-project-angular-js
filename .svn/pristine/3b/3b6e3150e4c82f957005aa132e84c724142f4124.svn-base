...
/// <summary>
/// (It's used for Softcopy & Original copy document report pages in Samfin)CadAppDocReport DataAccess Class accessed by API methods from related Controller class and is returning relevant response to client.
/// </summary>
/// <remarks>Written by Sumala,Logapriya and Abilash</remarks>

namespace ems.master.DataAccess
{
    public class DaMstCadAppDocReport
    {
        ...
        public void DaGetScannedDefPendingReport(string lsstatus, Mdlscannedapplicationexport objvalues)
        {
            if (lsstatus == "Maker")
            {
                msSQL = "select a.application_no as 'ARN NO',  " +
                    ...
                    " left join ocs_trn_tdeferraltagdoc f on f.application_gid = a.application_gid and f.groupdocumentchecklist_gid = c.groupdocumentchecklist_gid" +
        ...
        public void DaGetPhysicalDefPendingReport(string lsstatus, Mdlscannedapplicationexport objvalues)
        {
            ...
            MemoryStream ms = new MemoryStream();
            ExcelPackage excel = new ExcelPackage(ms);
            var workSheet = excel.Workbook.Worksheets.Add("CADPhysicalDeferral-" + lsstatus + "PendingReport");
            try
            {
                msSQL = " select company_code from adm_mst_tcompany";

                lscompany_code = objdbconn.GetExecuteScalar(msSQL);
                objvalues.lsname = "CADPhysicalDeferral-" + lsstatus + "PendingReport.xlsx";
        ...
        public void DaGetScannedCovPendingReport(Mdlscannedapplicationexport objvalues)
        {
            if (objvalues.lsstatus == "Maker")
            {
                msSQL = " ";
            }
            else if (objvalues.lsstatus == "Checker")
            {
                msSQL = " ";
            }
            else
            ...
            MemoryStream ms = new MemoryStream();
            ExcelPackage excel = new ExcelPackage(ms);
            var workSheet = excel.Workbook.Worksheets.Add("CADScannedCovenant-" + objvalues.lsstatus + "PendingReport");
            try
            {
                msSQL = " select company_code from adm_mst_tcompany";

                lscompany_code = objdbconn.GetExecuteScalar(msSQL);
                objvalues.lsname = "CADScannedCovenant-" + objvalues.lsstatus + "PendingReport.xlsx";
        ...
        public void DaGetPhysicalCovPendingReport(Mdlscannedapplicationexport objvalues)
        {
            if (objvalues.lsstatus == "Maker")
            {
                msSQL = " ";
            ...
            MemoryStream ms = new MemoryStream();
            ExcelPackage excel = new ExcelPackage(ms);
            var workSheet = excel.Workbook.Worksheets.Add("CADPhysicalCovenant-" + objvalues.lsstatus + "PendingReport");
            try
            {
                msSQL = " select company_code from adm_mst_tcompany";

                lscompany_code = objdbconn.GetExecuteScalar(msSQL);
                objvalues.lsname = "CADPhysicalCovenant-" + objvalues.lsstatus + "PendingReport.xlsx";
