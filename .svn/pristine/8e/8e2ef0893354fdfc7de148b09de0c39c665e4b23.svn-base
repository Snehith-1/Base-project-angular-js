(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnPMGPhysicalDocQueryController', AgrTrnPMGPhysicalDocQueryController);

    AgrTrnPMGPhysicalDocQueryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService', '$anchorScroll', 'cmnfunctionService'];

    function AgrTrnPMGPhysicalDocQueryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, DownloaddocumentService, $anchorScroll, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnPMGPhysicalDocQueryController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnPhysicalDocument/tmpphysicalclearQueryuploaded';
                ...
                if ($scope.waiverpendingcount == 0 && $scope.checklistcount == 0 && $scope.originaluploaddocumentlist != null && $scope.originaluploaddocumentlist.length != 0) {
                    ...
                    SocketService.getparams(url, params).then(function (resp) {
                        ...
                        if ($scope.Checklist == null || $scope.Checklist == undefined) {
                            ...
                            var url = 'api/AgrTrnPhysicalDocument/GetQueryRaisedinfo';
                            SocketService.getparams(url, params).then(function (resp) {
            ...
            var url = 'api/AgrTrnPhysicalDocument/GetMakerCheckerConversation';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
...
if ($scope.txtdocumenttype_name == null || $scope.txtdocumenttype_name == undefined || $scope.txtdocumenttype_name == '' )
{
$scope.lsquery_title = $scope.txtdoccheckpoint
}

else if ($scope.txtdocumenttype_name != null || $scope.txtdocumenttype_name != undefined || $scope.txtdocumenttype_name != '' )
{
    $scope.lsquery_title = $scope.txtdocumenttype_name + '-' + $scope.txtdoccheckpoint
}

            var params = {
                // query_title: $scope.txtdocumenttype_name + '-' + $scope.txtdoccheckpoint,
                query_title: $scope.lsquery_title,
            ...
            var url = 'api/AgrTrnPhysicalDocument/PostAppcadPhysicalqueryadd';
            lockUI();
        ...
        $scope.QueryDocumentUpload = function (val, val1, name) {
            ...
            if ($scope.uploadfrm != undefined) {
                var url = 'api/AgrTrnPhysicalDocument/PhysicalQueryDocumentUpload';
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                    ...
                    var url = 'api/AgrTrnPhysicalDocument/GetTmpPhysicalQueryDocument';
                    SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.view_querydesc = function (tagquery_gid, query_status, query_description, query_responseremarks) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.querystatusupdate = function (query_status) {
                    ...
                    var url = 'api/AgrTrnPhysicalDocument/GetUpdateCADRaiseQueryStatus';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        if (resp.data.status == true) {
                            unlockUI();
        ...
        $scope.responseclick = function () {
            ...
            var url = 'api/AgrTrnPhysicalDocument/postPhysicalMakerCheckerConversation';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.Back = function () {
            $location.url('app/AgrTrnPMGPhysicalDochecklist?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
        ...
        $scope.updatedocumentsubmissiondate = function () {
            ...
            var url = 'api/AgrTrnPhysicalDocument/GetUpdateDocumentSubmission';
            SocketService.post(url, params).then(function (resp) {
                if (resp.data.status == true) {
                    unlockUI();
                    Notify.alert(resp.data.message, {
        ...
        $scope.raise_query = function (deferralchecklist_gid, checklist_name) {
            ...
            var url = 'api/AgrTrnPhysicalDocument/tmpclearQueryuploaded';
            SocketService.get(url).then(function (resp) {
            });
        ...
        $scope.deletependingquery = function (tagquery_gid) {
            ...
            var url = 'api/AgrTrnPhysicalDocument/GetPendingQueryDelete';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == true) {
                    unlockUI();
        ...
        $scope.documentverifiededitcheck = function (index) {
            ...
            if (documentverifychecked != null && documentverifychecked.length == $scope.deferraltaggedchecklist.length) {
                ...
                var url = 'api/AgrTrnPhysicalDocument/GetConfirmationValidation';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
