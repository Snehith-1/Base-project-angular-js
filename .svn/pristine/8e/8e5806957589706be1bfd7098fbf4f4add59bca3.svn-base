(function () {
    ...
    angular
        .module('angle')
        .controller('MstChequeApprovalDtlsController', MstChequeApprovalDtlsController);

    MstChequeApprovalDtlsController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function MstChequeApprovalDtlsController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstChequeApprovalDtlsController';
        ...
        $scope.addMaker = function () {
            $location.url('app/MstUDCMakerAdd?application_gid=' + application_gid + '&lspage=approvalpending');
        }

        $scope.editUdc = function (udcmanagement_gid, udcmanagement2cheque_gid) {
            $location.url('app/MstUDCMakerEditCheque?lsudcmanagement2cheque_gid=' + udcmanagement2cheque_gid + '&lsudcmanagement_gid=' + udcmanagement_gid + '&application_gid=' + application_gid + '&lspage=approvalpending');
        }

        $scope.viewCheque = function (udcmanagement2cheque_gid, udcmanagement_gid) {
            $location.url('app/MstUDCMakerView?lsudcmanagement2cheque_gid=' + udcmanagement2cheque_gid + '&lsudcmanagement_gid=' + udcmanagement_gid + '&application_gid=' + application_gid + '&lspage=approvalpending');
        }
        ...
        $scope.Back = function () {
            $location.url('app/MstCadChequeMgmtApprovalSummary?application_gid=' + application_gid);
        }
        ...
        $scope.approve = function () {
            if ($scope.udcmanagement_list == undefined || $scope.udcmanagement_list == "" || $scope.udcmanagement_list == "null") {
                ...
                var url = "api/UdcManagement/PostChequeApproval";
                lockUI();
