(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditAssessedScoreEditController', MstCreditAssessedScoreEditController);

        MstCreditAssessedScoreEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstCreditAssessedScoreEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditAssessedScoreEditController';
        ...
        function activate() {
            ...
            var url = 'api/MstApplicationGradingTool/GetEditGradingToolassesment';
            SocketService.getparams(url, params).then(function (resp) {
                $scope.assesment_list = resp.data.gradingtool_list;

            });
            var url = 'api/MstApplicationGradingTool/GetEditGradingTooltotal';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        function assessmentcriteria_list() {
            ...
            var url = 'api/MstApplicationGradingTool/GetEdittmpGradingToolassesment';
            SocketService.getparams(url, params).then(function (resp) {
                $scope.assesment_list = resp.data.gradingtool_list;
        ...
        $scope.save_gradingtool = function () {
            ...
            var url = 'api/MstApplicationGradingTool/SaveasEditDraftGradingToolDetails';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    ...
                    $location.url('app/MstCreditAssessedScoreAdd?application_gid=' + application_gid);
                }
                else {
                    Notify.alert(resp.data.message, {
                        status: 'warning',
        ...
        $scope.Updategrading = function () {
            ...
            var url = 'api/MstApplicationGradingTool/UpdateGradingToolDetails';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.Back = function () {
            ...
            /*    $location.url('app/MstCreditAssessedScoreAdd');*/
                $location.url('app/MstCreditAssessedScoreAdd?application_gid=' + application_gid + '&lstab=' + lstab);
            }
