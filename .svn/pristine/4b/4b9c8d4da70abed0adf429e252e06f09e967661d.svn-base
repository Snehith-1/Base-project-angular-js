(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditIndividualRepaymentAddController', AgrTrnSuprCreditIndividualRepaymentAddController);

    AgrTrnSuprCreditIndividualRepaymentAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce','cmnfunctionService'];

    function AgrTrnSuprCreditIndividualRepaymentAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditIndividualRepaymentAddController';
            ...
            ($scope.txtBalanceTenure_month == '') || ($scope.txtBalanceTenure_days == '') || ($scope.cboAcctClassification == '') || ($scope.txtOverdue_Amount == '') || ($scope.txtnoofdays_overdue == '') || ($scope.txtremarks == '')) {
                ...
                if (($scope.cboLenderType.lendertype_name == 'Bank')) {
                    if (($scope.txtifsc_code == undefined) || ($scope.txtBank_Name == undefined) ||
                        ...
                        var url = 'api/AgrTrnSuprAppCreditUnderWriting/PostRepaymentTrack';
                        lockUI();
        ...
        $scope.creditrepayment_delete = function (creditrepaymentdtl_gid) {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/DeleteRepaymentTrack';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.creditrepayment_edit = function (creditrepaymentdtl_gid) {
            $location.url('app/AgrTrnSuprCreditIndividualRepaymentEdit?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&creditrepaymentdtl_gid=' + creditrepaymentdtl_gid + '&lspage=' + lspage);
        }
