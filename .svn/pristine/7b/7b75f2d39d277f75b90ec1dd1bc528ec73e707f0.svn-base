(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstSuprCreditAssessedScoreEditController', AgrMstSuprCreditAssessedScoreEditController);

    AgrMstSuprCreditAssessedScoreEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrMstSuprCreditAssessedScoreEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstSuprCreditAssessedScoreEditController';
        ...
        function activate() {
            ...
            var url = 'api/AgrMstSuprApplicationGradingTool/GetEditGradingToolassesment';
            SocketService.getparams(url, params).then(function (resp) {
                $scope.assesment_list = resp.data.gradingtool_list;

            });
            var url = 'api/AgrMstSuprApplicationGradingTool/GetEditGradingTooltotal';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        function assessmentcriteria_list() {
            ...
            var url = 'api/AgrMstSuprApplicationGradingTool/GetEdittmpGradingToolassesment';
            SocketService.getparams(url, params).then(function (resp) {
                $scope.assesment_list = resp.data.gradingtool_list;
        ...
        $scope.save_gradingtool = function () {
            if ($scope.txtdateofsurvey_date == '' || $scope.txtdateofsurvey_date == null || $scope.txtdateofsurvey_date == undefined) {
                ...
                var url = 'api/AgrMstSuprApplicationGradingTool/SaveasEditDraftGradingToolDetails';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
                        ...
                        $location.url('app/AgrMstSuprCreditAssessedScoreAdd?application_gid=' + application_gid);
                    }
                    else {
                        Notify.alert(resp.data.message, {
                            status: 'warning',
        ...
        $scope.Updategrading = function () {
            ...
            var url = 'api/AgrMstSuprApplicationGradingTool/UpdateGradingToolDetails';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.Back = function () {
            $location.url('app/AgrMstSuprCreditAssessedScoreAdd?application_gid=' + application_gid + '&lstab=' + lstab);
        }
