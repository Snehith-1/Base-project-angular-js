...
(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAOnboardingInstitutionApprovalController', MstSAOnboardingInstitutionApprovalController);

    MstSAOnboardingInstitutionApprovalController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstSAOnboardingInstitutionApprovalController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
    /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAOnboardingInstitutionApprovalController';
        ...
        function activate() { 
            ...
            SocketService.getparams(url, param).then(function (resp) {
                ...
                $scope.lblnextrenewaldate = resp.data.nextrenewaldate;
        ...
        $scope.back=function(){
            ...
            if (lspage == 'InstitutePending') {
                $location.url('app/MstSAApprovalInsPendingSummary');
            }
            else if (lspage == 'InstituteInitiate') {
                $location.url('app/MstSAOnboardingApprovalInsSummary');
            }
            else if (lspage == 'Institutioncodecreate') {
                $location.url('app/MstSBACodeCreationCompletedSummary');
            }
        }
