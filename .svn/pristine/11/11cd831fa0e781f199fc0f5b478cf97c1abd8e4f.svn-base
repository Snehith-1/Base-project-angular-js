(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnCADApplicationEditController', AgrTrnCADApplicationEditController);

    AgrTrnCADApplicationEditController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', '$sce', '$anchorScroll', 'DownloaddocumentService','cmnfunctionService'];

    function AgrTrnCADApplicationEditController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, $sce, $anchorScroll, DownloaddocumentService,cmnfunctionService) {

        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnCADApplicationEditController';
        ...
        function activate() {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.document_status = resp.data.document_status;
        ...
        $scope.Back = function () {
            if (lspage == "CADApplicationEdit") {
                $location.url('app/AgrTrnPendingCADReview?application_gid=' + application_gid);
            }
        ...
        $scope.creditinstitution_add = function (institution_gid) {
            $location.url('app/AgrTrnDocumentCheckList?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=' + lspage + '&lstype=Institution' + '&lsdocument_status=' + $scope.document_status  );
        }

        $scope.creditindividual_add = function (contact_gid) {
            $location.url('app/AgrTrnIndividualDocCheckList?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&lspage=' + lspage + '&lstype=Individual' + '&lsdocument_status=' + $scope.document_status );
        }
        ...
        $scope.addcompany_dtl = function () {
            $location.url('app/AgrMstCADInstitutionDtlAdd?application_gid=' + application_gid + '&lspage=' + lspage);
        }

        $scope.addindividual_dtl = function () {
            $location.url('app/AgrMstCADIndividualDtlAdd?application_gid=' + application_gid + '&lspage=' + lspage);
        }

        $scope.addgroup_dtl = function () {
            $location.url('app/AgrMstCADGroupDtlAdd?application_gid=' + application_gid + '&lspage=' + lspage);
        }
