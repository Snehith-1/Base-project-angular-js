(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditLoanDtlEditController', MstCreditLoanDtlEditController);

        MstCreditLoanDtlEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstCreditLoanDtlEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditLoanDtlEditController';
        ...
        function activate() {
            ...
            var url = 'api/MstApplicationEdit/GetEditLoanLimit';
            ...
            var url = 'api/MstApplicationEdit/CollateralDocumentList';
            SocketService.getparams(url, param).then(function (resp) {
        ...
        $scope.updateLoan = function () {
            ...
            if ((txtloanfaility_amount) > (lsoveralllimit_amount - lsloanfacility_amount)) {
                ...
                if ($scope.txteditfacilityreqon_date == null || $scope.txteditfacilityreqon_date == '' || $scope.cboProductTypelist == null
                    ...
                    || $scope.cboLoanTypelist == null || $scope.cboLoanTypelist == '' || $scope.txtloanfaility_amount == null || $scope.txtloanfaility_amount == ''
                    || $scope.txteditrate_interest == null || $scope.txteditrate_interest == '' || $scope.txteditpanel_interest == null || $scope.txteditpanel_interest == ''
                    || $scope.txtedittenure_years == null || $scope.txtedittenure_years == '' || $scope.txtedittenure_months == null || $scope.txtedittenure_months == ''
                    || $scope.cboFacilityTypelist == null || $scope.cboFacilityTypelist == '' || $scope.txtedittenure_days == null || $scope.txtedittenure_days == ''
                    || $scope.cboFacilitymodelist == null || $scope.cboFacilitymodelist == '' || $scope.txtedit_margin == '' || $scope.txtedit_margin == null || $scope.rdbinterest_status == '' || $scope.rdbinterest_status == null) {
        ...
        $scope.productdtl_add = function () {
            if (($scope.cboproduct_name == undefined) || ($scope.cboproduct_name == '') || ($scope.cboproduct_name == null) ||
                ...
                var url = 'api/MstApplicationAdd/PostProductDtlAdd';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        function Tempproductdetaillist() {
            ...
            var url = 'api/MstApplicationAdd/GetProductDtlList';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
        ...
        $scope.downloadallcall = function () {
