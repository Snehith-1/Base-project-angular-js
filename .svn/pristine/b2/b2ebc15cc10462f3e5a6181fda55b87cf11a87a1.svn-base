(function () {
    ...
    angular
        .module('angle')
        .controller('MstRMInitiateDisbursementController', MstRMInitiateDisbursementController);

    MstRMInitiateDisbursementController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstRMInitiateDisbursementController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstRMInitiateDisbursementController';       
        ...
        activate();        
        function activate() {   
            $scope.disbursementto_show = false;
            $scope.disbursementto_disabled = true;
            ...
            var application_gid = $location.search().application_gid;
            if (application_gid == '' || application_gid == undefined) {
                $scope.amount_validation = true;
                $scope.hide_product = true;
                $scope.disbursementto_show = false;
                $scope.disbursementto_disabled = true;
                $scope.supplierdtl_show = false;
                $scope.addsupplierdetails_show = false;
                $scope.editsanction_dropdown = false;
                $scope.sanction_dropdown = true;
                $scope.editlsa_dropdown = false;
                $scope.lsa_dropdown = false;
                $scope.hidelsa_dropdown = true;
                ...
                $scope.cbosanctionref_no = application_gid;
                $scope.cbodisbursement_to = disbursement_to;
                $scope.cbolsaref_no = lsgeneratelsa_gid;
                $scope.cboproductsubproduct_name = productsubproduct_gid;
                $scope.editsanction_dropdown = true;
                $scope.sanction_dropdown = false;
                $scope.disbursementto_show = true;
                $scope.disbursementto_disabled = false;
                $scope.amount_validation = true;
                ...
                $scope.buyerdtl_show = false;
                $scope.supplierdtl_show = false;
                $scope.editlsa_dropdown = true;
                $scope.lsa_dropdown = false;
                $scope.editproduct_dropdown = true;
                ...
                //    });
                    ...
                    var params = {
                        application2sanction_gid: $location.search().application2sanction_gid,
                        application_gid: application_gid
                    }
                ...
                var url = 'api/MstCreditOpsApplication/GetDisbSupplierSummary';
                SocketService.get(url).then(function (resp) {
                ...
                var url = 'api/MstCreditOpsApplication/GetProductBuyerList';
                SocketService.getparams(url, params).then(function (resp) {
                    $scope.mstproductbuyer_list = resp.data.mstproductbuyer_list;
                });

                $scope.farmer_delete = function () {
                    var farmercontact_gid = $location.search().farmercontact_gid;
                    var params = {
                ...
                if (cbodisbursement_to == "Applicant(B2B)") {
                        ...
                        if ($scope.fileinputvalue == '' || $scope.fileinputvalue == undefined || $scope.fileinputvalue == null) {
                            ...
                            SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                                ...
                                if (resp.data.status == true) {
                                    ...
                                    var url = 'api/MstCreditOpsApplication/GetFarmerIndividualSummary';
                                    SocketService.getparams(url, params).then(function (resp) {
                                        $scope.farmerindividualsummary_list = resp.data.farmerindividualsummary_list;
                                    });
                                }
                ...
                else if (cbodisbursement_to == "Buyer(B2B2C)") {
                    $scope.buyerdtl_show = true;
                    $scope.farmer_show = false;
                    $scope.applicant_show = false;
                    $scope.supplierdtl_show = false;
                ...
                else if (cbodisbursement_to == "Supplier") {
            ...
            today = mm + '-' + dd + '-' + yyyy;
            $scope.Loandisbursement_date = today;            
        ...
        $scope.sanctionrefno_change = function (cbosanctionref_no, generatelsa_gid) {
            
            var application_gid = $scope.cbosanctionref_no.application_gid;            
            var application2sanction_gid = $scope.cbosanctionref_no.application2sanction_gid;  
            $scope.disbursementto_show = false;
            $scope.disbursementto_disabled = true;
            $scope.hidelsa_dropdown = false;
            $scope.lsa_dropdown = true;
            $scope.hide_product = true;
            $scope.show_product = false;
            $scope.hide_product = false;
            ...
            var url = 'api/MstCreditOpsApplication/GetSancLsApprovalFlag';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                $scope.approver_approvalflag = resp.data.approver_approvalflag;
                unlockUI();
                if ($scope.approver_approvalflag != 'Y') {
                    $scope.hide_product = true;
                    $scope.show_product = false;
                    Notify.alert('LSA not approved you cannot initiate disbursement', 'warning');
                }
                else {
                    $scope.hide_product = false;
            ...
            var params = {
                application2sanction_gid: $scope.cbosanctionref_no.application2sanction_gid,
                application_gid: application_gid
            }
            var url = 'api/MstCreditOpsApplication/GetSanctionDtls';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.lsarefno_change = function (generatelsa_gid) {
            var lsgeneratelsa_gid = $scope.cbolsaref_no.generatelsa_gid;   
            $scope.disbursementto_show = false;
            $scope.disbursementto_disabled = true;
            $scope.hidelsa_dropdown = false;
            $scope.lsa_dropdown = true;
            $scope.show_product = true;
            $scope.hide_product = false;
            $scope.editproduct_dropdown = false;
            ...
            var url = 'api/MstCreditOpsApplication/GetNewApplicantSummary';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.product_change = function (cboproductsubproduct_name) {
            $scope.disbursementto_show = true;
            $scope.disbursementto_disabled = false;
            var params = {
        ...
        $scope.documentUpload = function (val) {           
            ...
            if (($scope.txtgeneraldocument_title == null) || ($scope.txtgeneraldocument_title == '') || ($scope.txtgeneraldocument_title == undefined)) {
                ...
                frm.append('rmdisbursementrequest_gid', '');
        ...
        $scope.disbursementto_change = function (cbodisbursement_to) {
            var cbodisbursement_to = cbodisbursement_to;
            ...
            if (cbodisbursement_to == "Applicant(B2B)") {
                ...
                var url = 'api/MstCreditOpsApplication/GetDisbSupplierSummary';
                SocketService.getparams(url).then(function (resp) {
                    $scope.disbsupplierdtl_list = resp.data.disbsupplierdtl_list;
                });
                ...
                $scope.suppliername_change = function (cbosupplier_name) {
                    ...
                    var cbosupplier_name = cbosupplier_name.supplier_gid;                  

                    var params = {
                        supplier_gid: cbosupplier_name
                    }
                    var url = 'api/MstCreditOpsApplication/GetSupplierIfscCodeDropDown';
                    lockUI();
                    SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.coapplicantdtl_add = function (farmercontact_gid) {
            ...
            var application2sanction_gid = $scope.cbosanctionref_no.application2sanction_gid;
            var lsgeneratelsa_gid = $scope.cbolsaref_no.generatelsa_gid; 
            var productsubproduct_gid = $scope.cboproductsubproduct_name.application2loan_gid;
            $location.url('app/MstDisbCoApplicantDtlAdd?farmercontact_gid=' + farmercontact_gid + '&application_gid=' + application_gid + '&disbursement_to=' + disbursement_to + '&customer_urn=' + customer_urn + '&lsgeneratelsa_gid=' + lsgeneratelsa_gid + '&application2sanction_gid=' + application2sanction_gid + '&productsubproduct_gid=' + productsubproduct_gid);
        }
        ...
        $scope.add_supplierdtl = function () {
            ...
            if (($scope.cbosupplier_name == undefined) || ($scope.cbosupplier_name == '') || ($scope.cbosupplier_name == undefined) ||
               ...
               ($scope.txtdisbursement_amount == '') || ($scope.txtdisbursement_amount == '') || ($scope.txtdisbursement_amount == undefined)) {
                ...
                try {
                   if ($scope.txtsuplacctopen_date.split("-"))
                       $scope.txtsuplacctopen_date = $scope.txtsuplacctopen_date.split("-").reverse().join("-");
                }
                catch (e) { $scope.txtsuplacctopen_date = $scope.txtsuplacctopen_date }
    ...
    $scope.BankAccValidation = function () {
        ...
        $scope.disbursementamount = function (primarygid, list_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.txtlastyear_turnoverchange = function () {
                    var input = document.getElementById('lastyear_turnover1').value;
                    var str = input.replace(/,/g, '');
                    var output = Number(str).toLocaleString('en-IN');
                    var lswords_totalamount = cmnfunctionService.fnConvertNumbertoWord(str);
                    if (output == "NaN") {
                ...
                $scope.confirm_Submit = function () {
                    ...
                    if ($scope.txtdisbursement_amount == "" || $scope.txtdisbursement_amount == null) {
                        $scope.bankaccvalidation = false;
                        alert('Kindly Add Disbursement Amount..!', 'warning');
                        $scope.txtacctholder_name = '';
                    } else {
                    if ($scope.lbldisbursementamount_status == "F") {
                        $scope.bankaccvalidation = false;
                        alert('Disbursement Amount Greater then Sanction Amount..!', 'warning');
                        $scope.txtacctholder_name = '';
                    } else {
                        var params = {
                            application_gid: list_gid,
        ...
        $scope.Updateapplicant_disbursement_amount = function (primarygid, list_gid, lbldisbursementamount_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.updateapplicant_creditopsamt = function () {
                    if ($scope.txteditapplicantcreditopsdisbursement_amount == "" || $scope.txteditapplicantcreditopsdisbursement_amount == null) {
                        $scope.bankaccvalidation = false;
                        alert('Kindly Add Disbursement Amount..!', 'warning');
                        $scope.txtacctholder_name = '';
                    } else {
        ...
        $scope.edit_supplierdtl = function (disbapplicantbankdtl_gid) {
            ...
            //$scope.lbldisbapplicantbankdtl_gid = disbapplicantbankdtl_gid;
            ...
            if (($scope.txteditIFSC_Code == undefined) ||
                ...
                Notify.alert('Enter All Mandatory Fields', 'warning');
                    ...
                    var params = {
                        ...
                        branch_address: $scope.txteditBank_Address,
        ...
        $scope.downloadalluploaddocument = function () {
            for (var i = 0; i < $scope.uploaddocument_list.length; i++) {
                DownloaddocumentService.Downloaddocument($scope.uploaddocument_list[i].chequeleaf_path, $scope.uploaddocument_list[i].chequeleaf_name);
            }
        }
        ...
        $scope.downloadallfilename = function () {
        ...
        //$scope.editbankacctdtl_uploaddocumentcancel = function (disbapplicantbankdocument_gid) {
        //    lockUI();
        //    var params = {
        //        disbapplicantbankdocument_gid: disbapplicantbankdocument_gid
        //    }
        ...
        //            var params = {
        //                disbapplicantbankdtl_gid: $scope.disbapplicantbankdtl_gid
        //            }
        //            var url = 'api/MstCreditOpsApplication/GetDisbAppilicantDtlView';
        //            lockUI();
        //            SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.txtamountchange5 = function () {
            ...
            var params = {
                application_gid: $scope.cbosanctionref_no.application_gid,
                rmdisbursementrequest_gid: '',
                validation_amount: str
            }
            var url = 'api/MstCreditOpsApplication/DisbursementAmountCalValidation';
        ...
        $scope.coapplicant_exportexcel = function () {
            ...
            var url = 'api/MstCreditOpsApplication/CoApplicantExportExcel';
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == true) {
                    unlockUI();
