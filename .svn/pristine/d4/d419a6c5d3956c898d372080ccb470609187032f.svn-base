(function () {
    ...
    angular
        .module('angle')
        .controller('MstCadDeferralStatusController', MstCadDeferralStatusController);

    MstCadDeferralStatusController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function MstCadDeferralStatusController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCadDeferralStatusController';
        ...
        function activate() {
            ...
            SocketService.getparams(url, param).then(function (resp) {
                ...
                $scope.lsextension_flag = resp.data.extension_flag;
                if (lsdeferraltag == "null" && $scope.lsextension_flag == "N") {
                ...
                else if (lsdeferraltag != "null" && $scope.lsextension_flag == "N") {
                    $scope.activity_typelist = [
                        { id: 0, activitytype_name: 'Waiver', activity_type: 'Waiver' },
                        { id: 1, activitytype_name: 'Deferral', activity_type: 'Tag to Deferral' },
                        /* { id: 3, activity_type: 'Document Upload' },*/
                ...
                else if (lsdeferraltag != "null" && $scope.lsextension_flag == "Y") {
                    $scope.activity_typelist = [
        ...
        $scope.Back = function () {
            ...
            //}
                ...
                $location.url('app/MstCadDeferralDochecklist?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
            }
        }
