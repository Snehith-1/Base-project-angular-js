    ...
    function routesConfig($stateProvider, $locationProvider, $urlRouterProvider, helper) {
        ...
        $stateProvider
         ...
         .state('app.caseAllocation', {
             ...
             templateUrl: helper.basepath('ems.ep/caseAllocation.html'),
             resolve: helper.resolveFor('oitozero.ngSweetAlert', 'ui.select', 'ngTable', 'ngDialog', 'datatables')
         })

             .state('app.caseAllocationView', {
                 url: '/caseAllocationView',
                 title: 'caseAllocationView',
                 templateUrl: helper.basepath('ems.ep/caseAllocationView.html'),
                 resolve: helper.resolveFor('oitozero.ngSweetAlert', 'ui.select', 'ngTable', 'ngDialog', 'datatables','textAngular')
             })

             .state('app.invoiceCreate', {
                 url: '/invoiceCreate',
                 title: 'invoiceCreate',
                 templateUrl: helper.basepath('ems.lp/invoiceCreate.html'),
                 resolve: helper.resolveFor('oitozero.ngSweetAlert', 'ui.select', 'ngTable', 'ngDialog', 'datatables')
             })
            ...
            .state('app.invoiceView', {
                url: '/invoiceView',
                title: 'invoiceView',
                templateUrl: helper.basepath('ems.lp/invoiceView.html'),
                resolve: helper.resolveFor('oitozero.ngSweetAlert', 'ui.select', 'ngTable', 'ngDialog', 'datatables')
            })

             .state('app.invoiceSummary', {
                 url: '/invoiceSummary',
                 title: 'invoiceSummary',
                 templateUrl: helper.basepath('ems.lp/invoiceSummary.html'),
                 resolve: helper.resolveFor('oitozero.ngSweetAlert', 'ui.select', 'ngTable', 'ngDialog', 'datatables')
             })
             ...
             .state('app.legalSR', {
                 url: '/legalSR',
                 title: 'legalSR',
                 templateUrl: helper.basepath('ems.lp/legalSR.html'),
                 resolve: helper.resolveFor('oitozero.ngSweetAlert', 'ui.select', 'ngTable', 'ngDialog', 'datatables')
             })

             .state('app.lawyerLegalSR360', {
                 url: '/lawyerLegalSR360',
                 title: 'lawyerLegalSR360',
                 templateUrl: helper.basepath('ems.lp/lawyerLegalSR360.html'),
                 resolve: helper.resolveFor('oitozero.ngSweetAlert', 'ui.select', 'ngTable', 'ngDialog', 'datatables', 'textAngular')
             })
          ...
          .state('page.lock', {
              ...
              templateUrl: 'app/pages/lock.html'
               ...
               .state('page.user_login', {
                   url: '/user_login',
                   title: 'user_login',
                   templateUrl: 'app/pages/user_login.html',
               })
          .state('page.404', {
              url: '/404',
              title: 'Not Found',
...
function ($rootScope, $http, $window, $state, $location,$cookies) {
    ...
    var ls_host = '';
    //lshostname = 'ku.directverify.in';
    var hostdtl = lshostname.split('.');
    var ls_length = hostdtl.length;

    if (lshostname.indexOf('localhost') >= 0) {
        ls_host = 'http://localhost/StoryboardAPI/';
    }
    else if (lshostname.indexOf('local.com') >= 0) {
        ls_host = 'http://api.local.com/';
    } 
    else {
        var ls_host = 'http://' + lshostname + '/StoryboardAPI/';
      //  var ls_host = 'https://tools.samunnati.com/StoryboardAPI/';
    }
    var stcFactory = {};         
...
(function() {
    ...
    function settingsRun($rootScope, $localStorage){
      ...
      // ----------------------------------- 
      $rootScope.app = {
        name: 'Tools::Samunnati',
        description: 'Storyboard ERP-VCIDEX',
        ...
        function activate() {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                ...
                if (resp.data.external_photo != "N") {
                    ...
                    $scope.external_photo = url.concat(str);
...
(function () {
    ...
    caseAllocationcontroller.$inject = ['$rootScope', '$scope', '$state', '$cookies', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', '$route'];

    function caseAllocationcontroller($rootScope, $scope, $state, $cookies, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, $route) {
        /* jshint validthis:true */
        var vm = this;
        ...
        $scope.View = function (val, customer_gid) {
            localStorage.setItem('allocation_gid', val);
...
(function () {
    ...
    function caseAllocationViewcontroller($rootScope, $scope, $state, $cookies, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, $route) {
        ...
        function activate() {
            lockUI();
            var allocation_gid = {
                allocation_gid: localStorage.getItem('allocation_gid')
            }

            var url = "api/allocationManagement/getExternalDetails";
            SocketService.getparams(url, allocation_gid).then(function (resp) {
                console.log(resp);
            ...
            var url = "api/customerManagement/getSanctiondetail";
            SocketService.getparams(url, params).then(function (resp) {
                $scope.customerSanction = resp.data.sanctiondetails;
            });
            unlockUI();
        }
