(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnPMGPhysicalDochecklistController', AgrTrnPMGPhysicalDochecklistController);

    AgrTrnPMGPhysicalDochecklistController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService', 'cmnfunctionService'];

    function AgrTrnPMGPhysicalDochecklistController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnPMGPhysicalDochecklistController';
        ...
        $scope.Back = function () {
            if (lspage == 'CadPhysicalDocMaker') {
                $location.url('app/AgrTrnCadPhysicalDocGuarantorDtls?application_gid=' + application_gid + '&lspage=CadPhysicalDocMaker&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
            }
            else {
                $location.url('app/AgrTrnCadPhysicalDocGuarantorDtls?application_gid=' + application_gid + '&lspage=' + lspage + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
            }
        }
        ...
        $scope.addcovenantperiods = function (groupdocumentchecklist_gid, documenttype_name, covenant_periods, buffer_days, Completed) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.addcovenentperiodsclick = function () {
                    ...
                    console.log(params);
                    var url = 'api/AgrTrnPhysicalDocument/PostPhysicalCovenantPeriods';
                ...
                $scope.covenantperiod_delete = function (covenantperioddtl_gid) {
                    ...
                    var url = 'api/AgrTrnPhysicalDocument/GetcancelPhysicalCovenantPeriod';
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.tagto_deferral = function (documentcheckdtl_gid, scanned_documentcount) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/AgrTrnPhysicalDocument/GetPhysicalMStDeferraltag';
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                ...
                $scope.tagdoc_submit = function () {
                    ...
                    var url = 'api/AgrTrnPhysicalDocument/PostPhysicalDeferralTaggedDoc';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.defferal_history = function (documentcheckdtl_gid) {
            $location.url('app/AgrTrnPhysicalDeferralHistory?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
        }

        $scope.raise_query = function (documentcheckdtl_gid, covenant_type) {
            $location.url('app/AgrTrnPMGPhysicalDocQuery?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath + '&lscovenant_type=' + covenant_type);
        }

        $scope.extension_waiver = function (documentcheckdtl_gid, deferraltagged) {
            $location.url('app/AgrTrnCadPhysicalDocStatus?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath + '&lsdeferraltag=' + deferraltagged);
        }
        ...
        $scope.deferraldocmultiple_upload = function (documentDropdown, covenant_type) {
            var modalInstance = $modal.open({
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.documentDropdown = documentDropdown;

                $scope.physicalDocumentUpload = function (val, val1, name) {
                    lockUI();
                    if ($scope.cbodocument == "" || $scope.cbodocument == undefined || $scope.cbodocument == null) {
                        alert('Kindly Choose the document title');
                    ...
                    var lsgroupdocumentchecklist_gid = $scope.cbodocument.map(function (val) { return val.groupdocumentchecklist_gid; }).join("','");
                    lsgroupdocumentchecklist_gid = "'" + lsgroupdocumentchecklist_gid + "'";
                    frm.append('document_title', '');
                    frm.append('application_gid', application_gid);
                    frm.append('groupdocumentchecklist_gid', lsgroupdocumentchecklist_gid)
                    frm.append('RMupload', 'N');
                    frm.append('covenant_type', covenant_type);
                    frm.append('project_flag', "Default");
                    $scope.uploadfrm = frm;
                    if ($scope.uploadfrm != undefined) {
                        var url = 'api/AgrTrnPhysicalDocument/PhysicalDocumentMultiUpload';
                        SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                            unlockUI();
                            $("#scannedmultiplefile").val('');
                            unlockUI();
