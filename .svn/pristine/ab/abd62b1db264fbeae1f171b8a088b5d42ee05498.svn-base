(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditIndividualRepaymentEditController', AgrTrnSuprCreditIndividualRepaymentEditController);

    AgrTrnSuprCreditIndividualRepaymentEditController.$inject = ['$rootScope', '$sce', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function AgrTrnSuprCreditIndividualRepaymentEditController($rootScope, $sce, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditIndividualRepaymentEditController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/LenderTypeList';
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/EditRepaymentTrack';
            ...
            SocketService.getparams(url, param).then(function (resp) {
                ...
                if($scope.txtcurrentoutstanding_Amt!=null && $scope.txtcurrentoutstanding_Amt!=undefined && $scope.txtcurrentoutstanding_Amt!="")
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtcurrentoutstanding_Amt.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
            ...
            ($scope.txtBalanceTenure_month == undefined) || ($scope.txtBalanceTenure_days == undefined) || ($scope.cboAcctClassification == undefined) || ($scope.txtOverdue_Amount == undefined) || ($scope.txtnoofdays_overdue == undefined) || ($scope.txtremarks == undefined)) {
                ...
                if ((lendertypename == 'Bank')) {
                    if (($scope.txtifsc_code == undefined) || ($scope.txtBank_Name == undefined) ||
                        ...
                        var url = 'api/AgrTrnSuprAppCreditUnderWriting/UpdateRepaymentTrack';
                        lockUI();
