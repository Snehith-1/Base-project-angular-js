...
namespace ems.osd.DataAccess
{
    public class DaOsdTrnSentMail
    {
        ...
        public bool DaOSDReplyMail(string email_gid, string decision_gid, string decision, string message_id, string reference_id, string tomail_id, string ccmail_id, string bccmail_id, string user_gid, string subject, string mailcontent)
        {
            try
            {
              ...
              " WHERE 1=1";
                ...
                msSQL = " SELECT document_name,document_path " +
                       " FROM osd_tmp_tmaildetailsattachement" +
                ...
                if (dt_datatable.Rows.Count != 0)
                {
                    ...
                    {
                        Attachment data = new Attachment((dt["document_path"].ToString()).Replace("erpdocument", ConfigurationManager.AppSettings["file_path"] +"/erpdocument"), MediaTypeNames.Application.Octet);
                        data.Name = dt["document_name"].ToString();
                        message.Attachments.Add(data);

                        msSQL = " INSERT into osd_trn_tmaildetailsattachement (" +
                                " email_gid," +
                                " document_name," +
                                " document_path," +
                                " created_date" +
                ...
                msSQL = " select frommail_id,tomail_id,ccmail_id,bccmail_id,date_format(created_date,'%d-%m-%Y %h:%i %p') as email_date,email_subject,mailcontent" +
                        " from osd_trn_tsentmail where email_gid='" + email_gid + "' and sentmail_gid <> '" + msGetGID + "' and decision<>'Transfer' order by email_date desc";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow dt in dt_datatable.Rows)
                ...
                msSQL = "select email_from,email_to,email_cc,email_bcc,date_format(email_date,'%d-%m-%Y %h:%i %p') as email_date,email_subject,email_content" +
                  " from osd_trn_treferencemail where  email_gid='" + email_gid + "'";
                ...
                msSQL = "select email_from,email_to,cc,bcc,date_format(email_date,'%d-%m-%Y %h:%i %p') as email_date,email_subject,email_content" +
                 " from osd_trn_tmaildetails where  email_gid='" + email_gid + "'";
