    ...
    public class DaRoleMaster : ApiController
    {
        ...
        public bool DaRoleSummary(rolelist objrolelist)
        {
            try
            {
                msSQL = "select role_gid, role_code, role_name, reporting_to, reportingto_gid, job_description," +
                       " probation_period ,role_responsible from hrm_mst_trole where 1=1";
        ...
        public bool DaRoleAdd(rolelist objrolelist,string user_gid)
        {
            try
            {
                ...
                string msGetAPICode = objcmnfunctions.GetApiMasterGID("ROLC");
        ...
        public bool DaRoleEdit(role objrole ,string user_gid,string role_gid)
        {
            try
            {
                ...
                {
                    objrole.role_gid = objODBCDatareader["role_gid"].ToString();
                    objrole.role_code = objODBCDatareader["role_code"].ToString();
                    objrole.role_name = objODBCDatareader["role_name"].ToString();
                    objrole.role_responsible = objODBCDatareader["role_responsible"].ToString();
                    objrole.reportingto_gid = objODBCDatareader["reportingto_gid"].ToString();
                    objrole.job_description = objODBCDatareader["job_description"].ToString();
                    objrole.reporting_to = objODBCDatareader["reporting_to"].ToString();
                    objrole.probation_period = objODBCDatareader["probation_period"].ToString();    
                    objrole.status = true;
                    return true;
                }
                else
        ...
        public bool DaRoleUpdate(role objrole,string user_gid)
        {
            try
            {
                ...
                if(mnResult==1)
                {
                    objrole.message = "Role has been Updated Successfully";
                    objrole.status = true;
                    return true;
                }
                else
