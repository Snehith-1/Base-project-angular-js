(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditEconomicCapitalAddController', AgrTrnSuprCreditEconomicCapitalAddController);

    AgrTrnSuprCreditEconomicCapitalAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce'];

    function AgrTrnSuprCreditEconomicCapitalAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditEconomicCapitalAddController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/GetSocialAndTradeCapital';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
        ...
        /* $scope.save_crediteconomic = function () {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/SocialAndTradeCapitalSave';
            lockUI();
        ...
        $scope.submit_crediteconomic = function () {
            if (($scope.SocialCapital == undefined) || ($scope.TradeCapital == undefined) || ($scope.SocialCapital == null) || ($scope.TradeCapital == null) || ($scope.SocialCapital == '') || ($scope.TradeCapital == '')) {
                ...
                var url = 'api/AgrTrnSuprAppCreditUnderWriting/SocialAndTradeCapitalSubmit';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.Edit_socialtrade = function () {
            $location.url('app/AgrTrnSuprCreditEconomicCapitalEdit?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=' + lspage);
        }
