(function () {
    ...
    angular
        .module('angle')
        .controller('AgrGradingToolAddController', AgrGradingToolAddController);

        AgrGradingToolAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrGradingToolAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrGradingToolAddController';
        ...
        {
            $location.url('app/AgrGradingToolEdit?application2gradingtool_gid=' + val + '&application_gid='+ application_gid );
        }

        $scope.gradingtooldetails_view = function (val) {
            $location.url('app/AgrGradingToolView?application2gradingtool_gid=' + val + '&application_gid=' + application_gid);
        }
