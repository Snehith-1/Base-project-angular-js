(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCompanyCrimeCheckRecordAPIController', AgrTrnSuprCompanyCrimeCheckRecordAPIController);

    AgrTrnSuprCompanyCrimeCheckRecordAPIController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce'];

    function AgrTrnSuprCompanyCrimeCheckRecordAPIController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCompanyCrimeCheckRecordAPIController';
        ...
        function activate() {
            ...
            var url = 'api/AgrSuprCrimeCheckAPI/GetTaggedCaseInstitutionSummary';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
        ...
        $scope.CrimeCheck_Tag = function (data) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    ...
                    lockUI();
                    var url = 'api/AgrSuprCrimeCheckAPI/TagCaseInstitution';
        ...
        $scope.TaggedCase_Delete = function (crimecasetaggedinstitution_gid) {
            ...
            var url = 'api/AgrSuprCrimeCheckAPI/DeleteTaggedCaseInstitution';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.search_crimecheck = function () {
            ...
            if ($scope.cboaddress == undefined || $scope.rdbSearchmode == undefined || searchIn == '') {
                ...
                var url = 'api/AgrSuprCrimeCheckAPI/GetCrimeRecordsCompany';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    setTimeout(function () {
