(function () {
    ...
    angular
        .module('angle')
        .controller('MstCompletedCallSummaryController', MstCompletedCallSummaryController);

    MstCompletedCallSummaryController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstCompletedCallSummaryController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCompletedCallSummaryController';
        activate();
        lockUI();
        function activate() {
            var url = 'api/TeleCalling/GetEmpCompletedIBCallSummary';
            SocketService.get(url).then(function (resp) {
                $scope.ibcallcompleted_list = resp.data.ibcall_list;
                unlockUI();
            });           
