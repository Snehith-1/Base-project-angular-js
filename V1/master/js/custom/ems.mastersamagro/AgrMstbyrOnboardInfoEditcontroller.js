(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstbyrOnboardInfoEditcontroller', AgrMstbyrOnboardInfoEditcontroller);

    AgrMstbyrOnboardInfoEditcontroller.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function AgrMstbyrOnboardInfoEditcontroller($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstbyrOnboardInfoEditcontroller';
        ...
        //    lockUI();
        //    var url = 'api/AgrMstBuyerOnboardEdit/GetEnableApprovalAction';
        //    SocketService.getparams(url, param).then(function (resp) {
        //        if (resp.data.status == true) {
        //            unlockUI();
        //            $scope.EnableApproval_list = resp.data.EnableApproval_list;

        //            if ($scope.EnableApproval_list != null) {
        //                var getcontactapplicant = $scope.EnableApproval_list.find(function (v) { return v.contact_stakeholder === "Applicant" });

        //                var getcompletedcontactapplicant = $scope.EnableApproval_list.find(function (v) { return v.contact_status === "Completed" });

        //                var getcompanyapplicant = $scope.EnableApproval_list.find(function (v) { return v.company_stakeholder === "Applicant" });

        //                var getcompletedcompanyapplicant = $scope.EnableApproval_list.find(function (v) { return v.institution_status === "Completed" });

        //                if ((getcompletedcontactapplicant && getcompletedcontactapplicant.contact_gid) && (getcompletedcompanyapplicant && getcompletedcompanyapplicant.institution_gid)) {

        //                    if ((getcontactapplicant && getcontactapplicant.contact_gid) || (getcompanyapplicant && getcompanyapplicant.institution_gid)) {
        //                        $scope.showapprovalsubmitbtn = true;
        //                    }
        //                }
        ...
        function companydocument_list() {
            var url = 'api/AgrMstBuyerOnboard/Getcompanydocumentlist';
            SocketService.get(url).then(function (resp) {
                $scope.institutionupload_list = resp.data.BuyerOnboardinstitutionupload_list;
            });
        }
        ...
        // $scope.preview_document = function (institution2documentupload_gid) {
        //     lockUI();
    ...
    $scope.Passportid_dob =  function (){
        ...
        $scope.individual_save = function () {
            ...
            // validatestring (txtindividual_dob, txtfather_dob, txtmother_dob, txtspouse_dob, txtnominee_dob);
        ...
        $scope.Editindividual = function (contact_gid) {
            ...
            var url = 'api/AgrMstApplication360/GetUserTypeNoApplicantList';
            SocketService.get(url).then(function (resp) {
                $scope.GetUserTypeNoApplicantList = resp.data.usertype_list;
            });
                ...
                //    //else if ($scope.institutionupload_list == null) {
                    ...
                    SocketService.post(url, params).then(function (resp) {
                        ...
                        if (resp.data.status == true) {
                            ...
                            $location.url('app/AgrMstbyrOnboardInfoEdit?hash=' + cmnfunctionService.encryptURL('application_gid=' + $scope.application_gid));
                            overallgeneralsummary();
