(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstApplicationInstitutionEditController', AgrMstApplicationInstitutionEditController);

    AgrMstApplicationInstitutionEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams',  'DownloaddocumentService','cmnfunctionService'];

    function AgrMstApplicationInstitutionEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstApplicationInstitutionEditController';
        ...
        //    $location.url('app/AgrMstCompanyBankAccountDtlEdit?application_gid=' + application_gid + '&institution2bankdtl_gid=' + institution2bankdtl_gid + '&lstab=' + lstab + '&lsstatus=' + lsstatus);
        //}
        ...
        $scope.Individual_add = function () {
            ...
            var application_status = $scope.application_status;
            if ($scope.application_status == 'Completed') {
                $location.url('app/AgrMstApplicationIndividualEdit?lsapplication_gid=' + application_gid + '&lstab=add&lsstatus=' + application_status);
            }
            else {
