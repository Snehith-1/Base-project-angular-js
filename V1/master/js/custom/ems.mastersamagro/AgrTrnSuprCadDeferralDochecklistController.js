(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCadDeferralDochecklistController', AgrTrnSuprCadDeferralDochecklistController);

    AgrTrnSuprCadDeferralDochecklistController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

        function AgrTrnSuprCadDeferralDochecklistController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCadDeferralDochecklistController';
        ...
        function activate() {
            ...
            if (lspage == "CadScannedCompleted") {
                ...
                var url = 'api/AgrMstSuprScannedDocument/GetPageloadScannedDocument';
        ...
        $scope.Back = function () {
            if (lspage == 'CadDeferralMaker') {
                $location.url('app/AgrTrnSuprCadDeferralGuarantorDtls?application_gid=' + application_gid + '&lspage=CadDeferralMaker&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
            }
            else {
                $location.url('app/AgrTrnSuprCadDeferralGuarantorDtls?application_gid=' + application_gid + '&lspage=' + lspage + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
            }
        }
        ...
        $scope.tagto_deferral = function (documentcheckdtl_gid, scanned_documentcount) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/AgrMstSuprScannedDocument/GetMStDeferraltag';
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                ...
                $scope.tagdoc_submit = function () {
                    ...
                    var url = 'api/AgrMstSuprScannedDocument/PostDeferralTaggedDoc';
                    lockUI();
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.yesmovedtosigned = function () {
                    ...
                    if (doc_gid != undefined) {
                        var url = 'api/AgrMstSuprScannedDocument/PostMovedtoSignedDoc';
                        SocketService.post(url, params).then(function (resp) {
                            if (resp.data.status == true) {
                                $modalInstance.close('closed');
                                activate();
        ...
        $scope.addcovenantperiods = function (groupdocumentchecklist_gid, documenttype_name, covenant_periods, Completed) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.addcovenentperiodsclick = function () {
                    ...
                    console.log(params);
                    var url = 'api/AgrMstSuprScannedDocument/PostScannedCovenantPeriods';
                    SocketService.post(url, params).then(function (resp) {
                        if (resp.data.status == true) {
                ...
                $scope.covenantperiod_delete = function (covenantperioddtl_gid) {
                    ...
                    var url = 'api/AgrMstSuprScannedDocument/GetcancelscannedCovenantPeriod';
                    SocketService.post(url, params).then(function (resp) {
        ...
        $scope.defferal_history = function (documentcheckdtl_gid) {
            $location.url('app/AgrTrnSuprScannedDeferralHistory?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
        }

        $scope.raise_query = function (documentcheckdtl_gid) {
            $location.url('app/AgrTrnSuprCadDeferralQuery?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
        }

        $scope.extension_waiver = function (documentcheckdtl_gid, deferraltagged) {
            $location.url('app/AgrTrnSuprCadDeferralStatus?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath + '&lsdeferraltag=' + deferraltagged + '&lsscanned=Y');
        }
                ...
                $scope.docconfirmation_Submit = function () {
                    if ($scope.txtfinalremarks == 'Others' && ($scope.scanfinal_remarks == '' || $scope.scanfinal_remarks == undefined || $scope.scanfinal_remarks == null)) {
                        ...
                        if (documentcheckdtl_gid != undefined) {
                            var url = 'api/AgrMstSuprScannedDocument/PostDocumentConfirmationDoc';
                            lockUI()
                            SocketService.post(url, params).then(function (resp) {
                                if (resp.data.status == true) {
                                    Notify.alert(resp.data.message, {
