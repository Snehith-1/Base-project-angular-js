(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditApprovalSummaryController', AgrTrnSuprCreditApprovalSummaryController);

    AgrTrnSuprCreditApprovalSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrTrnSuprCreditApprovalSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditApprovalSummaryController';
        lockUI();
        activate();
        function activate() {
            var url = 'api/AgrTrnSuprCreditApproval/GetMyAppApprovalSummary';
        ...
        $scope.applcreation_view = function (val) {
            $location.url('app/AgrTrnSuprCcCommitteeApplicationView?application_gid=' + val + '&lstab=CreditApproval');
        }
        ...
        $scope.credit_approval = function (application_gid, employee_gid, appcreditapproval_gid, initiate_flag) {
            $location.url('app/AgrTrnSuprCreditApproval?application_gid=' + application_gid + '&employee_gid=' + employee_gid + '&appcreditapproval_gid=' + appcreditapproval_gid + '&lspage=CreditApproval' + '&initiate_flag=' + initiate_flag);
        }
