(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstSuprIndividualTabController', AgrMstSuprIndividualTabController);

    AgrMstSuprIndividualTabController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

        function AgrMstSuprIndividualTabController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstSuprIndividualTabController';
    ...
    $scope.Passportid_dob =  function (){
        ...
        $scope.individual_save = function () {
            ...
            if ($scope.rdburn_status == 'Yes' && ($scope.txt_urn == '' || $scope.txt_urn == undefined || $scope.txt_urn == null)) {
                ...
                var url = 'api/AgrMstSuprApplicationAdd/IndividualSave';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.individual_submit = function () {
            ...
            //else {
                ...
                if ($scope.rdburn_status == 'Yes' && ($scope.txt_urn == '' || $scope.txt_urn == undefined || $scope.txt_urn == null)) {
                    ...
                    var url = 'api/AgrMstSuprApplicationAdd/IndividualSubmit';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
