(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditExistingBankDtlEditController', AgrTrnSuprCreditExistingBankDtlEditController);

    AgrTrnSuprCreditExistingBankDtlEditController.$inject = ['$rootScope', '$sce', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function AgrTrnSuprCreditExistingBankDtlEditController($rootScope, $sce, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditExistingBankDtlEditController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/BankNameList';
            SocketService.get(url).then(function (resp) {
                unlockUI();
                $scope.bankdtl_list = resp.data.bankdtl_list;
            });
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/CreditUnderwritingFacilityTypeList';
            SocketService.get(url).then(function (resp) {
                unlockUI();
                $scope.creditunderwritingfacilitytype_list = resp.data.creditunderwritingfacilitytype_list;
            });
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/FundedTypeIndicatorList';
            SocketService.get(url).then(function (resp) {
                unlockUI();
                $scope.fundedtypeindicator_list = resp.data.fundedtypeindicator_list;
            });
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/CreditInstalmentFrequencyList';
            SocketService.get(url).then(function (resp) {
                unlockUI();
                $scope.creditinstalmentfrequency_list = resp.data.creditinstalmentfrequency_list;
            });
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/CreditAccountClassificationList';
            SocketService.get(url).then(function (resp) {
                unlockUI();
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/EditExistingBankFacility';
            ...
            SocketService.getparams(url, param).then(function (resp) {
                ...
                if($scope.txtSanctioned_Limit!=null && $scope.txtSanctioned_Limit!=undefined && $scope.txtSanctioned_Limit!="")
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtSanctioned_Limit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                ...
                if($scope.txtInstalment_amount!=null && $scope.txtInstalment_amount!=undefined && $scope.txtInstalment_amount!="")
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtInstalment_amount.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
                ...
                if($scope.txtOutstanding_amount!=null && $scope.txtOutstanding_amount!=undefined && $scope.txtOutstanding_amount!="")
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtOutstanding_amount.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
                ...
                if($scope.txtOverdue_amount!=null && $scope.txtOverdue_amount!=undefined && $scope.txtOverdue_amount!="")
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtOverdue_amount.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
                ...
                if($scope.txtoverdue_dpd!=null && $scope.txtoverdue_dpd!=undefined && $scope.txtoverdue_dpd!="")
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtoverdue_dpd.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
        ...
        $scope.update_ExistingBankacctdtl = function () {
            if (($scope.cboBankName == undefined) || ($scope.cboBankName == '')) {
                ...
                var url = 'api/AgrTrnSuprAppCreditUnderWriting/UpdateExistingBankFacility';
                lockUI();
