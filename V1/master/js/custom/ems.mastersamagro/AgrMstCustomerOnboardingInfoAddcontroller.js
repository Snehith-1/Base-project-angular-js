(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstCustomerOnboardingInfoAddcontroller', AgrMstCustomerOnboardingInfoAddcontroller);

    AgrMstCustomerOnboardingInfoAddcontroller.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function AgrMstCustomerOnboardingInfoAddcontroller($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstCustomerOnboardingInfoAddcontroller';
        ...
        $scope.GetBuyergeneral = function () {
            ...
            unlockUI();
            lockUI();
            var url = 'api/AgrMstBuyerOnboard/GetIndividualSummary';
            SocketService.get(url).then(function (resp) {
            ...
            lockUI();
            var url = 'api/AgrMstBuyerOnboard/GetInstitutionSummary';
            SocketService.get(url).then(function (resp) {
                if (resp.data.status == true) {
                    unlockUI();
                    ...
                    if ($scope.institution_list != null) {
                        ...
                        var getpendingapplicant = $scope.individual_list.find(function (v) { return v.institution_status === "Pending" });
        ...
        $scope.Save_generaldetails = function () {
            ...
            if ($scope.cboprogram != undefined || $scope.cboprogram != null) {
                lsprogram_name = $scope.cboprogram.program_name;
            ...
            var url = 'api/AgrMstBuyerOnboardEdit/SaveGeneralDtl';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.submit_institution = function () {
            ...
            //else
                ...
                var url = 'api/AgrMstBuyerOnboard/SubmitInstitutionDtl';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.individual_submit = function () {
            ...
            //else
                ...
                var url = 'api/AgrMstBuyerOnboard/IndividualSubmit';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.Submittoapproval = function () {
            ...
            SocketService.getparams(url, params).then(function (resp) { 
                if (resp.data.status == true) {
                    ...
                    $location.url('app/AgrMstCustomerOnboardingSummary?selectedIndex=0');
                }
                else {
                    Notify.alert(resp.data.message, {
                        status: 'warning',
