(function () {
    ...
    angular
        .module('angle')
        .controller('AgrProductDescVisitReportAddController', AgrProductDescVisitReportAddController);


    AgrProductDescVisitReportAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function AgrProductDescVisitReportAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrProductDescVisitReportAddController';
        ...
        function activate() {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                $scope.ProductDescVisitReport_List = resp.data.VisitReportList;
        ...
        $scope.edit_productdescvisitreport = function (val) {
            $location.url('app/AgrMstProductDescVisitReportEdit?visitreport_gid=' + val + '&application_gid=' + application_gid + '&lstab=' + lstab);
        }

        $scope.Back = function () {
            if (lstab == 'ProductDescAssigned') {
        ...
        $scope.editaddress = function (val) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.onchangepostal_code = function () {
                    ...
                    var url = 'api/Agrbuyer/GetPostalCodeDetails';
        ...
        $scope.view_RMVisitReport = function (visitreport_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrMstRMVisitReportView";
            window.open(URL, '_blank');
        }

        $scope.view_productdescvisitreport = function (visitreport_gid) {
            var visitreport_gid = visitreport_gid;
            localStorage.setItem('visitreport_gid', visitreport_gid);
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrMstProductDescVisitReportView";
            window.open(URL, '_blank');
        }
