(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnCreditAddCovenantCheckListController', AgrTrnCreditAddCovenantCheckListController);

        AgrTrnCreditAddCovenantCheckListController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce'];

    function AgrTrnCreditAddCovenantCheckListController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnCreditAddCovenantCheckListController';
        ...
        $scope.applytoallcovenant = function () {
            ...
            if (taggeddocument_list && taggeddocument_list != null) {
                var url = "api/AgrTrnCAD/ApplyALLCovenantDocumentList";
                lockUI();
        ...
        $scope.updateDoc_covenantperiod = function (documentcheckdtl_gid) {
            ...
            if (emptycovenant) {
                angular.forEach($scope.covenant_documentlist, function (val) {
                    val.covenantperiod = val.covenantperiod == 'Every month' ? 1 : val.covenantperiod == 'Every 3 months' ? 2 : val.covenantperiod == 'Every 6 months' ? 3 : val.covenantperiod == 4 ? 'Every 1 year' : "";
                });
                unlockUI();
