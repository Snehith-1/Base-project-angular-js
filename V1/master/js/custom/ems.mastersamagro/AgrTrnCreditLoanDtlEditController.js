(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnCreditLoanDtlEditController', AgrTrnCreditLoanDtlEditController);

    AgrTrnCreditLoanDtlEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService', 'cmnfunctionService'];

    function AgrTrnCreditLoanDtlEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnCreditLoanDtlEditController';
        ...
        function activate() {
            ...
            // Calender Popup... //
            $scope.amount_insurancevalidation = true;
            $scope.amount_validation = true;
            ...
            SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.application2loan_gid = resp.data.application2loan_gid;
                ...
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtguidelinevalue.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                ...
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtmarketValue.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
                ...
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtforcedsource_value.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
                ...
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtcollateralSSV_value.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
                ...
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtloanfaility_amount.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
                ...
                if (lspage == "myapp" || lspage == "CreditApproval") {
                    $scope.enablecreditteam = false;
                }
                else {
                    $scope.enablecreditteam = true;
                }
        ...
        $scope.updateLoan = function () {
            ...
            try {
                if ($scope.txtguideline_date.split("-"))
                    $scope.txtguideline_date = $scope.txtguideline_date.split("-").reverse().join("-");
            }
            catch (e) { $scope.txtguideline_date = $scope.txtguideline_date }

            try {
                if ($scope.txtforcedvalueassessed_on.split("-"))
                    $scope.txtforcedvalueassessed_on = $scope.txtforcedvalueassessed_on.split("-").reverse().join("-");
            }
            catch (e) { $scope.txtforcedvalueassessed_on = $scope.txtforcedvalueassessed_on }
                    ...
                    if ($scope.rdbmoratorium_status == 'Yes') {
                        if ($scope.cbomoratorium_type == null || $scope.cbomoratorium_type == '' || $scope.txtmoratorium_startdate == null || $scope.txtmoratorium_startdate == '' || $scope.txtmoratorium_enddate == null || $scope.txtmoratorium_enddate == '') {
                            ...
                            var params = {
                                ...
                                //insurance_availability: $scope.insurance_availability,
        ...
        $scope.upload_doc = function (val, val1, name, application2loan_gid) {
            if (($scope.cboDocumentTitle == null) || ($scope.cboDocumentTitle == '') || ($scope.cboDocumentTitle == undefined)) {
                $("#file").val('');
                Notify.alert('Kindly Select the Document Title', 'warning');
            }
        ...
        $scope.txtinsurancefacility = function () {
            var input = document.getElementById('insuranceamount').value;
            var str1 = input.replace(/,/g, '');
            var str = Math.round(str1);
            var output = Number(str).toLocaleString('en-IN');
            var lswords_totalamount1 = cmnfunctionService.fnConvertNumbertoWord(str);
            if (output == "NaN") {
                ...
                //  $scope.txtvalue = output;
                document.getElementById('words_totalamountinsurance').innerHTML = lswords_totalamount1;
                var txtloanfaility_amount = parseInt($scope.insurance_availability.replace(/[\s,]+/g, '').trim());
                var lsoveralllimit_amount = parseInt($scope.lbloveralllimit_amount.replace(/[\s,]+/g, '').trim());
                if ((lsoveralllimit_amount) < (txtloanfaility_amount)) {
                    $scope.amount_insurancevalidation = false;
                }
                else {
                    $scope.amount_insurancevalidation = true;
                }
            ...
            if ($scope.txtdisbursementmaxpercent_paymentterm > 100) {
                ...
                SocketService.getparams(url, params).then(function (resp) {
                    if (resp.data.status == true) {
                        ...
                        valOutput = parseInt(valInput) + parseInt(valDB);
