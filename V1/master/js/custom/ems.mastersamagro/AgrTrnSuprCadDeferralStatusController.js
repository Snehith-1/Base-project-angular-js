(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCadDeferralStatusController', AgrTrnSuprCadDeferralStatusController);

    AgrTrnSuprCadDeferralStatusController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function AgrTrnSuprCadDeferralStatusController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCadDeferralStatusController';
        ...
        function activate() {
            ...
            if (lsscanned == "Y")
                var url = 'api/AgrMstSuprScannedDocument/GetInitiatedExtensionorwaiver';
            else
            ...
            if (lspage == "CadScannedCompleted" || lscompleted == "CadScannedCompleted") {
        ...
        $scope.intiatextension_submit = function () {
            if ($scope.cboactivitytype === "Extension" && $scope.txtextendeddue_date == "") {
                ...
                if (lsscanned == "Y")
                    var url = 'api/AgrMstSuprScannedDocument/PostInitiateExtensionorwaiver';
                else
        ...
        $scope.Back = function () {
            if (lspage == "RMDocChecklist") {
                $location.url('app/AgrTrnSuprRMDeferralDtlsView?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype);
            }
            else {
