(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnUpcomingCreditApprovalSummaryController', AgrTrnUpcomingCreditApprovalSummaryController);

        AgrTrnUpcomingCreditApprovalSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrTrnUpcomingCreditApprovalSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnUpcomingCreditApprovalSummaryController';
        lockUI();
        activate();
        function activate() {
            var url = 'api/AgrTrnCreditApproval/GetMyUpcomingApprovalSummary';
        ...
        $scope.applcreation_view = function (val) {
            $location.url('app/AgrTrnCcCommitteeApplicationView?application_gid=' + val + '&lspage=UpcomingCreditApproval');
        }
        ...
        $scope.upcomingprogress_appl = function () {
            $location.url('app/AgrTrnCreditUpcomingApprovalSummary');
        }
        ...
        $scope.credit_approval = function (application_gid, employee_gid, appcreditapproval_gid, initiate_flag) {
            $location.url('app/AgrTrnCreditApproval?application_gid=' + application_gid + '&employee_gid=' + employee_gid + '&appcreditapproval_gid=' + appcreditapproval_gid + '&lspage=UpcomingCreditApproval' + '&initiate_flag=' + initiate_flag);
        }
