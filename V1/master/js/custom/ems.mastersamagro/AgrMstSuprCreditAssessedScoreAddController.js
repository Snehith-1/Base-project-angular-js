(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstSuprCreditAssessedScoreAddController', AgrMstSuprCreditAssessedScoreAddController);

    AgrMstSuprCreditAssessedScoreAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrMstSuprCreditAssessedScoreAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstSuprCreditAssessedScoreAddController';
        ...
        function activate() {
            ...
            var url = 'api/AgrMstSuprApplicationGradingTool/DeletetmpGradingTool';
            ...
            SocketService.getparams(url, params).then(function (resp) {
                $scope.gradingtoolrm_list = resp.data.grading_list;
        ...
        var url = 'api/AgrMstSuprApplicationGradingTool/GetAssessmentCriteriaDropDown';
        SocketService.get(url).then(function (resp) {
        ...
        $scope.editAssessmentCriteria = function (val) {
            ...
            var url = 'api/AgrMstSuprApplicationGradingTool/EditAssessmentCriteriaDetails';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.assessmentUpdate = function () {
            if ($scope.editcboassessmentcriteria_name == '' || $scope.editcboassessmentcriteria_name == null || $scope.editcboassessmentcriteria_name == undefined) {
                ...
                var url = "api/AgrMstSuprApplicationGradingTool/UpdateAssessmentCriteriaDetails";
                lockUI();
        ...
        $scope.assessmentcriteria_add = function () {
            if ($scope.cboassessmentcriteria_name == '' || $scope.cboassessmentcriteria_name == null || $scope.cboassessmentcriteria_name == undefined) {
                ...
                var url = 'api/AgrMstSuprApplicationGradingTool/SubmitGradingassesment';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        function assessmentcriteria_list() {
            var url = 'api/AgrMstSuprApplicationGradingTool/GettmpGradingTool';
            SocketService.get(url).then(function (resp) {
        ...
        $scope.save_gradingtool = function () {
            if ($scope.txtdateofsurvey_date == '' || $scope.txtdateofsurvey_date == null || $scope.txtdateofsurvey_date == undefined || $scope.txtdateofsurvey_date ==  " ") {
                ...
                var url = 'api/AgrMstSuprApplicationGradingTool/SaveasDraftGradingToolDetails';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.submitgrading = function () {
            ...
            var url = 'api/AgrMstSuprApplicationGradingTool/SubmitGradingToolDetails';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.gradingtooldetails_edit = function (val) {
            $location.url('app/AgrMstSuprCreditAssessedScoreEdit?application2gradingtool_gid=' + val + '&application_gid=' + application_gid + '&lstab=' + lstab);
        }

        $scope.gradingtooldetails_view = function (application2gradingtool_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrMstSuprCreditAssessedScoreView";
        ...
        $scope.assessmentcriteriadetails_delete = function (application2gradingassesment_gid) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    var url = 'api/AgrMstSuprApplicationGradingTool/Deletegradingassesment';
                    SocketService.getparams(url, params).then(function (resp) {
                        if (resp.data.status == true) {
                            assessmentcriteria_list();
                        }
        ...
        $scope.RMassessedscore_view = function (application2gradingtool_gid) {
            var application2gradingtool_gid = application2gradingtool_gid;
            localStorage.setItem('application2gradingtool_gid', application2gradingtool_gid);
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrMstSuprRMAssessedScoreView";
            window.open(URL, '_blank');
        }
