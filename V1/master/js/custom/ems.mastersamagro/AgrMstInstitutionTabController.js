(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstInstitutionTabController', AgrMstInstitutionTabController);

    AgrMstInstitutionTabController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

        function AgrMstInstitutionTabController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstInstitutionTabController';
        ...
        $scope.save_institution = function () {
            ...
            var url = 'api/AgrMstApplicationAdd/SaveInstitutionDtl';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.submit_institution = function () {
            ...
            //}
                ...
                var url = 'api/AgrMstApplicationAdd/SubmitInstitutionDtl';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
