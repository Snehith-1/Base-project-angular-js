(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCCMeetingRescheduleController', AgrTrnSuprCCMeetingRescheduleController);

    AgrTrnSuprCCMeetingRescheduleController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrTrnSuprCCMeetingRescheduleController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCCMeetingRescheduleController';
        ...
        $scope.reschedule = function () {
            ...
            ($scope.txteditccmeeting_date == undefined) || ($scope.txteditccmeeting_date == '') || ($scope.txteditccmeeting_date == null))
            {
                ...
                if (($scope.txteditstart_time == undefined) || ($scope.txteditend_time == undefined)||($scope.txteditstart_time == "") || ($scope.txteditend_time == "")) {
                    ...
                    lockUI();
                    var url = 'api/AgrTrnSuprCC/RecheduleMeeting';
                    SocketService.post(url, params).then(function (resp) {
