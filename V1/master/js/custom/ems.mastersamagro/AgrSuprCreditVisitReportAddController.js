(function () {
    ...
    angular
        .module('angle')
        .controller('AgrSuprCreditVisitReportAddController', AgrSuprCreditVisitReportAddController);

    AgrSuprCreditVisitReportAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function AgrSuprCreditVisitReportAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrSuprCreditVisitReportAddController';
        ...
        function activate() {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                $scope.VisitReportRMList = resp.data.VisitReportList;
            ...
            var url = 'api/AgrMstSuprApplicationVisitReport/GetVisitedplace';
            SocketService.get(url).then(function (resp) {
        ...
        $scope.edit_creditvisitreport = function (val) {
            $location.url('app/AgrMstSuprCreditVisitReportEdit?visitreport_gid=' + val + '&application_gid=' + application_gid + '&lstab=' + lstab);
        }

        $scope.Back = function () {
            if (lstab == 'ApplAssignmnet') {
        ...
        function address_list() {
            var url = 'api/AgrMstSuprApplicationVisitReport/GetVisittmpAddressList';
            SocketService.get(url).then(function (resp) {
        ...
        function visitedpersondetails_list() {
            var url = 'api/AgrMstSuprApplicationVisitReport/GetLVisittmpPersondtlList';
            SocketService.get(url).then(function (resp) {
        ...
        $scope.save_visitreport = function () {
            if ($scope.txtdateof_visit == '' || $scope.txtdateof_visit == null || $scope.txtdateof_visit == undefined) {
                ...
                var url = 'api/AgrMstSuprApplicationVisitReport/PostApplicationVisitReport';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
                        ...
                        var url = 'api/AgrMstSuprApplicationVisitReport/GetVisittmpDocumentList';
                        SocketService.get(url).then(function (resp) {
                            $scope.UploadDocumentList = resp.data.UploadDocumentList;

                        });
                        var url = 'api/AgrMstSuprApplicationVisitReport/GetVisittmpPhotosList';
                        SocketService.get(url).then(function (resp) {
        ...
        $scope.submit_visitreport = function () {
            ...
            var url = 'api/AgrMstSuprApplicationVisitReport/PostSubmitApplicationVisitReport';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.view_RMVisitReport = function (visitreport_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrMstSuprRMVisitReportView";
            window.open(URL, '_blank');
        }

        $scope.view_creditvisitreport = function (visitreport_gid) {
            var visitreport_gid = visitreport_gid;
            localStorage.setItem('visitreport_gid', visitreport_gid);
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/AgrMstSuprCreditVisitReportView";
            window.open(URL, '_blank');
        }
