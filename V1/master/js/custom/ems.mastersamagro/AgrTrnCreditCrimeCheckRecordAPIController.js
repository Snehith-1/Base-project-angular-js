(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnCreditCrimeCheckRecordAPIController', AgrTrnCreditCrimeCheckRecordAPIController);

    AgrTrnCreditCrimeCheckRecordAPIController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce'];

    function AgrTrnCreditCrimeCheckRecordAPIController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnCreditCrimeCheckRecordAPIController';
        ...
        function activate() {
            ...
            var url = 'api/AgrCrimeCheckAPI/GetTaggedCaseIndividualSummary';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
        ...
        $scope.CrimeCheck_Tag = function (data) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    ...
                    lockUI();
                    var url = 'api/AgrCrimeCheckAPI/TagCaseIndividual';
        ...
        $scope.TaggedCase_Delete = function (crimecasetaggedcontact_gid) {
            ...
            var url = 'api/AgrCrimeCheckAPI/DeleteTaggedCaseIndividual';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.search_crimecheck = function () {
            ...
            if ($scope.cboaddress == undefined || $scope.rdbSearchmode == undefined || searchIn == '') {
                ...
                var url = 'api/AgrCrimeCheckAPI/GetCrimeRecordsIndividual';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    setTimeout(function () {
