(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCADGroupProcessAssignController', AgrTrnSuprCADGroupProcessAssignController);

    AgrTrnSuprCADGroupProcessAssignController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function AgrTrnSuprCADGroupProcessAssignController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCADGroupProcessAssignController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnSuprCAD/GetMenu';
            ...
            var url = 'api/AgrTrnSuprCAD/GetProcessTypeSummary';
            SocketService.getparams(url, params).then(function (resp) {
                ...
                if ($scope.processtype_name == 'Accept') {
                    ...
                    var url = 'api/AgrTrnSuprCAD/GetCADMembers';
                    lockUI();
                    SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.Process_add = function () {
            if (($scope.cboprocesstype == 'Sendback to CC' || $scope.cboprocesstype == 'Sendback to Credit Underwriting') && ($scope.remarks == '' || $scope.remarks == null)) {
                ...
                if ($scope.rdbassign == 'Menuselection') {
                    ...
                    var url = "api/AgrTrnSuprCAD/PostProcessType";
                    lockUI();
        ...
        $scope.Submit = function () {
            ...
            if (($scope.cboprocesstype == 'Sendback to CC' || $scope.cboprocesstype == 'Sendback to Credit Underwriting') && ($scope.remarks == '' || $scope.remarks == null)) {
                ...
                if ($scope.mdlprocesstype_list != null) {
                    ...
                    var url = "api/AgrTrnSuprCAD/UpdateProcessType";
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status = true) {
                    ...
                    var url = "api/AgrTrnSuprCAD/PostRevertCADtoCC";
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status = true) {
                    ...
                    var url = "api/AgrTrnSuprCAD/PostRevertCADtoCredit";
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status = true) {
        ...
        $scope.delete = function (processtypeassign_gid) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    var url = 'api/AgrTrnSuprCAD/DeleteCADAssignment';
                    SocketService.getparams(url, params).then(function (resp) {
                        if (resp.data.status == true) {
                            activate();
                        }
