(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstSuprUDCMakerAddController', AgrMstSuprUDCMakerAddController);

    AgrMstSuprUDCMakerAddController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', '$anchorScroll','cmnfunctionService'];

    function AgrMstSuprUDCMakerAddController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, $anchorScroll,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstSuprUDCMakerAddController';
        ...
        function activate() {
            ...
            var url = 'api/AgrSuprUdcManagement/GetUdcTempClear';
            SocketService.get(url).then(function (resp) {
            });

            var url = 'api/AgrSuprUdcManagement/GetCustomers';
            lockUI();
            SocketService.get(url).then(function (resp) {
            ...
            var url = 'api/AgrSuprUdcManagement/GetStakeholders';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.add_cheque = function () {
            ...
            var url = 'api/AgrSuprUdcManagement/PostChequeDetail';
        ...
        function cheque_list() {
            var url = 'api/AgrSuprUdcManagement/GetChequeSummary';
            SocketService.get(url).then(function (resp) {
        ...
        $scope.delete_cheque = function (udcmanagement2cheque_gid) {
            ...
            var url = 'api/AgrSuprUdcManagement/DeleteChequeDetail';
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == true) {
            ...
            if ($scope.chequedocument_list == '' || $scope.chequedocument_list == undefined || $scope.chequedocument_list == null) {
                ...
                SocketService.post(url, params).then(function (resp) {
                    ...
                    if (resp.data.status == true) {
                        ...
                        $location.url('app/AgrMstUDCMakerSummary?application_gid=' + application_gid);
                    }
                    else {
                        Notify.alert(resp.data.message, {
                            status: 'warning',
        ...
        $scope.viewCheque = function (udcmanagement2cheque_gid) {
            $location.url('app/AgrMstSuprUDCMakerView?lsudcmanagement_gid=' + $scope.udcmanagement_gid + '&lstab=add');
        }
        ...
        $scope.UploadDocument = function (val, val1, name) {
            ...
            if ($scope.uploadfrm != undefined) {
                var url = 'api/AgrUdcManagement/ChequeDocumentUpload';
                lockUI();
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
                        var url = 'api/AgrSuprKyc/ChequeOCR';
                        SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
        ...
        function chequedocument_list() {
            var url = 'api/AgrSuprUdcManagement/GetChequeDocumentList';
            SocketService.get(url).then(function (resp) {
        ...
        $scope.delete_document = function (cheque2document_gid) {
            ...
            var url = 'api/AgrSuprUdcManagement/ChequeDocumentDelete';
            SocketService.getparams(url, params).then(function (resp) {
