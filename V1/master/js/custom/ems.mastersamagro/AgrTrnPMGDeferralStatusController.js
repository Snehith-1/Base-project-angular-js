(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnPMGDeferralStatusController', AgrTrnPMGDeferralStatusController);

    AgrTrnPMGDeferralStatusController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function AgrTrnPMGDeferralStatusController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnPMGDeferralStatusController';
        ...
        var lstagquery_gid = $location.search().lstagquery_gid;
        ...
        function activate() {
            ...
            if (lstype == "Institution")
                var url = 'api/AgrMstScannedDocument/GetCompanyDocumentSeverity';
            else
                var url = 'api/AgrMstScannedDocument/GetIndividualDocumentSeverity';
            lockUI();
            SocketService.getparams(url, param).then(function (resp) {
                $scope.txtseverity_gid = resp.data.documentseverity_gid;
                $scope.txtseverity_name = resp.data.documentseverity_name;

                if (lsdeferraltag == "null" && $scope.txtseverity_name != 'Critical') {
                    $scope.activity_typelist = [
                        { id: 0, activitytype_name: 'Waiver', activity_type: 'Waiver' },
                        { id: 1, activitytype_name: 'Deferral', activity_type: 'Tag to Deferral' },
                        /* { id: 3, activity_type: 'Document Upload' },*/
                        { id: 2, activitytype_name: '', activity_type: '' }
                    ]
                }

                else if (lsdeferraltag == "null" && $scope.txtseverity_name == 'Critical') {
                    $scope.activity_typelist = [
                        { id: 0, activitytype_name: 'Waiver', activity_type: 'Waiver' },
                        //{ id: 1, activitytype_name: 'Deferral', activity_type: 'Tag to Deferral' },
                        /* { id: 3, activity_type: 'Document Upload' },*/
                    ...
                    $scope.activity_typelist = [
                        ...
                        { id: 1, activitytype_name: 'Waiver', activity_type: 'Waiver' },
                        //{ id: 2, activitytype_name: 'Deferral', activity_type: 'Tag to Deferral' },
                        { id: 2, activitytype_name: '', activity_type: '' }
                    ]
                }
            ...
            var url = 'api/AgrMstScannedDocument/GetScannedApprovalList';
            lockUI();
            SocketService.getparams(url, param).then(function (resp) {
                if (resp.data != null) {
        ...
        $scope.intiatextension_submit = function () {
            ...
            //}
                ...
                var approval_status = $scope.cboapprovalperson == undefined ? 'No Approval' : 'Initiation Pending';
                var params = {
                    ...
                    due_date: $scope.txtdue_date,
                    scanneddoc_flag: lsscanned,
                    tagquery_gid: lstagquery_gid
                }
                if (lsscanned == "Y")
                    var url = 'api/AgrMstScannedDocument/PostInitiateExtensionorwaiver';
                else
                    var url = 'api/AgrTrnPhysicalDocument/PostPhysicalInitiateExtensionorwaiver';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
