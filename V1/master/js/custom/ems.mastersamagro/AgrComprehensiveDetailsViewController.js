(function () {
    ...
    angular
        .module('angle')
        .controller('AgrComprehensiveDetailsViewController', AgrComprehensiveDetailsViewController);

    AgrComprehensiveDetailsViewController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', '$sce'];

    function AgrComprehensiveDetailsViewController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrComprehensiveDetailsViewController';        
        ...
        function activate() {
            ...
            var url = 'api/AgrProbeAPI/ComprehensiveDetailsView';
            SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.pan = resp.data.data.company.pan;
                $scope.website = resp.data.data.company.website;
        ...
        $scope.basic_detail = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=BASICDETAIL');
            activate(); window.scroll(0, 0)
        }

        $scope.authorized_signatories = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=AUTHORIZEDSIGNATORIES');
            activate(); window.scroll(0, 0)
        }

        $scope.director_network = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=DIRECTORNETWORK');
            activate(); window.scroll(0, 0)
        }

        $scope.contact_details = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=CONTACTDETAILS');
            activate(); window.scroll(0, 0)
        }

        $scope.open_charges = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=OPENCHARGES');
            activate(); window.scroll(0, 0)
        }

        $scope.charge_sequence = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=CHARGESEQUENCE');
            activate(); window.scroll(0, 0)
        }

        $scope.financials = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=FINANCIALS');
            activate(); window.scroll(0, 0)
        }

        $scope.nbfc_financials = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=NBFCFINANCIALS');
            activate(); window.scroll(0, 0)
        }

        $scope.financial_parameters = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=FINANCIALPARAMETERS');
            activate(); window.scroll(0, 0)
        }

        $scope.industry_segments = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=INDUSTRYSEGMENTS');
            activate(); window.scroll(0, 0)
        }

        $scope.principal_businessactivities = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=PRINCIPALBUSINESSACTIVITIES');
            activate(); window.scroll(0, 0)
        }
        $scope.shareholdings = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=SHAREHOLDINGS');
            activate(); window.scroll(0, 0)
        }

        $scope.shareholdings_summary = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=SHAREHOLDINGSSUMMARY');
            activate(); window.scroll(0, 0)
        }

        $scope.director_shareholdings = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=DIRECTORSHAREHOLDINGS');
            activate(); window.scroll(0, 0)
        }

        $scope.bifr_history = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=BIFRHISTORY');
            activate(); window.scroll(0, 0)
        }

        $scope.cdr_history = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=CDRHISTORY');
            activate(); window.scroll(0, 0)
        }

        $scope.defaulters_list = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=DEFAULTERLIST');
            activate(); window.scroll(0, 0)
        }

        $scope.legal_history = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=LEGALHISTORY');
            activate(); window.scroll(0, 0)
        }

        $scope.credit_ratings = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=CREDITRATINGS');
            activate(); window.scroll(0, 0)
        }

        $scope.holding_entities = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=HOLDINGENTITIES');
            activate(); window.scroll(0, 0)
        }

        $scope.subsidiary_entities = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=SUBSIDIARYENTITIES');
            activate(); window.scroll(0, 0)
        }

        $scope.associate_entities = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=ASSOCIATEENTITIES');
            activate(); window.scroll(0, 0)
        }

        $scope.joint_ventures = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=JOINTVENTURES');
            activate(); window.scroll(0, 0)
        }

        $scope.securities_allotment = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=SECURITIESALLOTMENT');
            activate(); window.scroll(0, 0)
        }

        $scope.peer_comparison = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=PEERCOMPARISON');
            activate(); window.scroll(0, 0)
        }

        $scope.gst_details = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=GSTDETAILS');
            activate(); window.scroll(0, 0)
        }

        $scope.struckoff248_details = function () {
            $location.url('app/AgrComprehensiveDetailsView?lsdetail_name=STRUCKOFF248DETAILS');
            activate(); window.scroll(0, 0)
        }
