(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditBankAccountDtlAddController', AgrTrnSuprCreditBankAccountDtlAddController);

    AgrTrnSuprCreditBankAccountDtlAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function AgrTrnSuprCreditBankAccountDtlAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, DownloaddocumentService,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditBankAccountDtlAddController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/GetCreditAccountType';
            SocketService.get(url).then(function (resp) {
                unlockUI();
                $scope.accounttype_list = resp.data.creditbankacc_list;
            });
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/ChequeTmpClear';
            SocketService.get(url).then(function (resp) {
            });
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/GetCrediBankAccDtl';
            ...
            ($scope.cboAccountType == undefined) || ($scope.cboAccountType == '') || ($scope.rdbJoint_Account == undefined) || ($scope.rdbJoint_Account == '') ||
                ...
                if ($scope.rdbJoint_Account == 'Yes') {
                    ...
                    if (($scope.txtjointacctholder_name == undefined) || ($scope.txtjointacctholder_name == '')) {
                        ...
                        var url = 'api/AgrTrnSuprAppCreditUnderWriting/PostCreditBank';
                        lockUI();
                        SocketService.post(url, params).then(function (resp) {
                            unlockUI();
        ...
        $scope.creditbankacctdtl_delete = function (creditbankdtl_gid) {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/DeletecreditBankAcc';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    $scope.creditbankacc_list = resp.data.creditbankacc_list;
        ...
        $scope.chequeleafdocumentUpload = function (val) {
            if (($scope.txtdocument_title == null) || ($scope.txtdocument_title == '') || ($scope.txtdocument_title == undefined)) {
                ...
                if ($scope.uploadfrm != undefined) {
                    lockUI();
                    var url = 'api/AgrTrnSuprAppCreditUnderWriting/chequeleafdocumentUpload';
                    console.log(url)
        ...
        $scope.uploaddocumentcancel = function (creditbankdtl2cheque_gid) {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/DeleteCreditcheque';
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == true) {
                    $scope.credituploaddocument_list = resp.data.credituploaddocument_list;
                    Notify.alert(resp.data.message, {
        ...
        $scope.creditbankacctdtl_edit = function (creditbankdtl_gid) {
            $location.url('app/AgrTrnSuprCreditBankAccountDtlEdit?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&creditbankdtl_gid=' + creditbankdtl_gid + '&lspage=' + lspage);
        }
