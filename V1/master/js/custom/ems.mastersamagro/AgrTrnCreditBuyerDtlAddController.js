(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnCreditBuyerDtlAddController', AgrTrnCreditBuyerDtlAddController);

        AgrTrnCreditBuyerDtlAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','$sce','cmnfunctionService'];

    function AgrTrnCreditBuyerDtlAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnCreditBuyerDtlAddController';
        ...
        $scope.add_creditbuyerdtl = function () {
            ...
            ($scope.txtBankCreditValue == '') || ($scope.rdbDeductionat_Source == ''))
           {
             ...
             var url = 'api/AgrTrnAppCreditUnderWriting/PostCreditBuyer';
              lockUI();
        ...
        var url = 'api/AgrTrnAppCreditUnderWriting/DeleteCreditBuyer';
        SocketService.getparams(url, params).then(function (resp) {
            unlockUI();
            if (resp.data.status == true) {
                $scope.institutionbuyer_list = resp.data.creditbuyer_list;
        ...
        $scope.creditbuyer_edit = function (creditbuyer_gid) {
            $location.url('app/AgrTrnCreditBuyerDtlEdit?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&creditbuyer_gid=' + creditbuyer_gid + '&lspage=' + lspage);
        }
