(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCadGuarantorDetailsController', AgrTrnSuprCadGuarantorDetailsController);

    AgrTrnSuprCadGuarantorDetailsController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function AgrTrnSuprCadGuarantorDetailsController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCadGuarantorDetailsController';
        ...
        var params =
       {
        ...
        $scope.Back = function () {
            if (lspage = 'CadDocumentChecklist') {
                $location.url('app/AgrTrnSuprCadDocChecklistSummary?application_gid=' + application_gid + '&lspage=CadDocumentChecklist');
            }
            else {
        ...
        $scope.institution_add = function (institution_gid) {
            $location.url('app/AgrTrnSuprCadDocumentChecklistAdd?application_gid=' + application_gid + '&credit_gid=' + institution_gid + '&lspage=' + lspage + '&lstype=Institution&lspath=' + lspath + '&lsfollowup=' + lsfollowup);
        }

        $scope.individual_add = function (contact_gid) {
            $location.url('app/AgrTrnSuprCadDocumentChecklistAdd?application_gid=' + application_gid + '&credit_gid=' + contact_gid + '&lspage=' + lspage + '&lstype=Individual&lspath=' + lspath + '&lsfollowup=' + lsfollowup);
        }

        $scope.group_add = function (group_gid) {
            $location.url('app/AgrTrnSuprCadDocumentChecklistAdd?application_gid=' + application_gid + '&credit_gid=' + group_gid + '&lspage=' + lspage + '&lstype=Group&lspath=' + lspath + '&lsfollowup=' + lsfollowup);
        }
        ...
        $scope.proceedtochecker = function () {
            ...
            var url = "api/AgrTrnSuprCAD/PostDocChecklistMakerSubmit";
            lockUI();
        ...
        $scope.proceedtoapprove = function () {
            ...
            var url = "api/AgrTrnSuprCAD/PostDocChecklistCheckerSubmit";
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status = true) {
