(function () {
    ...
    angular
        .module('angle')
        .controller('AgrMstCreditStageSummaryController', AgrMstCreditStageSummaryController);

    AgrMstCreditStageSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function AgrMstCreditStageSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrMstCreditStageSummaryController';
        lockUI();
        activate();
        function activate() {
        ...
        //    $location.url('app/AgrMstBusinessHierarchyUpdate?application_gid=' + application_gid + '&employee_gid=' + created_by + '&lspage=CreditStage');
            $location.url("app/AgrMstBusinessHierarchyUpdate?hash=" + cmnfunctionService.encryptURL("application_gid=" + application_gid + '&employee_gid=' + created_by + '&lspage=CreditStage'));
        ...
        //    $location.url('app/AgrMstBusinessHierarchyUpdateHistory?application_gid=' + application_gid + '&lspage=CreditStage');
            $location.url("app/AgrMstBusinessHierarchyUpdateHistory?hash=" + cmnfunctionService.encryptURL("application_gid=" + application_gid + '&lspage=CreditStage'));
