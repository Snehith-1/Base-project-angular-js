(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprRMDeferralDtlsController', AgrTrnSuprRMDeferralDtlsController);

        AgrTrnSuprRMDeferralDtlsController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function AgrTrnSuprRMDeferralDtlsController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprRMDeferralDtlsController';
        var application_gid = $location.search().application_gid;
        activate();

        function activate() {
            lockUI();
            var url = 'api/AgrMstSuprScannedDocument/tmpclearRMuploaded';
        ...
        var params =
       {
        ...
        $scope.institution_view = function (institution_gid) {
            $location.url('app/AgrTrnSuprRMDeferralDtlsView?application_gid=' + application_gid + '&credit_gid=' + institution_gid + '&lspage=RMDocChecklist&lstype=Institution');
        }

        $scope.individual_view = function (contact_gid) {
            $location.url('app/AgrTrnSuprRMDeferralDtlsView?application_gid=' + application_gid + '&credit_gid=' + contact_gid + '&lspage=RMDocChecklist&lstype=Individual');
        }

        $scope.group_view = function (group_gid) {
            $location.url('app/AgrTrnSuprRMDeferralDtlsView?application_gid=' + application_gid + '&credit_gid=' + group_gid + '&lspage=RMDocChecklist&lstype=Group');
        }
