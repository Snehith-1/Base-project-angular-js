(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditSuppliersDtlEditController', AgrTrnSuprCreditSuppliersDtlEditController);

    AgrTrnSuprCreditSuppliersDtlEditController.$inject = ['$rootScope', '$sce', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function AgrTrnSuprCreditSuppliersDtlEditController($rootScope, $sce, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditSuppliersDtlEditController';
        ...
        function activate() {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/GetSupplierList';
            SocketService.get(url).then(function (resp) {
                unlockUI();
                $scope.supplier_list = resp.data.supplier_list;
            });
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/EditGetCreditSupplier';
            ...
            SocketService.getparams(url, param).then(function (resp) {
                ...
                $scope.txtBankDebitamt = resp.data.bankdebit_amount;
                if($scope.txtBankDebitamt!=null && $scope.txtBankDebitamt!=undefined && $scope.txtBankDebitamt!="")
                {
                    $scope.txtannual_income_edit = (parseInt($scope.txtBankDebitamt.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountseperator = (parseInt($scope.txtannual_income_edit.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                    $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
        ...
        $scope.update_supplierdtl = function () {
            ...
            ($scope.txtBankDebitamt == undefined) || ($scope.txtrelationship_supplier == undefined)) {
                ...
                var url = 'api/AgrTrnSuprAppCreditUnderWriting/UpdateCreditSupplier';
                lockUI();
