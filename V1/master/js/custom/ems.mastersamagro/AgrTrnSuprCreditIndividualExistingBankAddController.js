(function () {
    ...
    angular
        .module('angle')
        .controller('AgrTrnSuprCreditIndividualExistingBankAddController', AgrTrnSuprCreditIndividualExistingBankAddController);

    AgrTrnSuprCreditIndividualExistingBankAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce','cmnfunctionService'];

    function AgrTrnSuprCreditIndividualExistingBankAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'AgrTrnSuprCreditIndividualExistingBankAddController';
        ...
        $scope.add_creditexistingbankdtl = function () {
            if (($scope.cboBankName == undefined) || ($scope.cboBankName == '')) {
                ...
                var url = 'api/AgrTrnSuprAppCreditUnderWriting/PostExistingBankFacility';
                lockUI();
        ...
        $scope.creditexistingbank_delete = function (existingbankfacility_gid) {
            ...
            var url = 'api/AgrTrnSuprAppCreditUnderWriting/DeleteExistingBankFacility';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.creditexistingbank_edit = function (existingbankfacility_gid) {
            $location.url('app/AgrTrnSuprCreditIndividualExistingBankEdit?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&existingbankfacility_gid=' + existingbankfacility_gid + '&lspage=' + lspage);
        }
        //KYC API
