(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADApplicationEditController', MstCADApplicationEditController);

    MstCADApplicationEditController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', '$sce', '$anchorScroll', 'DownloaddocumentService','cmnfunctionService'];

    function MstCADApplicationEditController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, $sce, $anchorScroll, DownloaddocumentService,cmnfunctionService) {

        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADApplicationEditController';
        ...
        const lspagename = 'MstCADApplicationEdit';
        ...
        function activate() {
            ...
            var url = 'api/MstCADApplication/GetGeneticDetailsView';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
            ...
            var url = 'api/MstCADApplication/GetIndividualList';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
            ...
            var url = 'api/MstCADApplication/GetInstitutionList';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
        ...
        $scope.PurposeofLoanOther_view = function (application2loan_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstCADApplication/GetPurposeofLoan';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                    $scope.txtpurposeof_loan = resp.data.enduse_purpose;
        ...
        $scope.Buyer_view = function (application2loan_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstCADApplication/GetLoantoBuyerList';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                    $scope.buyer_list = resp.data.mstbuyer_list;
        ...
        $scope.group_view = function (group_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCADGroupdtlView";
            window.open(URL, '_blank');
        }
        ...
        var url = "api/MstCADApplication/GetGroupSummary";
        lockUI();
        SocketService.getparams(url, params).then(function (resp) {
            unlockUI();
            ...
            angular.forEach($scope.group_list, function (value, key) {
                ...
                var url = 'api/MstCADApplication/GetGrouptoMemberList';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
        ...
        $scope.appcreainstitution_view = function (institution_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCADRMInstitutionView";
            window.open(URL, '_blank');
        }
        ...
        $scope.appcreaindividual_view = function (contact_gid) {
            var contact_gid = contact_gid;
            localStorage.setItem('contact_gid', contact_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCADRMIndividualView";
            window.open(URL, '_blank');
        }
        ...
        $scope.creditinstitution_view = function (institution_gid) {
            var institution_gid = institution_gid;
            localStorage.setItem('institution_gid', institution_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCADCreditCompanyDtlView?application_gid=" + application_gid + '&institution_gid='+ institution_gid + '&lspage='+ lspage +'&lspagetype='+ lspagetype;
            window.open(URL, '_blank');
        }
        ...
        $scope.creditindividual_view = function (contact_gid) {
            var contact_gid = contact_gid;
            localStorage.setItem('contact_gid', contact_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCADCreditIndividualDtlView?application_gid=" + application_gid + '&contact_gid='+ contact_gid + '&lspage='+ lspage +'&lspagetype='+ lspagetype;
            window.open(URL, '_blank');
        }
        ...
        $scope.creditgroup_view = function (group_gid) {
            var group_gid = group_gid;
            localStorage.setItem('group_gid', group_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCADCreditGroupDtlView?application_gid=" + application_gid + '&group_gid='+ group_gid + '&lspage='+ lspage +'&lspagetype='+ lspagetype;
            window.open(URL, '_blank');
        }
        ...
        $scope.creditinstitution_add = function (institution_gid) {
            $location.url('app/MstCADGuaranteeDetailAdd?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=' + lspage + '&lstype=Institution');
        }

        $scope.creditindividual_add = function (contact_gid) {
            $location.url('app/MstCADIndividualGuaranteeDtlAdd?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&lspage=' + lspage + '&lstype=Individual');
        }

        $scope.creditgroup_add = function (group_gid) {
            $location.url('app/MstCADGroupGuaranteeDtlAdd?application_gid=' + application_gid + '&group_gid=' + group_gid + '&lspage=' + lspage + '&lstype=Group');
        }
        ...
        $scope.cadgeneraldtl_edit = function () {
            $location.url('app/MstCADGeneralDtlEdit?application_gid=' + application_gid + '&lspage=' + lspage + '&product_gid=' + product_gid + '&variety_gid=' + variety_gid);
        }

        $scope.applcreainstitution_edit = function (institution_gid) {
            $location.url('app/MstCADInstitutionDtlEdit?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=' + lspage);
        }

        $scope.appcreaindividual_edit = function (contact_gid) {
            $location.url('app/MstCADIndividualDtlEdit?application_gid=' + application_gid + '&contact_gid=' + contact_gid + '&lspage=' + lspage);
        }

        $scope.appcreagroup_edit = function (group_gid) {
            $location.url('app/MstCADGroupDtlEdit?application_gid=' + application_gid + '&group_gid=' + group_gid + '&lspage=' + lspage);
        }
        ...
        $scope.hypothecation_edit = function () {
            $location.url('app/MstCADHypothecationEdit?application_gid=' + application_gid + '&lspage=' + lspage);
        }

        $scope.hypothecation_add = function () {
            $location.url('app/MstCadAppEditHypothecationAdd?application_gid=' + application_gid + '&lspage=' + lspage + '&product_gid=' + product_gid + '&variety_gid=' + variety_gid);
        }

        $scope.addcompany_dtl = function () {
            $location.url('app/MstCADInstitutionDtlAdd?application_gid=' + application_gid + '&lspage=' + lspage);
        }

        $scope.addindividual_dtl = function () {
            $location.url('app/MstCADIndividualDtlAdd?application_gid=' + application_gid + '&lspage=' + lspage);
        }

        $scope.addgroup_dtl = function () {
            $location.url('app/MstCADGroupDtlAdd?application_gid=' + application_gid + '&lspage=' + lspage);
        }
