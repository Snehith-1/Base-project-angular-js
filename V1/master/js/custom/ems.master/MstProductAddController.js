(function () {
    ...
    angular
        .module('angle')
        .controller('MstProductAddController', MstProductAddController);

    MstProductAddController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService', 'cmnfunctionService'];

    function MstProductAddController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams,DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstProductAddController';
        ...
        $scope.submit = function () {
            varietylist();
            if (($scope.variety_list == undefined) || ($scope.variety_list == '') || ($scope.variety_list == null)) {
                Notify.alert('Enter Variety Details', 'warning');
            }
            else {
                var lsbusinessunit_gid = '';
                var lsbusinessunit_name = '';
                if ($scope.cbosector != undefined || $scope.cbosector != null) {
                    lsbusinessunit_gid = $scope.cbosector.businessunit_gid;
                    lsbusinessunit_name = $scope.cbosector.businessunit_name;
                }

                var lsvaluechain_gid = '';
                var lsvaluechain_name = '';
                if ($scope.cbocategory != undefined || $scope.cbocategory != null) {
                    lsvaluechain_gid = $scope.cbocategory.valuechain_gid;
                    lsvaluechain_name = $scope.cbocategory.valuechain_name;
                }
                var lscategory_gid = '';
                var lscategory_name = '';
                if ($scope.cbocsacategory != undefined || $scope.cbocsacategory != null) {
                    lscategory_gid = $scope.cbocsacategory.category_gid;
                    lscategory_name = $scope.cbocsacategory.category_name;
                }
                ...
                var params = {
                    ...
                    valuechain_gid: lsvaluechain_gid,
                    valuechain_name: lsvaluechain_name,
                    category_gid: lscategory_gid,
                    category_name: lscategory_name,     
                }
                lockUI();
                var url = 'api/MstApplication360/CreateProducts';
        ...
        function varietylist() {
            var url = 'api/MstApplication360/GetVariety';
            SocketService.get(url).then(function (resp) {
                $scope.variety_list = resp.data.application_list;
            });
        }

        $scope.varity_delete = function (variety_gid) {
            var params =
                {
