(function () {
    ...
    angular
        .module('angle')
        .controller('Mstcustomer2userdtleditController', Mstcustomer2userdtleditController);

    Mstcustomer2userdtleditController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route','cmnfunctionService'];

    function Mstcustomer2userdtleditController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'Mstcustomer2userdtleditController';
        activate();
        ...
        function activate() {
            ...
            console.log(localStorage.getItem('customer_type'))
            $scope.txtcountry = "India";
            if (localStorage.getItem('customer_type') == 'Individual')
            {
                ...
                SocketService.getparams(url, params).then(function (resp) {
                    ...
                    $scope.txtindividualpan_no = resp.data.pan_no;
                    $scope.txtindividualaadhar_no = resp.data.aadhar_no;
                    $scope.txtcontactperson = resp.data.contact_person;
                    $scope.txtindividualtelephoneno = resp.data.telephone_no;
                    $scope.txtofficialemail_address = resp.data.officailemail_address;
                    ...
                    $scope.cboindividualuser_type = resp.data.usertype_gid;
                    $scope.txtdesignation = resp.data.contactperson_designation;
                ...
                SocketService.getparams(url, params).then(function (resp) {
                    ...
                    $scope.mobileno_list = resp.data.mobileno_list;
                    $scope.member_list = resp.data.member_list;
                    $scope.cbocompanytype = resp.data.customer_type;
                    $scope.txtinstitutiongst_no = resp.data.gst_no;
                    ...
                    $scope.cbouser_type = resp.data.usertype_gid;
        ...
        $scope.institutionmobileno_add = function () {
            ...
            if (($scope.txtmobile_no == undefined) || ($scope.txtmobile_no == '') || ($scope.rdbprimarymobile_no == undefined)) {
                ...
                SocketService.post(url, params).then(function (resp) {
                    ...
                    $scope.txtmobile_no = '';
                    $scope.rdbprimarymobile_no = null;
                    mobileno_list();
                });
            }
        }
        ...
        //--------Individul Update----------//
        $scope.individualupdate = function () {
            var user_type = $('#user_type :selected').text();
            if (($scope.txtindividualname == undefined) || ($scope.txtpersonalemail_address == undefined) || ($scope.txtindividualaadhar_no == undefined) || ($scope.txtindividualpan_no == undefined) || ($scope.cboindividualuser_type == "")) {
                $scope.Warning = true;
            }
            else {
                $scope.Warning = false;
        ...
        function customer2userlist() {
            ...
            SocketService.get(url).then(function (resp) {
                $scope.customer2userdtl_list = resp.data.customer2user_list;
            });
        }
        $scope.institutionupdate= function () {
            if (($scope.txtinstitution_name == undefined) || ($scope.txtcoi_date == undefined) || ($scope.txtcoi_date == "") || ($scope.txtcompanyemail_address == undefined) || ($scope.txtinstitutioncontact_name == undefined) || ($scope.cbocompanytype == undefined) || ($scope.txtinstitutionpan_no == undefined) || ($scope.txtinstitutiongst_no == undefined) || ($scope.cbouser_type == undefined) || ($scope.txtinstitutioncin_no == undefined)) {
                $scope.Warning = true;
            }
            else {
                $scope.Warning = false;
                ...
                var url = 'api/MstCustomerAdd/PostInstitutionUpdate';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.customerSubmit = function () {
            var params = {
                ...
                sa_idname: $scope.cbosa_idname.name,
                sa_payout: $scope.txtsa_payout,
