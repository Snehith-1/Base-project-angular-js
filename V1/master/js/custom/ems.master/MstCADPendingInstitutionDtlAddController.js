(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADPendingInstitutionDtlAddController', MstCADPendingInstitutionDtlAddController);

    MstCADPendingInstitutionDtlAddController.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', '$sce', '$anchorScroll', 'DownloaddocumentService','cmnfunctionService'];

    function MstCADPendingInstitutionDtlAddController($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, $sce, $anchorScroll, DownloaddocumentService,cmnfunctionService) {

        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADPendingInstitutionDtlAddController';
            ...
            if ($scope.rdburn_status == 'Yes' && ($scope.txt_urn == '' || $scope.txt_urn == undefined || $scope.txt_urn == null)) {
                ...
                SocketService.post(url, params).then(function (resp) {
                    ...
                    if (resp.data.status == true) {
                        ...
                        $location.url('app/MstCADPendingApplicationEdit?application_gid=' + application_gid + '&lstab=edit');
                    }
                    else {
                        Notify.alert(resp.data.message, {
                            status: 'warning',
        ...
        $scope.submit_institution = function () {
            if ($scope.rdburn_status == 'Yes' && ($scope.txt_urn == '' || $scope.txt_urn == undefined || $scope.txt_urn == null)) {
                ...
                if ($scope.cbointernalrating != undefined || $scope.cbointernalrating != null) {
                    lsinternalrating_gid = $scope.cbointernalrating.internalrating_gid;
                    lsinternalrating_name = $scope.cbointernalrating.internalrating_name;
                }
