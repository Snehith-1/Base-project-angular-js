(function () {
    ...
    angular
        .module('angle')
        .controller('MstCCMeetingRescheduleController', MstCCMeetingRescheduleController);

    MstCCMeetingRescheduleController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstCCMeetingRescheduleController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCCMeetingRescheduleController';
        ...
        function activate() {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                $scope.ccgroup_list = resp.data.ccgroup_lists;
                $scope.employee_list = resp.data.employee_lists;
        ...
        $scope.reschedule = function () {
            ...
            ($scope.txteditccmeeting_date == undefined) || ($scope.txteditccmeeting_date == '') || ($scope.txteditccmeeting_date == null)) {
                ...
                if (($scope.txteditstart_time == undefined) || ($scope.txteditend_time == undefined) || ($scope.txteditstart_time == "") || ($scope.txteditend_time == "")) {
                    ...
                    lockUI();
                    var url = 'api/MstCC/RecheduleMeeting';
                    SocketService.post(url, params).then(function (resp) {
