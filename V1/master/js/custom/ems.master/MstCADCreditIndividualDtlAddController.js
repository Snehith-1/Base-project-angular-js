(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADCreditIndividualDtlAddController', MstCADCreditIndividualDtlAddController);

    MstCADCreditIndividualDtlAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function MstCADCreditIndividualDtlAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADCreditIndividualDtlAddController';
        ...
        const lspagename = 'MstCADCreditIndividualDtlAdd';
        
        activate();
        function activate() {

            var url = 'api/MstCADCreditAction/GetIndividualBureauTempClear';
            lockUI();
        ...
        $scope.generateDetails = function () {
            ...
            if (bureauname_name == 'TransUnion') {
                ...
                var url = 'api/CADBureauAPI/GetTransUnionConsumerCreditInfo';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    ...
                    if (resp.data.status == true) {
                        ...
                        var url = 'api/MstCADCreditAction/CICUploadIndividualDocList';
                        SocketService.getparams(url, param).then(function (resp) {
            ...
            else if (bureauname_name == 'High Mark')
            {
                ...
                $scope.html_content = '';
                var url = 'api/CADBureauAPI/GetHighmarkCreditInfo';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.add_bureauDtl = function () {
            ...
            || ($scope.txtbureau_response == undefined)) {
                ...
                var url = 'api/MstCADCreditAction/PostCICUploadIndividual';
                lockUI();
        ...
        $scope.bureau_delete = function (contact2bureau_gid) {
            ...
            var url = 'api/MstCADCreditAction/DeleteContactBureau';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.highriskalert_view = function (contact2bureau_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/CADBureauAPI/GetHighRiskAlertDetails';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
      ...
      $scope.CICDocumentUpload = function (val, val1, name) {
          ...
          if ($scope.uploadfrm != undefined) {
              lockUI();
              var url = 'api/MstCADCreditAction/CICIndividualDocumentUpload';
      ...
      $scope.uploaddocumentcancel = function (tmpcicdocument_gid) {
          ...
          var url = 'api/MstCADCreditAction/CICUploadIndividualDocDelete';
          SocketService.getparams(url, params).then(function (resp) {
              if (resp.data.status == true) {
        ...
        $scope.bureau_edit = function (contact2bureau_gid) {
            $location.url('app/MstCADCreditIndividualBureauEdit?lscontact2bureau_gid=' + contact2bureau_gid + '&lscontact_gid=' + $scope.contact_gid + '&lsapplication_gid=' + application_gid + '&lspage=' + lspage);
        }

        $scope.bureau_view = function (contact2bureau_gid) {
            $location.url('app/MstCADCreditIndividualBureauView?lscontact2bureau_gid=' + contact2bureau_gid + '&lscontact_gid=' + $scope.contact_gid + '&lsapplication_gid=' + application_gid + '&lspage=' + lspage + '&lspagename=' + lspagename);
        }
