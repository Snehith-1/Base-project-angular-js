(function () {
    ...
    angular
        .module('angle')
        .controller('MstSanctionAcceptedController', MstSanctionAcceptedController);

    MstSanctionAcceptedController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService'];

    function MstSanctionAcceptedController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSanctionAcceptedController';

        activate();

        function activate() {
            lockUI();
            var url = "api/MstCAD/SanctionAcceptedSummary";
        ...
        $scope.view = function (val) {
            $location.url('app/MstCadApplicationView?application_gid=' + val + '&lspage=SanctionAcceptedCustomer');
        }

        $scope.approval_process = function (val) {
            $location.url('app/MstSanctionDtlSummary?application_gid=' + val + '&lspage=SanctionAcceptedCustomer');
        }

        $scope.checkerapprovalview = function (customer2sanction_gid, application_gid) {
            $location.url('app/MstSanctionDtlViewSummary?sanction_gid=' + customer2sanction_gid + '&application_gid=' + application_gid + '&lspage=SanctionAcceptedCustomer');
        }
