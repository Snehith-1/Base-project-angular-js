(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAVerificationIndividualPendingController', MstSAVerificationIndividualPendingController);

    MstSAVerificationIndividualPendingController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function MstSAVerificationIndividualPendingController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAVerificationIndividualPendingController';

        activate();

        function activate() {
            var url = 'api/MstSAOnboardingIndividual/GetSAVerfiyindividualPendingSummary';
            lockUI();
            SocketService.get(url).then(function (resp) {
                $scope.saOnboardSummary_list = resp.data.saOnboardSummary_list;
                unlockUI();
            });
            var url = 'api/MstSAOnboardingIndividual/GetSaAssignmentCounts';
            SocketService.get(url).then(function (resp) {
                unlockUI()
        ...
        $scope.saonboardingverification = function (sacontact_gid) {
            ...
            $location.url('app/MstSAVerificationAssignedIndividualView?hash=' + cmnfunctionService.encryptURL('lssacontact_gid=' + sacontact_gid + '&lspage=IndividualPending'));
        }
        ...
        $scope.samapping = function (sacontact_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submit = function () {
                    ...
                    var url = 'api/MstSAOnboardingIndividual/CreateSAMappingAssignment';
                    lockUI();
                ...
                var url = 'api/MstSAOnboardingIndividual/CreateSAmappingLog';
                lockUI();
                SocketService.getparams(url, param).then(function (resp) {
                    $scope.samappingassign_list = resp.data.samappingassign_list;
                    unlockUI();
                });
            }
        }
