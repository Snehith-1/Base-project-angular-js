(function () {
    ...
    angular
        .module('angle')
        .controller('MstDisbursementCheckerController', MstDisbursementCheckerController);

    MstDisbursementCheckerController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams','cmnfunctionService','DownloaddocumentService'];

    function MstDisbursementCheckerController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, cmnfunctionService,DownloaddocumentService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstDisbursementCheckerController';
        ...
        $scope.Back = function () {
            $location.url('app/MstMyDisbursementCheckerSummary?application_gid=' + application_gid);
        }
        ...
        $scope.approve = function () {
            ...
            var params = {
                ...
                application_gid: application_gid,
                rmdisbursementrequest_gid: rmdisbursementrequest_gid,
                disbursement_to: $scope.disbursement_to
            }
        ...
        //    var application_gid = $location.search().application_gid;
        //    var loandisbursement_date = $scope.loandisbursement_date;
        //    var editloandisbursement_date = $scope.editloandisbursement_date;
        //    var params = {
        //        application_gid: application_gid,
        ...
        //        updated_person: 'Checker',
        //        amounttobe_disbursed: $scope.amounttobe_disbursed,
        //        loandisbursement_date: editloandisbursement_date
        //    }
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submitapplicant_creditopsamt = function () {
                    var params = {
                        ...
                        application_gid: $location.search().application_gid,
                        rmdisbursementrequest_gid: $location.search().rmdisbursementrequest_gid,
                        checkerdisbursement_amount: $scope.txtapplicantdisbursement_amount
                    }
                    var url = 'api/MstCreditOpsApplication/CheckerApplicantDisbAmountUpdate';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.Updateapplicant_creditopsamt = function (lbldisbursementamount_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstCreditOpsApplication/GetCheckerApplicantDisbAmountView';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                    $scope.txtsamplemakerdisbursement_amount = resp.data.checkerdisbursement_amount;
                ...
                $scope.updateapplicant_creditopsamt = function () {
                    var params = {
                        ...
                        application_gid: $location.search().application_gid,
                        rmdisbursementrequest_gid: $location.search().rmdisbursementrequest_gid,
                        checkerdisbursement_amount: $scope.txteditapplicantcreditopsdisbursement_amount
                    }
                    var url = 'api/MstCreditOpsApplication/CheckerApplicantDisbAmountUpdate';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
        ...
        $scope.farmerdisbamt_exportexcel = function () {
            ...
            var url = 'api/MstCreditOpsApplication/ExportCheckerFarmerDisbAmount';
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == true) {
                    unlockUI();
        ...
        $scope.uploadFarmerDisAmt = function (val, val1, name) {
            ...
            if (!allowedExtensions.exec(filePath)) {
                ...
                frm.append('fileupload', item.file);
                frm.append('file_name', item.name);
                if (item.name.includes("Checker Individual Disbursement Amount Details")) {
                    frm.append('rmdisbursementrequest_gid', rmdisbursementrequest_gid);
                    frm.append('application_gid', application_gid);
                    frm.append('project_flag', "Default");
                    $scope.uploadfrm = frm;
        ...
        $scope.uploadExcelIndividual_DisbAmount = function () {
            if ($scope.fileinputvalue == '' || $scope.fileinputvalue == undefined || $scope.fileinputvalue == null) {
                ...
                var url = 'api/MstCreditOpsApplication/ImportExcelCheckerFarmerDisbAmount';
                lockUI();
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                    if (resp.data.status == true) {
                        unlockUI();
