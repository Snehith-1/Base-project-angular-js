(function () {
    ...
    angular
        .module('angle')
        .controller('MstGuarantorView', MstGuarantorView);

    MstGuarantorView.$inject = ['$rootScope', '$scope', '$state', '$modal', 'AuthenticationService', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route'];

    function MstGuarantorView($rootScope, $scope, $state, $modal, AuthenticationService, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route) {
        ...
        function activate() {
            var guarantor_id = localStorage.getItem('guarantor_id');
            $scope.guarantorid = guarantor_id;
            var params = {
        ...
        $scope.back = function () {
            $state.go('app.MstGuarantorSummary');
        }
