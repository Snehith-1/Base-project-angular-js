(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditCrimeCheckRecordAPIController', MstCreditCrimeCheckRecordAPIController);

    MstCreditCrimeCheckRecordAPIController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce'];

    function MstCreditCrimeCheckRecordAPIController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditCrimeCheckRecordAPIController';
        ...
        function activate() {
            ...
            var url = 'api/CrimeCheckAPI/GetCrimeRecordIndividualDetail';
            ...
            var url = 'api/CrimeCheckAPI/GetTaggedCaseIndividualSummary';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                $scope.tagcaseindividual_list = resp.data.tagcaseindividual_list;
                $scope.searchrecord_flag = resp.data.searchrecord_flag;
        ...
        $scope.CrimeCheck_Tag = function (data) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    ...
                    lockUI();
                    var url = 'api/CrimeCheckAPI/TagCaseIndividual';
        ...
        $scope.TaggedCase_Delete = function (crimecasetaggedcontact_gid) {
            ...
            var url = 'api/CrimeCheckAPI/DeleteTaggedCaseIndividual';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.search_crimecheck = function () {
            ...
            if ($scope.cboaddress == undefined || $scope.rdbSearchmode == undefined || searchIn == '') {
                ...
                var url = 'api/CrimeCheckAPI/GetCrimeRecordsIndividual';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    setTimeout(function () {
