(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADCreditBuyerDtlEditController', MstCADCreditBuyerDtlEditController);

        MstCADCreditBuyerDtlEditController.$inject = ['$rootScope', '$sce', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function MstCADCreditBuyerDtlEditController($rootScope, $sce, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADCreditBuyerDtlEditController';
        ...
        function activate() { 
            ...
            var url = 'api/MstCADCreditAction/EditGetCreditBuyer';
            ...
            SocketService.getparams(url, param).then(function (resp) {
                ...
                $scope.txtSampleBankCreditValue = resp.data.bankcredit_value;
                $scope.txtBankCreditValue = (parseInt($scope.txtSampleBankCreditValue.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountseperator = (parseInt($scope.txtBankCreditValue.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                $scope.lblamountwords = defaultamountwordschange($scope.lblamountseperator);
        ...
        $scope.buyerdtl_Back = function () {
            $location.url('app/MstCADCreditBuyerDtlAdd?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=CADAcceptanceCustomers');
        }
        ...
        $scope.update_buyerdtl = function () {
            ...
            ($scope.txtBankCreditValue == undefined) || ($scope.rdbDeductionat_Source == undefined) || ($scope.txtrelationship_borrower == undefined) || ($scope.txtEndUse_Monitoring == undefined))
           {
            ...
            Notify.alert('Buyer End Date Is Less Then Start Date', 'warning');
        }
          else {

            var buyer_Name = $('#BuyerName :selected').text();

             var params = {
                 creditbuyer_gid: creditbuyer_gid,
                 applicant_type:'Institution',
                 buyer_gid : $scope.cboBuyerName,
             ...
             var url = 'api/MstCADCreditAction/UpdateCreditBuyer';
            lockUI();
