...
(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAOnboardingIndividualVerificationController', MstSAOnboardingIndividualVerificationController);

    MstSAOnboardingIndividualVerificationController.$inject = ['DownloaddocumentService', '$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'cmnfunctionService', 'ngTableParams'];

    function MstSAOnboardingIndividualVerificationController(DownloaddocumentService, $rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, cmnfunctionService, ngTableParams) {
    /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAOnboardingIndividualVerificationController';
        ...
        $scope.back=function(){
            ...
            if (lspage == 'IndividualPending') {
                $location.url('app/MstSAVerificationIndPendingSummary');
            }
        ...
        $scope.checkerindividualraisequery = function (sacontact_gid) {
            var modalInstance = $modal.open({
                templateUrl: '/checkerindividualClose.html',
                controller: ModalInstanceCtrl,
                backdrop: 'static',
                keyboard: false,
                size: 'md'
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submit = function () {
                    ...
                    var url = 'api/MstSAOnboardingIndividual/PostCheckerIndividualRaiseQuery';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
                            ...
                            //activate();
                            checkerraise_list(sacontact_gid);
                        }
                        else {
                            Notify.alert(resp.data.message, {
                                status: 'info',
        ...
        function checkerraise_list(sacontact_gid) {
            var params = {
                sacontact_gid: sacontact_gid,
        ...
        $scope.reject = function () {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.rejectSubmit = function () {
                    ...
                    var url = 'api/MstSAOnboardingIndividual/IndividualCheckerRejected';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
