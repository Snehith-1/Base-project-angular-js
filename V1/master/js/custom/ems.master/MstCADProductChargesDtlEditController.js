(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADProductChargesDtlEditController', MstCADProductChargesDtlEditController);

        MstCADProductChargesDtlEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function MstCADProductChargesDtlEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADProductChargesDtlEditController';
        ...
        function activate() {
            ...
            var url = 'api/MstCADApplication/GetEditLimit';
            ...
            var url = 'api/MstCADApplication/LoanDetailList';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
     ...
     /*       var url = 'api/MstCADApplication/BuyerDetailsList';
            ...
            var url = 'api/MstCADApplication/CollateralDetailsList';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.Collateral_list = resp.data.collatertal_list;
            });

            var url = 'api/MstCADApplication/HypothecationDetailsList';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.Hypothecation_list = resp.data.hypothecation_list;
            });
            var url = 'api/MstCADApplication/GetEditproduct';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.product_list = resp.data.product_list;
            });
            var url = 'api/MstCADApplication/GetProductChargesEdit';
                ...
                if ($scope.txtfacilityreqon_date == null || $scope.txtfacilityreqon_date == '' || $scope.cboProductTypelist == null || $scope.cboProductTypelist == '' || $scope.cboProductSubTypelist == null || $scope.cboProductSubTypelist == '' || $scope.cboLoanTypelist == null || $scope.cboLoanTypelist == '' || $scope.txtloanfaility_amount == null || $scope.txtloanfaility_amount == '' || $scope.txtrate_interest == null || $scope.txtrate_interest == '' || $scope.txtpanel_interest == null || $scope.txtpanel_interest == '' || $scope.txttenure_year == null || $scope.txttenure_year == '' || $scope.txttenure_month == null || $scope.txttenure_month == '' || $scope.cboFacilityTypelist == null || $scope.cboFacilityTypelist == '' || $scope.txttenure_days == null || $scope.txttenure_days == '' || $scope.cboFacilitymodelist == null || $scope.cboFacilitymodelist == '') {
                    ...
                    if ($scope.rdbmoratorium_status == 'Yes') {
                        if ($scope.cbomoratorium_type == null || $scope.cbomoratorium_type == '' || $scope.txtmoratorium_startdate == null || $scope.txtmoratorium_startdate == '' || $scope.txtmoratorium_enddate == null || $scope.txtmoratorium_enddate == '') {
                            ...
                            var url = 'api/MstCADApplication/PostLoanEditDtl';
                            lockUI();
        ...
        $scope.creditproductchargesloan_edit = function (application2loan_gid, product_gid, variety_gid) {
            $location.url('app/MstCADLoanDtlEdit?application2loan_gid=' + application2loan_gid + '&application_gid=' + $scope.application_gid + '&lspage=' + lspage + '&product_gid=' + product_gid + '&variety_gid=' + variety_gid);
        }
        ...
        $scope.uploadhypothecationdoc = function (val, val1, name) {
            if (($scope.cbohypodoc_title == null) || ($scope.cbohypodoc_title == '') || ($scope.cbohypodoc_title == undefined)) {
                ...
                $scope.uploadfrm = frm;
                lockUI();
                var url = 'api/MstCADApplication/EditHypoDoc';
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                    unlockUI();
                    $("#file").val('');
                    if (resp.data.status == true) {
        ...
        $scope.update_Hypothecationdtl = function () {
            ...
            var url = 'api/MstCADApplication/HypothecationDetailsUpdate';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.delete_Hypothecationdtls = function (application2hypothecation_gid) {
            ...
            var url = 'api/MstCADApplication/DeleteHypothecationDetails';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.downloads = function (val1, val2) {
         ...
         $scope.update_productcharge = function () {
            ...
            var url = 'api/MstCADApplication/UpdateProductCharges';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.SubmitOverallLimit = function () {
            ...
            var sector_Name = $('#Sector :selected').text();
             ...
             var url = 'api/MstCADApplication/UpdateOverallLimit';
             lockUI();
             SocketService.post(url, params).then(function (resp) {
                 unlockUI();
                 if (resp.data.status == true) {
         ...
         $scope.Deleteloan = function (application2loan_gid, producttype_gid) {
             ...
             var url = 'api/MstCADApplication/DeleteLoan';
             lockUI();
             SocketService.post(url, params).then(function (resp) {
                 unlockUI();
                 if (resp.data.status == true) {
         ...
         $scope.DeleteCharge = function (application2servicecharge_gid) {
             ...
             var url = 'api/MstCADApplication/DeleteCharge';
             lockUI();
             SocketService.getparams(url, params).then(function (resp) {
                 unlockUI();
                 if (resp.data.status == true) {
         ...
         $scope.SubmitServiceCharges = function () {
             ...
             var url = 'api/MstCADApplication/PostEditServiceCharges';
             lockUI();
             SocketService.post(url, params).then(function (resp) {
                 unlockUI();
                 if (resp.data.status == true) {
        ...
        $scope.servicedtl_edit = function (application2servicecharge_gid) {
            $location.url('app/MstCADServicesDtlEdit?application2servicecharge_gid=' + application2servicecharge_gid + '&application_gid=' + $scope.application_gid + '&lspage=' + lspage);
        }
