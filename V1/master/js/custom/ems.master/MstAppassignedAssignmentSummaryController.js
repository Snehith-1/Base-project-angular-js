(function () {
    ...
    angular
        .module('angle')
        .controller('MstAppassignedAssignmentSummaryController', MstAppassignedAssignmentSummaryController);

    MstAppassignedAssignmentSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstAppassignedAssignmentSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstAppassignedAssignmentSummaryController';

        activate();

        function activate() {
            var url = 'api/MstApplicationAdd/GetAppAssignedAssignmentSummary';
        ...
        $scope.GradingTool_add = function (val) {
            $location.url('app/MstCreditAssessedScoreAdd?application_gid=' + val + '&lstab=AssignmentSummary');
        }

        $scope.VisitReport_add = function (val) {
            $location.url('app/MstCreditVisitReportAdd?application_gid=' + val + '&lstab=AssignmentVisit');
        }

        $scope.applcreation_view = function (val) {
            $location.url('app/MstApplicationCreationView?application_gid=' + val + '&lstab=Asscreditmapping');
        }
        ...
        $scope.reassignapplication = function (creditgroup_gid, application_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.reassign_application = function () {
                    ...
                    //if ($scope.creditgroup_name != undefined || $scope.creditgroup_name != null) {
                    //    lscreditgroup_gid = $scope.creditgroup_name;
                    //    lscreditgroup_name = $scope.creditgrouplist.find(function (x) { return x.employee_gid === $scope.cbocreditgroup }).creditgroup_name;
                    //    if (lscreditgroup_name)
                    //        lscreditgroup_name = lscreditgroup_name.creditgroup_name;
                    //    else
                    //        lscreditgroup_name = $scope.cbocreditmanager;
                    //}
