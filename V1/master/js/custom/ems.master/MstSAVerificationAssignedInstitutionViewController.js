(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAVerificationAssignedInstitutionViewController', MstSAVerificationAssignedInstitutionViewController);

    MstSAVerificationAssignedInstitutionViewController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstSAVerificationAssignedInstitutionViewController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAVerificationAssignedInstitutionViewController';
        ...
        $scope.back = function () {
            $state.go('app.MstSAVerificationPending');
        }
        ...
        $scope.uploaddownloadall1 = function (val1, val2) {
        ...
        $scope.upload_downloads = function (val1, val2) {
            DownloaddocumentService.Downloaddocument(val1, val2);
        }
        $scope.uploaddocument_viewer = function (val1, val2) {
            lockUI();
        ...
        $scope.Verify_Institution = function () {
            ...
            SocketService.post(url, params).then(function (resp) {
                ...
                if (resp.data.status == true) {
                    ...
                    $location.url('app/MstSAOnboardingBussDevtVerificationSummary');
                }
                else {
                    Notify.alert(resp.data.message, {
                        status: 'warning',
