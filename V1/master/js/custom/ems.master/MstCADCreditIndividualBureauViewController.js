(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADCreditIndividualBureauViewController', MstCADCreditIndividualBureauViewController);

    MstCADCreditIndividualBureauViewController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService', 'cmnfunctionService'];

    function MstCADCreditIndividualBureauViewController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADCreditIndividualBureauViewController';
        ...
        function activate() {
            ...
            var url = 'api/MstCADCreditAction/CICIndividualEdit';
        ...
        //     $location.url('app/MstCADCreditIndividualDtlAdd?application_gid=' + $scope.application_gid + '&contact_gid=' + $scope.contact_gid + '&lspage=' + $scope.lspage);
        // }
        $scope.CICUploads_Back = function () {
            if(lspagename=='MstCADCreditIndividualDtlAdd')
                $location.url('app/MstCADCreditIndividualDtlAdd?application_gid=' + $scope.application_gid + '&contact_gid=' + $scope.contact_gid + '&lspage=' + $scope.lspage );
            
            else if(lspagename=='MstCADCreditIndividualDtlView')
                $location.url('app/MstCADCreditIndividualDtlView?application_gid=' + $scope.application_gid + '&contact_gid='+ $scope.contact_gid + '&lspage='+ $scope.lspage + '&lspagename=' + lspagename + '&lspagetype=' + lspagetype );
        
            else if(lspagename=='MstCadIndividualView')
                $location.url('app/MstCadIndividualView?application_gid=' + $scope.application_gid + '&contact_gid='+ $scope.contact_gid + '&lspage='+ $scope.lspage + '&lspagename=' + lspagename + '&lspagetype=' + lspagetype);
