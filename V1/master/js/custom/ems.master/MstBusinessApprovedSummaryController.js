(function () {
    ...
    angular
        .module('angle')
        .controller('MstBusinessApprovedSummaryController', MstBusinessApprovedSummaryController);

        MstBusinessApprovedSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstBusinessApprovedSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstBusinessApprovedSummaryController';
        lockUI();
        activate();

        function activate() {
        
            var url = 'api/MstApplicationApproval/GetApplicationApprovedSummary';
        ...
        $scope.business_approval = function (application_gid, employee_gid, applicationapproval_gid) {
            $location.url('app/MstBusinessApproval?application_gid=' + application_gid + '&employee_gid=' + employee_gid + '&applicationapproval_gid=' + applicationapproval_gid + '&lspage=BusinessApproved' + '&lsflag=N' + '&lsccmeetingflag=Y');
        }
