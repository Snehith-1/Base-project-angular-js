(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAVerificationPendingController', MstSAVerificationPendingController);

    MstSAVerificationPendingController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$filter', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstSAVerificationPendingController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $filter, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAVerificationPendingController';


        activate();

        function activate() {
            var url = 'api/MstSAOnboardingInstitution/GetSAVerfiyinstitutionPendingSummary';
            lockUI();
            SocketService.get(url).then(function (resp) {
                $scope.saOnboardSummary_list = resp.data.saOnboardSummary_list;
                unlockUI();
            });
            var url = 'api/MstSAOnboardingInstitution/GetSaAssignmentCounts';
            SocketService.get(url).then(function (resp) {
                unlockUI()
        ...
        $scope.saverificationedit = function (sacontactinstitution_gid) {
            ...
            $location.url('app/MstSAVerificationAssignedInstitutionView?hash=' + cmnfunctionService.encryptURL('lssacontactinstitution_gid=' + sacontactinstitution_gid));
        }
        $scope.saonboardingmapping = function (sacontactinstitution_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submit = function () {
                    ...
                    var url = 'api/MstSAOnboardingInstitution/CreateSAMappingAssignment';
                    lockUI();
                ...
                var url = 'api/MstSAOnboardingInstitution/CreateSAmappingLog';
                lockUI();
                SocketService.getparams(url, param).then(function (resp) {
                    $scope.samappingassign_list = resp.data.samappinginstitutionassign_list;
                    unlockUI();
                });
            }
        }
