(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAVerificationMakerIndInstSummaryController', MstSAVerificationMakerIndInstSummaryController);

    MstSAVerificationMakerIndInstSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstSAVerificationMakerIndInstSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAVerificationMakerIndInstSummaryController';

        activate();

        function activate() {
            var url = 'api/MstSAOnboardingIndividual/GetSAVerifyMakerIndividualInitiatedSummary';
            lockUI();
        ...
        $scope.saonboardingverification = function (sacontact_gid) {
            ...
            $location.url('app/MstSAVerificationMakerIndividualInitiated?hash=' + cmnfunctionService.encryptURL('lssacontact_gid=' + sacontact_gid + '&lspage=IndividualInitiate'));
        }
    }
})();
