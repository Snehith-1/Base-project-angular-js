(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditVisitReportAddController', MstCreditVisitReportAddController);

    MstCreditVisitReportAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstCreditVisitReportAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditVisitReportAddController';
      ...
      /*  var lstab = $scope.lstab;*/
       lockUI();
        ...
        $scope.edit_creditvisitreport = function (val) {         
            $location.url('app/MstCreditVisitReportEdit?visitreport_gid=' + val + '&application_gid=' + application_gid + '&lstab=' + lstab);  

           
        }

        $scope.Back = function () {
           /* $location.url('app/MstAppassignedAssignmentSummary?application_gid=' + application_gid);*/
            if (lstab == 'AssignmentVisit') {
                $state.go('app.MstAppassignedAssignmentSummary');
            }
            else if (lstab == 'AssessedScore') {
                $state.go('app.MstMyApplicationsSummary');
            }
            else if (lstab == 'InProgress') {
                $state.go('app.MstMyApplicationsSummary');
            }
        }
        ...
        function address_list() {
            var url = 'api/MstApplicationVisitReport/GetVisittmpAddressList';
            SocketService.get(url).then(function (resp) {
        ...
        function visitedpersondetails_list() {
            var url = 'api/MstApplicationVisitReport/GetLVisittmpPersondtlList';
            SocketService.get(url).then(function (resp) {
        ...
        $scope.editaddress = function (val) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.getGeoCoding = function () {
                    if($scope.txteditpostal_code == undefined){
                        $scope.txtlatitude = '';
                        $scope.txtlongitude = '';
                    }
        ...
        $scope.save_visitreport = function () {
            ...
            var url = 'api/MstApplicationVisitReport/PostApplicationVisitReport';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    ...
                    var url = 'api/MstApplicationVisitReport/GetVisittmpDocumentList';
                    SocketService.get(url).then(function (resp) {
                        $scope.UploadDocumentList = resp.data.UploadDocumentList;

                    });
                    var url = 'api/MstApplicationVisitReport/GetVisittmpPhotosList';
                    SocketService.get(url).then(function (resp) {
        ...
        $scope.submit_visitreport = function () {
            ...
            var url = 'api/MstApplicationVisitReport/PostSubmitApplicationVisitReport';
            lockUI();
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.view_RMVisitReport = function (visitreport_gid) {
            ...
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstRMVisitReportView";
        ...
        $scope.view_creditvisitreport = function (visitreport_gid) {
            ...
            localStorage.setItem('visitreport_gid', visitreport_gid);
            localStorage.setItem('application_gid', application_gid);
            var URL = location.protocol + "//" + location.hostname + "/v1/#/app/MstCreditVisitReportView";
            window.open(URL, '_blank');
        }
