(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADGroupProcessAssignController', MstCADGroupProcessAssignController);

    MstCADGroupProcessAssignController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams'];

    function MstCADGroupProcessAssignController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADGroupProcessAssignController';
        ...
        function activate() {
            ...
            var url = 'api/MstCAD/GetProcessTypeSummary';
            SocketService.getparams(url, params).then(function (resp) {
        ...
        $scope.Process_add = function () {
            if (($scope.cboprocesstype == 'Sendback to CC' || $scope.cboprocesstype == 'Sendback to Credit Underwriting') && ($scope.remarks == '' || $scope.remarks == null)) {
                ...
                if ($scope.rdbassign == 'Menuselection') {
                    ...
                    var url = "api/MstCAD/PostProcessType";
                    lockUI();
        ...
        $scope.Submit = function () {
            ...
            if (($scope.cboprocesstype == 'Sendback to CC' || $scope.cboprocesstype == 'Sendback to Credit Underwriting' || $scope.cboprocesstype == 'Sendback to Credit Without CC') && ($scope.remarks == '' || $scope.remarks == null)) {
                ...
                if ($scope.mdlprocesstype_list != null) {
                    ...
                    var url = "api/MstCAD/UpdateProcessType";
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status = true) {
                    ...
                    var url = "api/MstCAD/PostRevertCADtoCC";
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status = true) {
                            ...
                            var url = "api/MstCAD/SentBackUpdateProcessType";
                            lockUI();
                            SocketService.post(url, params).then(function (resp) {
                                unlockUI();
                                if (resp.data.status = true) {
                    ...
                    var url = "api/MstCAD/PostRevertCADtoCredit";
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status = true) {
        ...
        $scope.delete = function (processtypeassign_gid) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    var url = 'api/MstCAD/DeleteCADAssignment';
                  lockUI();
                  SocketService.getparams(url, params).then(function (resp) {
                      unlockUI();
                        if (resp.data.status == true) {
