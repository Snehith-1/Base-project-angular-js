(function () {
    ...
    angular
        .module('angle')
        .controller('MstDigitalSignatureController', MstDigitalSignatureController);

    MstDigitalSignatureController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function MstDigitalSignatureController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, $route, ngTableParams, $sce, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstDigitalSignatureController';
        ...
        function activate() {
            lockUI();
            var url = 'api/MstApplication360/GetDigitalSignatureList';
        ...
        $scope.addSignature = function () {
            ...
            function ModalInstanceCtrl($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, $route, ngTableParams) {
                ...
                var url = 'api/MstApplication360/GetEmployeeList';
                SocketService.get(url).then(function (resp) {
                ...
                $scope.signaturesubmit = function () {
                    var url = 'api/MstApplication360/SignatureUpload';
                    lockUI();
        ...
        $scope.deletesignature = function (digitalsignature_gid) {
            ...
            var url = 'api/MstApplication360/DeleteSignature';
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == true) {
