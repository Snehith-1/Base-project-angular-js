(function () {
    ...
    angular
        .module('angle')
        .controller('MstDisbursementMakerController', MstDisbursementMakerController);

    MstDisbursementMakerController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams','cmnfunctionService','DownloaddocumentService'];

    function MstDisbursementMakerController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, cmnfunctionService,DownloaddocumentService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstDisbursementMakerController';
        ...
        function activate() {
            ...
            SocketService.getparams(url, params).then(function (resp) {
                ...
                $scope.txtprocessing_fees = resp.data.processing_fees;
                $scope.txtgst = resp.data.gst;
                $scope.txtadditional_charges = resp.data.finance_charges;
                $scope.txtod_amount = resp.data.od_amount;
                $scope.rdbpayment_escrow = resp.data.escrow_payment;
                $scope.rdbnach_status = resp.data.nach_status;
                ...
                $scope.lbldisbursement_to = resp.data.disbursement_to;
        ...
        $scope.Back = function () {
            $location.url('app/MstMyDisbursementSummary?application_gid=' + application_gid);
        }
       ...
       $scope.downloadall = function () {
        ...
        $scope.proceedtochecker = function () {            
            ...
            var params = {
                ...
                maker_remarks: $scope.txtmaker_remarks,
                updated_person: 'Maker',
                application_gid: application_gid,
                rmdisbursementrequest_gid: rmdisbursementrequest_gid,
                disbursement_to: $scope.lbldisbursement_to
            }

            var url = "api/MstCreditOpsApplication/PostDisbursementMakerApprove";
            lockUI();
        ...
        $scope.Updatesupplier_creditopsamt = function (disbursementsupplier_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstCreditOpsApplication/GetCreditOpsSupplierDisbAmountView';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                    $scope.txtsampleeditsupplierdisbursement_amount = resp.data.creditopsdisbursement_amount;
                ...
                $scope.updatesupplier_creditopsamt = function () {
                    var params = {
                        disbursementsupplier_gid: disbursementsupplier_gid,
                        creditopsdisbursement_amount:$scope.txteditsupplierdisbursement_amount,
                        application_gid: $location.search().application_gid,
                    }
                    var url = 'api/MstCreditOpsApplication/CreditOpsSupplierDisbAmountUpdate';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.addsupplier_creditopsamt = function (disbursementsupplier_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submitsupplier_creditopsamt = function () {
                    var params = {
                        disbursementsupplier_gid: disbursementsupplier_gid,
                        creditopsdisbursement_amount: $scope.txtsupplierdisbursement_amount,
                        application_gid: $location.search().application_gid,
                    }
                    var url = 'api/MstCreditOpsApplication/CreditOpsSupplierDisbAmountUpdate';
                    lockUI();
        ...
        $scope.addfarmer_creditopsamt = function (farmercontact_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submitfarmer_creditopsamt = function () {
                    var params = {
                        farmercontact_gid: farmercontact_gid,
                        creditopsdisbursement_amount: $scope.txtfarmerdisbursement_amount,
                        application_gid: $location.search().application_gid,
                    }
                    var url = 'api/MstCreditOpsApplication/CreditOpsFarmerDisbAmountUpdate';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.Updatefarmer_creditopsamt = function (farmercontact_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstCreditOpsApplication/GetCreditOpsFarmerDisbAmountView';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                    $scope.txtsampleeditfarmerdisbursement_amount = resp.data.creditopsdisbursement_amount;
                ...
                $scope.updatefarmer_creditopsamt = function () {
                    var params = {
                        farmercontact_gid: farmercontact_gid,
                        creditopsdisbursement_amount: $scope.txteditFarmerdisbursement_amount,
                        application_gid: $location.search().application_gid,
                    }
                    var url = 'api/MstCreditOpsApplication/CreditOpsFarmerDisbAmountUpdate';
                    lockUI();
        ...
        $scope.add_bankaccountdtl = function () {
            ...
            if (($scope.txtIFSC_Code == undefined) || ($scope.txtIFSC_Code == '') || ($scope.txtbankacct_no == undefined) ||
                ($scope.txtbankacct_no == '') || ($scope.txtconfirmbankacct_no == undefined) || ($scope.txtconfirmbankacct_no == '') ||
                ($scope.txtacctholder_name == undefined) ||
        ...
        $scope.addapplicant_disbursementamt = function (lbldisbursementamount_gid) {
            var modalInstance = $modal.open({
                templateUrl: '/AddApplicantCreditOpsAmount.html',
                controller: ModalInstanceCtrl,
                backdrop: 'static',
                keyboard: false,
                size: 'md'
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.txtamountchange74 = function () {
                    var input = document.getElementById('ApplicantDisbursementCreditOps_amount').value;
                    var str1 = input.replace(/,/g, '');
                    var str = Math.round(str1);
                    var output = Number(str).toLocaleString('en-IN');
                    var words_totalamount74 = cmnfunctionService.fnConvertNumbertoWord(str);
                    if (output == "NaN") {
                        Notify.alert('Accept Number Format Only..!', {
                            status: 'danger',
                            pos: 'top-center',
                        ...
                        $scope.txtapplicantdisbursement_amount = "";
                    }
                    else {
                        //   $scope.txtprocessing_fee = output;
                        document.getElementById('words_totalamount74').innerHTML = words_totalamount74;
                        var txtapplicantdisbursement_amount = parseInt($scope.txtapplicantdisbursement_amount.replace(/[\s,]+/g, '').trim());
                    }
                    $scope.txtapplicantdisbursement_amount = (parseInt(input.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                ...
                $scope.submitapplicant_creditopsamt = function () {
                    var params = {
                        disbursementamount_gid: lbldisbursementamount_gid,
                        application_gid: $location.search().application_gid,
                        rmdisbursementrequest_gid: $location.search().rmdisbursementrequest_gid,
                        makerdisbursement_amount: $scope.txtapplicantdisbursement_amount
                    }
                    var url = 'api/MstCreditOpsApplication/CreditOpsApplicantDisbAmountUpdate';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.Updateapplicant_creditopsamt = function (lbldisbursementamount_gid) {
            var modalInstance = $modal.open({
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                    $scope.txtsamplemakerdisbursement_amount = resp.data.makerdisbursement_amount;
                    $scope.txteditapplicantcreditopsdisbursement_amount = (parseInt($scope.txtsamplemakerdisbursement_amount.replace(/[^\d]+/gi, '')) || 0).toLocaleString('en-IN');
                ...
                $scope.updateapplicant_creditopsamt = function () {
                    var params = {
                        ...
                        application_gid: $location.search().application_gid,
                        rmdisbursementrequest_gid: $location.search().rmdisbursementrequest_gid,
                        makerdisbursement_amount: $scope.txteditapplicantcreditopsdisbursement_amount
                    }
                    var url = 'api/MstCreditOpsApplication/CreditOpsApplicantDisbAmountUpdate';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
        ...
        $scope.applicantbankdtl_edit = function (disbapplicantbankdtl_gid) {
            lockUI();
        ...
        $scope.chequeleafdocumentUploadedit = function (val, disbapplicantbankdtl_gid) {
            if (($scope.txtdocument_title == null) || ($scope.txtdocument_title == '') || ($scope.txtdocument_title == undefined)) {
                $("#chequefilefile").val('');
                Notify.alert('Kindly Enter the Document Title', 'warning');
            } else {
        ...
        $scope.farmerdisbamt_exportexcel = function () {
            var rmdisbursementrequest_gid = $location.search().rmdisbursementrequest_gid;
            var application_gid = $location.search().application_gid;
            lockUI();
            var params = {
            ...
            var url = 'api/MstCreditOpsApplication/ExportFarmerDisbAmount';
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == true) {
                    unlockUI();
        ...
        $scope.uploadFarmerDisAmt = function (val, val1, name) {
            ...
            if (!allowedExtensions.exec(filePath)) {
                ...
                frm.append('fileupload', item.file);
                frm.append('file_name', item.name);
                if (item.name.includes("Individual Disbursement Amount Details")) {
                    frm.append('rmdisbursementrequest_gid', rmdisbursementrequest_gid);
                    frm.append('application_gid', application_gid);
                    frm.append('project_flag', "Default");
                    $scope.uploadfrm = frm;
        ...
        $scope.uploadExcelIndividual_DisbAmount = function () {
            if ($scope.fileinputvalue == '' || $scope.fileinputvalue == undefined || $scope.fileinputvalue == null) {
                ...
                var url = 'api/MstCreditOpsApplication/ImportExcelFarmerDisbAmount';
                lockUI();
                SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                    if (resp.data.status == true) {
                        unlockUI();
