(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAOnboardingApprovalInsSummaryController', MstSAOnboardingApprovalInsSummaryController);

    MstSAOnboardingApprovalInsSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstSAOnboardingApprovalInsSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
    /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAOnboardingApprovalInsSummaryController';

        activate();

        function activate() { 
            var url = 'api/MstSAOnboardingBussDevtVerification/GetSaApprovalInitiatedSummary';
            lockUI();
        ...
        $scope.saonboardingverification = function (sacontactinstitution_gid) {
            ...
            $location.url('app/MstSAOnboardingInstitutionApproval?hash=' + cmnfunctionService.encryptURL('lssacontactinstitution_gid=' + sacontactinstitution_gid + '&lspage=InstituteInitiate' ));
        }
    }
})();
