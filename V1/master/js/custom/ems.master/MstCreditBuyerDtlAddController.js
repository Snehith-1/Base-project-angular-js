(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditBuyerDtlAddController', MstCreditBuyerDtlAddController);

        MstCreditBuyerDtlAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','$sce','cmnfunctionService'];

    function MstCreditBuyerDtlAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce,cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditBuyerDtlAddController';
        ...
        $scope.add_creditbuyerdtl = function () {
            ...
            ($scope.txtBankCreditValue == '') || ($scope.rdbDeductionat_Source == ''))
           {
             ...
             var url = 'api/MstAppCreditUnderWriting/PostCreditBuyer';
              lockUI();
        ...
        var url = 'api/MstAppCreditUnderWriting/DeleteCreditBuyer';
        lockUI();
        SocketService.getparams(url, params).then(function (resp) {
            unlockUI();
            if (resp.data.status == true) {
        ...
        $scope.creditbuyer_edit = function (creditbuyer_gid) {
            $location.url('app/MstCreditBuyerDtlEdit?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&creditbuyer_gid=' + creditbuyer_gid + '&lspage=' + lspage);
        }
