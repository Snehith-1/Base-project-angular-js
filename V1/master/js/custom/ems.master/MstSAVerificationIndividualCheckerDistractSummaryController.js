(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAVerificationIndividualCheckerDistractSummaryController', MstSAVerificationIndividualCheckerDistractSummaryController);

    MstSAVerificationIndividualCheckerDistractSummaryController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'cmnfunctionService'];

    function MstSAVerificationIndividualCheckerDistractSummaryController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAVerificationIndividualCheckerDistractSummaryController';

        activate();

        function activate() {
            var url = 'api/MstSAOnboardingIndividual/GetSAVerfiyIndividualCheckerDistractSummary';
            lockUI();
        ...
        $scope.saonboardingverification = function (sacontact_gid) {
            ...
            $location.url('app/MstSAVerificationIndividualCheckerTrashView?hash=' + cmnfunctionService.encryptURL('lssacontact_gid=' + sacontact_gid + '&lspage=IndividualPending'));
        }
    }
})();
