(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADPSLCSAGuarantorDetailsController', MstCADPSLCSAGuarantorDetailsController);

        MstCADPSLCSAGuarantorDetailsController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams'];

    function MstCADPSLCSAGuarantorDetailsController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADPSLCSAGuarantorDetailsController';
        ...
        var params =
       {
        ...
        $scope.institution_add = function (institution_gid) { 
            $location.url('app/MstCADPSLCSADataFlaggingAdd?application_gid=' + application_gid + '&credit_gid=' + institution_gid + '&lspage=' + lspage + '&lstype=Institution&lspath=' + lspath);
        }

        $scope.individual_add = function (contact_gid) {
            $location.url('app/MstCADPSLCSAIndividualPSLDataFlagAdd?application_gid=' + application_gid + '&credit_gid=' + contact_gid + '&lspage=' + lspage + '&lstype=Individual&lspath=' + lspath);
        }

        $scope.group_add = function (group_gid) {
            $location.url('app/MstCADPSLCSAGroupPSLDataFlagAdd?application_gid=' + application_gid + '&group_gid=' + group_gid + '&lspage=' + lspage + '&lstype=Group&lspath=' + lspath);
        }
