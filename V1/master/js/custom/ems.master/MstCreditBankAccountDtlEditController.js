(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditBankAccountDtlEditController', MstCreditBankAccountDtlEditController);

    MstCreditBankAccountDtlEditController.$inject = ['$rootScope', '$sce', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService', 'cmnfunctionService'];

    function MstCreditBankAccountDtlEditController($rootScope, $sce, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditBankAccountDtlEditController';
        ...
        function activate() { 
            ...
            var url = 'api/MstAppCreditUnderWriting/EditGetCreditBankAccDtl';
        ...
        $scope.Bankacctdtl_Back = function () {
            $location.url('app/MstCreditBankAccountDtlAdd?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&lspage=' + $scope.lspage);
        }
               ...
               ($scope.rdbCheque_Book == undefined) || ($scope.rdbCheque_Book == '')) {
                ...
                if ($scope.rdbJoint_Account == 'Yes') {
                    ...
                    if (($scope.txtjointacctholder_name == undefined) || ($scope.txtjointacctholder_name == '')) {
                        ...
                        var url = 'api/MstAppCreditUnderWriting/UpdateCreditBankAccDtl';
                        lockUI();
                        SocketService.post(url, params).then(function (resp) {
                            unlockUI();
                    ...
                    $location.url('app/MstCreditBankAccountDtlAdd?application_gid=' + application_gid + '&institution_gid=' + institution_gid);
                }
                    }
        }
