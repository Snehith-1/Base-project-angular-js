(function () {
    ...
    angular
        .module('angle')
        .controller('MstCadPhysicalDochecklistController', MstCadPhysicalDochecklistController);

    MstCadPhysicalDochecklistController.$inject = ['$rootScope', '$scope', '$modal', '$state', 'AuthenticationService', 'SweetAlert', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstCadPhysicalDochecklistController($rootScope, $scope, $modal, $state, AuthenticationService, SweetAlert, ScopeValueService, $http, SocketService, Notify, $location, apiManage, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCadPhysicalDochecklistController';
        ...
        $scope.Back = function () {
            if (lspage == 'CadPhysicalDocMaker') {
                $location.url('app/MstCadPhysicalDocGuarantorDtls?application_gid=' + application_gid + '&lspage=CadPhysicalDocMaker&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
            }
            else {
                $location.url('app/MstCadPhysicalDocGuarantorDtls?application_gid=' + application_gid + '&lspage=' + lspage + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
            }
        }
        ...
        $scope.addcovenantperiods = function (groupdocumentchecklist_gid, documenttype_name, covenant_periods, buffer_days, Completed) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstPhysicalDocument/GetPhysicalCovenantPeriodsSummary';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                ...
                $scope.addcovenentperiodsclick = function () {
                    ...
                    console.log(params);
                    var url = 'api/MstPhysicalDocument/PostPhysicalCovenantPeriods';
                ...
                $scope.covenantperiod_delete = function (covenantperioddtl_gid) {
                    ...
                    var url = 'api/MstPhysicalDocument/GetcancelPhysicalCovenantPeriod';
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.tagto_deferral = function (documentcheckdtl_gid, scanned_documentcount) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstPhysicalDocument/GetPhysicalMStDeferraltag';
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
        ...
        $scope.defferal_history = function (documentcheckdtl_gid) {
            $location.url('app/MstPhysicalDeferralHistory?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath);
        }

        $scope.raise_query = function (documentcheckdtl_gid,covenant_type) {
            $location.url('app/MstCadPhysicalDocQuery?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath + '&lscovenant_type=' + covenant_type);
        }

        $scope.extension_waiver = function (documentcheckdtl_gid, deferraltagged) {
            $location.url('app/MstCadPhysicalDocStatus?application_gid=' + application_gid + '&credit_gid=' + credit_gid + '&lspage=' + lspage + '&lstype=' + lstype + '&lsdocumentcheckdtl_gid=' + documentcheckdtl_gid + '&processtypeassign_gid=' + processtypeassign_gid + '&lspath=' + lspath + '&lsdeferraltag=' + deferraltagged);
        }
        ...
        $scope.deferraldocmultiple_upload = function (documentDropdown,covenant_type) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.physicalDocumentUpload = function (val, val1, name) {
                    ...
                    $scope.uploadfrm = frm;
                    if ($scope.uploadfrm != undefined) {
                        var url = 'api/MstPhysicalDocument/PhysicalDocumentMultiUpload';
                        SocketService.postFile(url, $scope.uploadfrm).then(function (resp) {
                            unlockUI();
                            $("#scannedmultiplefile").val('');
                            unlockUI();
