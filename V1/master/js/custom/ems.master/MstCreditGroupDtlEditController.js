(function () {
    ...
    angular
        .module('angle')
        .controller('MstCreditGroupDtlEditController', MstCreditGroupDtlEditController);

    MstCreditGroupDtlEditController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', 'DownloaddocumentService','cmnfunctionService'];

    function MstCreditGroupDtlEditController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCreditGroupDtlEditController';
        ...
        function activate() {
            ...
            var url = 'api/MstAppCreditUnderWriting/GetGroupTempClear';
            ...
            var url = 'api/MstAppCreditUnderWriting/GetGroupEquipmentHoldingList';
            SocketService.getparams(url, param).then(function (resp) {
                $scope.mstequipmentholding_list = resp.data.mstequipmentholding_list;
            });

            var url = 'api/MstAppCreditUnderWriting/GetGroupLivestockList';
            SocketService.getparams(url, param).then(function (resp) {
                $scope.mstlivestockholding_list = resp.data.mstlivestockholding_list;
            });

           
            var url = 'api/MstAppCreditUnderWriting/EditGroup';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
            ...
            var url = 'api/MstAppCreditUnderWriting/GroupDocumentList';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.groupdocument_list = resp.data.groupdocument_list;
            });

            var url = 'api/MstAppCreditUnderWriting/GroupBankList';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.mstbank_list = resp.data.mstbank_list;
            });

            var url = 'api/MstAppCreditUnderWriting/GroupAddressList';
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
        ...
        function groupaddresstemp_list() {
            ...
            var url = 'api/MstAppCreditUnderWriting/GroupAddressTmpList';
            lockUI();
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.mstaddress_list = resp.data.mstaddress_list;
        ...
        $scope.address_add = function () {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.addressSubmit = function () {
                    ...
                    var url = 'api/MstAppCreditUnderWriting/PostGroupAddressDetail';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
        ...
        $scope.address_edit = function (group2address_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
               ...
               SocketService.get(url).then(function (resp) {
                ...
                var url = 'api/MstAppCreditUnderWriting/EditGroupAddressDetail';
              lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                ...
                $scope.address_update = function () {
                    ...
                    var url = 'api/MstAppCreditUnderWriting/UpdateGroupAddressDetail';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        function groupbanktemp_list() {
            ...
            var url = 'api/MstAppCreditUnderWriting/GroupBankTmpList';
           lockUI();
           SocketService.getparams(url, param).then(function (resp) {
               unlockUI();
                $scope.mstbank_list = resp.data.mstbank_list;
        ...
        $scope.bank_add = function () {
            ...
            if (($scope.txtbank_name == '') || ($scope.txtbank_name == undefined) || ($scope.txtbank_branch == '') || ($scope.txtbank_branch == undefined) || ($scope.txtifsc_code == '') || ($scope.txtifsc_code == undefined) || ($scope.txtbank_accountno == '') || ($scope.txtbank_accountno == undefined) || ($scope.txtaccountholder_name == '') || ($scope.txtaccountholder_name == undefined)) {
                ...
                var url = 'api/MstAppCreditUnderWriting/PostGroupBankDetail';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
        ...
        $scope.bank_edit = function (group2bank_gid) {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                var url = 'api/MstAppCreditUnderWriting/EditGroupBankDetail';
                lockUI();
                SocketService.getparams(url, params).then(function (resp) {
                    unlockUI();
                ...
                $scope.update_bank = function () {
                    ...
                    var url = 'api/MstAppCreditUnderWriting/UpdateGroupBankDetail';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
        ...
        $scope.bank_delete = function (group2bank_gid) {
            ...
            console.log(params)
            var url = 'api/MstAppCreditUnderWriting/DeleteGroupBankDetail';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        function groupdocumenttemp_list() {
            ...
            var url = 'api/MstAppCreditUnderWriting/GroupDocumentTmpList';
            lockUI();
            SocketService.getparams(url, param).then(function (resp) {
                unlockUI();
                $scope.groupdocument_list = resp.data.groupdocument_list;
        ...
        $scope.groupdocument_upload = function (val, val1, name) {
            ...
            if (($scope.cbogroupdocumentname == '') || ($scope.cbogroupdocumentname == undefined)) {
                ...
                if ($scope.uploadfrm != undefined) {
                    var url = 'api/MstAppCreditUnderWriting/GroupDocumentUpload';
                  lockUI();
        ...
        $scope.groupdocument_delete = function (group2document_gid) {
            ...
            lockUI();
            var url = 'api/MstAppCreditUnderWriting/GroupDocumentDelete';
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    Notify.alert(resp.data.message, {
        ...
        $scope.downloads = function (val1, val2) {
           ...
           $scope.group_update = function () {
            if (($scope.URN_yes == true) && (($scope.txtgroup_urn == '') || ($scope.txtgroup_urn == undefined))) {
                ...
                var url = 'api/MstAppCreditUnderWriting/UpdateGroupDtl';
                lockUI();
                SocketService.post(url, params).then(function (resp) {
                    unlockUI();
                    if (resp.data.status == true) {
           ...
           $scope.addequipment_holding = function () {
               ...
               function ModalInstanceCtrl($scope, $modalInstance) {
                   ...
                   $scope.equipment_submit = function () {
                       ...
                       var url = 'api/MstAppCreditUnderWriting/PostGroupEquipmentHolding';
                       lockUI();
                       SocketService.post(url, params).then(function (resp) {
                           unlockUI();
                           if (resp.data.status == true) {
           ...
           $scope.deleteequipment_holding = function (group2equipment_gid) {
               ...
               var url = 'api/MstAppCreditUnderWriting/DeleteGroupEquipmentHolding';
               lockUI();
               SocketService.getparams(url, params).then(function (resp) {
                   unlockUI();
                   if (resp.data.status == true) {
           ...
           $scope.equipment_View = function (group2equipment_gid) {
               ...
               function ModalInstanceCtrl($scope, $modalInstance) {
                   ...
                   var url = 'api/MstAppCreditUnderWriting/GetGroupEquipmentHoldingView';
                   lockUI();
                   SocketService.getparams(url, params).then(function (resp) {
                       unlockUI();
           ...
           function equipmentholding_list() {
               ...
               var url = 'api/MstAppCreditUnderWriting/GetEditGroupEquipmentHoldingList';
               SocketService.getparams(url, params).then(function (resp) {
                   $scope.mstequipmentholding_list = resp.data.mstequipmentholding_list;
           ...
           $scope.addlivestock_holding = function () {
               ...
               function ModalInstanceCtrl($scope, $modalInstance) {
                   ...
                   $scope.livestock_submit = function () {
                       ...
                       var url = 'api/MstAppCreditUnderWriting/PostGroupLivestock';
                       lockUI();
                       SocketService.post(url, params).then(function (resp) {
                           unlockUI();
                           if (resp.data.status == true) {
           ...
           $scope.deletelivestock_holding = function (group2livestock_gid) {
               ...
               var url = 'api/MstAppCreditUnderWriting/DeleteGroupLivestock';
               lockUI();
               SocketService.getparams(url, params).then(function (resp) {
                   unlockUI();
                   if (resp.data.status == true) {
           ...
           $scope.livestock_View = function (group2livestock_gid) {
               ...
               function ModalInstanceCtrl($scope, $modalInstance) {
                   ...
                   var url = 'api/MstAppCreditUnderWriting/GetGroupLivestockHoldingView';
                   lockUI();
                   SocketService.getparams(url, params).then(function (resp) {
                       unlockUI();
           ...
           function livestock_list() {
               ...
               var url = 'api/MstAppCreditUnderWriting/GetEditGroupLivestockList';
               SocketService.getparams(url, params).then(function (resp) {
                   $scope.mstlivestockholding_list = resp.data.mstlivestockholding_list;
           ...
           $scope.downloadallgroup = function () {
