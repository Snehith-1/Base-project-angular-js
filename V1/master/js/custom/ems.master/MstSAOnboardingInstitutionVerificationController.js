...
(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAOnboardingInstitutionVerificationController', MstSAOnboardingInstitutionVerificationController);

    MstSAOnboardingInstitutionVerificationController.$inject = ['DownloaddocumentService', '$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function MstSAOnboardingInstitutionVerificationController(DownloaddocumentService, $rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams,cmnfunctionService) {
    /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAOnboardingInstitutionVerificationController';
        ...
        $scope.view_campaignquerydesc = function (query_description, queryresponse_remarks, queryresponse_by) {
            var modalInstance = $modal.open({
                templateUrl: '/makerqueryDescriptionView.html',
        ...
        $scope.final_approval = function () {
            ...
            if (($scope.rbointerview_evalution_form == undefined) || ($scope.rbointerview_evalution_form == '') || ($scope.rboapplication_form == undefined) || ($scope.rboapplication_form == '') || ($scope.rbocancelledcheckleaf == undefined) || ($scope.rbocancelledcheckleaf == '') || ($scope.rbokycdocuments == undefined) || ($scope.rbokycdocuments == '') || ($scope.rbophotographs == undefined) || ($scope.rbophotographs == '') || ($scope.rbovettingstatus == undefined) || ($scope.rbovettingstatus == '') || ($scope.rboaddressproof == undefined) || ($scope.rboaddressproof == '')) {
                ...
                Notify.alert("Enter Samfin agreement Details");
            }
                ...
                Notify.alert("Enter Samagro agreement Details");
            }
            else {
                var params = {
                    //satype_gid: $scope.cbosatype,
        ...
        $scope.credit_amountChange = function () {
            ...
            var str = input.replace(/,/g, '');
            var output = Number(str).toLocaleString('en-IN');
            var lswords_creditedamount = cmnfunctionService.fnConvertNumbertoWord(str);
            if (output == "NaN") {
                Notify.alert('Accept Number Format Only..!', {
                    status: 'danger',
                    pos: 'top-center',
        ...
        $scope.checkerinstitutionraisequery = function (sacontactinstitution_gid) {
            var modalInstance = $modal.open({
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.submit = function () {
                    ...
                    var url = 'api/MstSAOnboardingInstitution/PostCheckerInstitutionRaiseQuery';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
                            ...
                            //activate();
                            checkerraise_list(sacontactinstitution_gid);
                        }
                        else {
                            Notify.alert(resp.data.message, {
                                status: 'info',
        ...
        function checkerraise_list(sacontactinstitution_gid) {
            var params = {
                sacontactinstitution_gid: sacontactinstitution_gid,
        ...
        $scope.reject = function () {
            ...
            function ModalInstanceCtrl($scope, $modalInstance) {
                ...
                $scope.rejectSubmit = function () {
                    ...
                    var url = 'api/MstSAOnboardingInstitution/InstitutionCheckerRejected';
                    lockUI();
                    SocketService.post(url, params).then(function (resp) {
                        unlockUI();
                        if (resp.data.status == true) {
