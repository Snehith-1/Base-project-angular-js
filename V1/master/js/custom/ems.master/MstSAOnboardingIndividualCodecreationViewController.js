...
(function () {
    ...
    angular
        .module('angle')
        .controller('MstSAOnboardingIndividualCodecreationViewController', MstSAOnboardingIndividualCodecreationViewController);

    MstSAOnboardingIndividualCodecreationViewController.$inject = ['DownloaddocumentService', '$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams','cmnfunctionService'];

    function MstSAOnboardingIndividualCodecreationViewController(DownloaddocumentService, $rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstSAOnboardingIndividualCodecreationViewController';
        ...
        function activate() {
            ...
            SocketService.getparams(url, param).then(function (resp) {
                ...
                $scope.txtrasteason_date = resp.data.ratingas_date;
                $scope.txtsamfin = resp.data.samfin_code;
                $scope.txtsamagro = resp.data.samagro_code;
                $scope.codecreation_date = resp.data.codecreation_date;
            ...
            var url = 'api/MstSAOnboardingIndividual/SamcodesAutoGenerate';

            SocketService.getparams(url,param).then(function (resp) {
                $scope.txtsamfin = resp.data.fincode;
                $scope.txtsamagro = resp.data.agrocode;
        ...
        $scope.datecheck = function (val) {
            var params = {
                date: val.toDateString()
            }
            var url = 'api/MstSAOnboardingInstitution/DateCheck';
            SocketService.getparams(url, params).then(function (resp) {
                if (resp.data.status == false) {
                    Notify.alert(resp.data.message, 'warning')
                    $scope.codecreation_date = '';
                }
            });
        }
        ...
        $scope.submit_code = function () {
            if ($scope.txtsamfin == undefined || $scope.txtsamfin == null || $scope.txtsamfin == "") {

                Notify.alert("Enter samfin code", 'warning')
            }
            else if ($scope.txtsamagro == undefined || $scope.txtsamagro == null || $scope.txtsamagro == "") {
                Notify.alert("Enter samagro code", 'warning')
            }
            else {
                var params = {
                    sacontact_gid: sacontact_gid,

                    samfin_code: $scope.txtsamfin,
                    samagro_code: $scope.txtsamagro,
                    codecreation_date: $scope.txt_codedate
                }

                var url = 'api/MstSAOnboardingBussDevtVerification/IndividualSamCodesupdate';
                lockUI();
        ...
        $scope.back = function () {
            ...
            if (lspage == 'IndividualPending') {
                $location.url('app/MstSAApprovalIndCodePendingSummary');
            }
            else if (lspage == 'IndividualInitiate') {
                $location.url('app/MstSAOnboardingCodeApprovalIndSummary');
            }
        }
        ...
        $scope.Approve = function (txtremarks) {
            ...
            lockUI();
            var url = "api/MstSAOnboardingIndividual/PostApprove";
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    Notify.alert(resp.data.message, {
        ...
        $scope.Reject = function (txtremarks) {
            ...
            lockUI();
            var url = "api/MstSAOnboardingIndividual/PostRejected";
            SocketService.post(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
                    Notify.alert(resp.data.message, {
        ...
        /* var url = 'api/';
         SocketService.get(url, params).then(function (resp) {
             $scope.prospects_list=resp.data.application_list;
         });
         var url = 'api/A';
         SocketService.get(url, params).then(function (resp) {
         $scope.individual_list=resp.data.application_list;
         });
         var url = 'api/';
         SocketService.get(url, params).then(function (resp) {
         $scope.mobileno_list=resp.data.application_list;
         });
         var url = 'api/';
         SocketService.get(url, params).then(function (resp) {
         $scope.mailaddress_list=resp.data.application_list;
         }); 
         var url = 'api/';
         SocketService.get(url, params).then(function (resp) {
        ...
        //        };
        //        var url = 'api/MstSAOnboardingIndividual/CICUploadIndividualDocList';
        //        SocketService.getparams(url, param).then(function (resp) {
