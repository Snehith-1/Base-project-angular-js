(function () {
    ...
    angular
        .module('angle')
        .controller('MstCADCreditBankAccountDtlAddController', MstCADCreditBankAccountDtlAddController);

    MstCADCreditBankAccountDtlAddController.$inject = ['$rootScope', '$scope', '$state', 'AuthenticationService', '$modal', 'ScopeValueService', '$http', 'SocketService', 'Notify', '$location', 'apiManage', 'SweetAlert', '$route', 'ngTableParams', '$sce', 'DownloaddocumentService','cmnfunctionService'];

    function MstCADCreditBankAccountDtlAddController($rootScope, $scope, $state, AuthenticationService, $modal, ScopeValueService, $http, SocketService, Notify, $location, apiManage, SweetAlert, $route, ngTableParams, $sce, DownloaddocumentService, cmnfunctionService) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'MstCADCreditBankAccountDtlAddController';
        ...
        function activate() {
            ...
            var url = 'api/MstCADCreditAction/GetCrediBankAccDtl';
            lockUI();
            ...
            ($scope.cboAccountType == undefined) || ($scope.cboAccountType == '') || ($scope.rdbJoint_Account == undefined) || ($scope.rdbJoint_Account == '') ||
                ...
                if ($scope.rdbJoint_Account == 'Yes')
                {
                    ...
                    if(($scope.txtjointacctholder_name == undefined)||($scope.txtjointacctholder_name == ''))
                    {
                        ...
                        var url = 'api/MstCADCreditAction/PostCreditBank';
                        lockUI();
                        SocketService.post(url, params).then(function (resp) {
                            unlockUI();
        ...
        $scope.creditbankacctdtl_delete = function (creditbankdtl_gid) {
            ...
            var url = 'api/MstCADCreditAction/DeletecreditBankAcc';
            lockUI();
            SocketService.getparams(url, params).then(function (resp) {
                unlockUI();
                if (resp.data.status == true) {
        ...
        $scope.creditbankacctdtl_edit = function (creditbankdtl_gid) {
            $location.url('app/MstCADCreditBankAccountDtlEdit?application_gid=' + application_gid + '&institution_gid=' + institution_gid + '&creditbankdtl_gid=' + creditbankdtl_gid + '&lspage=' + lspage);
        }
